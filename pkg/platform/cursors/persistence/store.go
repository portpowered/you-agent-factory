...
func (s *FileStore) Load(
	...
	encoded, err := os.ReadFile(s.checkpointPath(identity))
...
func (s *FileStore) Save(
	...
	temporary, err := os.CreateTemp(s.dir, ".cursor-*.tmp")
	if err != nil {
	...
	if err := os.Rename(temporaryPath, s.checkpointPath(identity)); err != nil {
