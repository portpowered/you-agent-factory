...
import (
	...
	"github.com/portpowered/infinite-you/pkg/platform/cursors/persistence"
)

func TestFileStoreRestartResumesAtNextObservableEvent(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "cursors")
	identity := testIdentity()
	first, err := persistence.NewFileStore(dir)
	if err != nil {
	...
	reconstructed, err := persistence.NewFileStore(dir)
	if err != nil {
...
func TestFileStoreValidatesConstructionAndRequests(t *testing.T) {
	if _, err := persistence.NewFileStore(" "); err == nil {
