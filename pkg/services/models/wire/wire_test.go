...
func TestNewServiceWithBackendArtifactResolverAndInvocationBackendConstructsRoot(t *testing.T) {
	t.Parallel()

	service, err := validConstructionEdges().newServiceWithInvocationBackends()
	if err != nil {
...
func (edges constructionEdges) newServiceWithInvocationBackends() (models.Service, error) {
	return NewServiceWithBackendArtifactResolverAndInvocationBackend(
		edges.assetPlatform,
		edges.assetHTTP,
		edges.assetEndpoints,
		edges.assetMkdirAll,
...
func TestCloneASRParametersDetachesNestedValues(t *testing.T) {
	...
	cloned := cloneASRParameters(original)
	...
	if original["nested"].(map[string]any)["value"] != "original" ||
		...
		t.Fatalf("cloneASRParameters mutated original = %#v", original)
	}
}
