...
func TestRootAdapter_InvokeGenericZeroInputsUseCatalogRequiredSlots(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "asr",
			model:     modelinference.BuiltInModelNameASR,
			operation: modelinference.OperationASR,
			inputs: []modelinference.OperationSlot{{
				Name: "audio", Modality: modelinference.ModalityAudio, Required: boolPointer(true),
			}},
			outputs:        []modelinference.OperationSlot{{Name: "transcript", Modality: modelinference.ModalityText}},
			wantMissing:    []string{"audio"},
			wantValidNames: []string{"audio"},
			wantMessage:    "required input slot is missing: audio",
		},
		{
			name:      "tts",
			model:     modelinference.BuiltInModelNameTTS,
			operation: modelinference.OperationTTS,
			inputs: []modelinference.OperationSlot{{
			...
			wantMissing:    []string{"text"},
			wantValidNames: []string{"text"},
			wantMessage:    "required input slot is missing: text",
		},
		{
			name:      "embed",
			model:     modelinference.BuiltInModelNameEmbed,
		...
		{
			name:      "omni",
			model:     modelinference.BuiltInModelNameLLM,
			operation: modelinference.OperationOMNI,
			inputs: []modelinference.OperationSlot{{
				Name: "prompt", Modality: modelinference.ModalityText, Required: boolPointer(true),
			}},
			outputs:        []modelinference.OperationSlot{{Name: "text", Modality: modelinference.ModalityText}},
			wantMissing:    []string{"prompt"},
			wantValidNames: []string{"prompt"},
			wantMessage:    "required input slot is missing: prompt",
		},
		{
			name:      "operator-defined multi-slot operation",
			...
			wantValidNames: []string{"alpha", "optional", "zeta"},
...
func assertMappedCLIResponse(t *testing.T, data []byte) {
	...
	if response.Outputs[1].Artifact == nil || response.Outputs[1].Artifact.ArtifactRef != "artifact:usage" || response.Outputs[1].Artifact.SizeBytes == nil || *response.Outputs[1].Artifact.SizeBytes != 14 || response.Outputs[1].Artifact.Properties == nil || (*response.Outputs[1].Artifact.Properties)["digest"] != "sha256:usage" {
