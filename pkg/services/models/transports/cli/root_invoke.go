...
func (service *rootService) invokeGeneric(cfg InvokeConfig, modelName string) error {
	if strings.TrimSpace(cfg.OutputPath) != "" {
		return fmt.Errorf("--output cannot be used with generic --input; use --output-map or --json")
	}
	inputs, err := parseGenericCLIInputs(cfg.Context, cfg.InputBindings, service.inputFileReader)
	if err != nil {
		return err
	}
	if len(inputs) == 0 {
		return fmt.Errorf("--input requires at least one slot binding")
	}
	if service.openInvokeScope == nil {
		return fmt.Errorf("models invoke runtime scope opener is required")
	}
	...
	outputMode := modelinference.OutputModeAuto
	if cfg.JSON {
		outputMode = modelinference.OutputModeJSON
	}
	result, err := service.models.InvokeModel(cfg.Context, modelinference.InvokeModelRequest{
		Scope:      scope.Scope,
		Holder:     modelsCLIInvokeHolder,
