...
func TestGenericCLIOutputPublicationHelpersCoverSuccessAndFailures(t *testing.T) {
	...
	mappings := map[string]genericCLIOutputMapping{"text": {slot: "text", path: "result.txt"}}
	...
	if _, err := stageGenericCLIOutputs(ctx, fileSystem, result, map[string]genericCLIOutputMapping{}); err == nil || !strings.Contains(err.Error(), "unmapped output") {
	...
	if _, err := stageGenericCLIOutputs(ctx, fileSystem, emptyResult, mappings); err == nil || !strings.Contains(err.Error(), "no inline bytes") {
		t.Fatalf("empty output error = %v, want no inline bytes", err)
	}

	writeFailure := &genericOutputFileSystem{writeErr: errors.New("write failed")}
	if _, err := stageGenericCLIOutputs(ctx, writeFailure, result, mappings); err == nil || !strings.Contains(err.Error(), "write mapped output") {
		t.Fatalf("write failure = %v, want mapped output context", err)
	}
	closeFailure := &genericOutputFileSystem{closeErr: errors.New("close failed")}
	if _, err := stageGenericCLIOutputs(ctx, closeFailure, result, mappings); err == nil || !strings.Contains(err.Error(), "close failed") {
		t.Fatalf("close failure = %v, want close error", err)
	}
	shortWrite := &genericOutputFileSystem{shortWrite: true}
	if _, err := stageGenericCLIOutputs(ctx, shortWrite, result, mappings); !errors.Is(err, io.ErrShortWrite) {
