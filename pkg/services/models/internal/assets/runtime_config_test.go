...
func projectTestModelsRuntimeConfig(factoryDir string, cfg *testFactoryConfig) *modelRuntimeConfig {
	return projectModelsTestConfig(factoryDir, cfg)
}

func projectModelsTestConfig(factoryDir string, cfg *testFactoryConfig) *modelRuntimeConfig {
	if cfg == nil {
		return nil
	}
	result := &modelRuntimeConfig{FactoryDirectory: factoryDir}
	result.Resources = append([]modelRuntimeResource(nil), cfg.Resources...)
	result.Workers = append([]modelRuntimeWorker(nil), cfg.Workers...)
	return result
}
