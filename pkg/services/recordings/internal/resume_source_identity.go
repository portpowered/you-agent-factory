...
func resumeSourceCanonicalSessionIDForPath(
	...
	// A resumable artifact may contain a canonical identity before it has
	// crossed the terminal source close. It remains a valid restart input, but
	// it is not a valid metrics predecessor; retaining it here would make a
	// successor appear to have durable source lineage that does not exist.
	if !replayInputHasSessionCompleted(input) {
		return "", nil
	}
