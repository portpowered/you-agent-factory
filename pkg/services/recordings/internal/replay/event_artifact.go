...
func runStartedEventFromSnapshot(recordedAt time.Time, factorySnapshot *interfaces.FactorySnapshot, wallClock *interfaces.ReplayWallClockMetadata, diagnostics interfaces.ReplayDiagnostics) (interfaces.FactoryEvent, error) {
	if recordedAt.IsZero() {
		return interfaces.FactoryEvent{}, fmt.Errorf("recorded at is required")
	}
...
func hydrateArtifactFromEvents(artifact *interfaces.ReplayArtifact) error {
	return hydrateArtifactFromEventsWithDecoder(artifact, factorySnapshotFromJSON)
}
...
func hydrateArtifactFromEventsAtBoundary(
	...
	return hydrateArtifactFromEventsWithDecoder(
		artifact,
		func(data []byte) (*interfaces.FactorySnapshot, error) {
			return validatedFactorySnapshotFromJSON(data, decode)
		},
	)
}

func hydrateArtifactFromEventsWithDecoder(
	artifact *interfaces.ReplayArtifact,
	...
	for _, event := range artifact.Events {
		...
		case interfaces.FactoryEventTypeRunRequest:
			payload, err := runStartedPayloadFromEventWithDecoder(event, decode)
			if err != nil {
				return err
			}
...
func runStartedPayloadFromEvent(event interfaces.FactoryEvent) (interfaces.RunRequestEventPayload, error) {
	return runStartedPayloadFromEventWithDecoder(event, factorySnapshotFromJSON)
}
...
func runStartedPayloadFromEventAtBoundary(
	...
	return runStartedPayloadFromEventWithDecoder(
		event,
		func(data []byte) (*interfaces.FactorySnapshot, error) {
			return validatedFactorySnapshotFromJSON(data, decode)
		},
...
func runStartedPayloadFromEventWithDecoder(
	event interfaces.FactoryEvent,
	decode func([]byte) (*interfaces.FactorySnapshot, error),
) (interfaces.RunRequestEventPayload, error) {
	var raw struct {
	...
	factorySnapshot, err := runStartedFactorySnapshotFromEventWithDecoder(event, decode)
	if err != nil {
...
func runStartedFactorySnapshotFromEventWithDecoder(
	...
	factorySnapshot, err := decode(normalizedFactoryJSON)
	if err != nil {
