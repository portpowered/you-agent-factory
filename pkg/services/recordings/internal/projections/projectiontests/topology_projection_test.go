...
func TestReconstructCanonicalFactoryWorldState_DecodesLegacyDomainWorkTypes(t *testing.T) {
	snapshot, err := interfaces.NewFactorySnapshot(map[string]any{
		"name": "legacy-domain-factory",
		"work_types": []any{map[string]any{
			"id": "task", "name": "task",
			"states": []any{
				map[string]any{"id": "init", "name": "init", "type": "INITIAL"},
				map[string]any{"id": "done", "name": "done", "type": "TERMINAL"},
			},
		}},
		"resources": []any{}, "workers": []any{}, "workstations": []any{},
	})
	if err != nil {
		t.Fatalf("NewFactorySnapshot: %v", err)
	}
	...
	event := interfaces.FactoryEvent{
		Id: "legacy-domain-initial", SchemaVersion: interfaces.FactoryEventSchemaVersionV1,
		Type:    interfaces.FactoryEventTypeInitialStructureRequest,
		Context: interfaces.FactoryEventContext{Tick: 0}, Payload: payload,
	}
	...
	if len(worldState.Topology.WorkTypes) != 1 || worldState.Topology.WorkTypes[0].ID != "task" {
		t.Fatalf("work types = %#v, want legacy task definition", worldState.Topology.WorkTypes)
	}
	if got := worldState.Topology.Places; !reflect.DeepEqual(got, []interfaces.FactoryPlace{
		{ID: "task:init", TypeID: "task", State: "init", Category: "INITIAL"},
		{ID: "task:done", TypeID: "task", State: "done", Category: "TERMINAL"},
	}) {
		t.Fatalf("places = %#v, want legacy task state places", got)
	}
}
