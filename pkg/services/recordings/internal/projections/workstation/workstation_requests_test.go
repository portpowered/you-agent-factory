...
func TestBuildFactoryWorldWorkstationRequestProjectionSlice_UsesTerminalWorkFallbackLineage(t *testing.T) {
	completedInput := work.FactoryWorkItem{
	...
	terminalOutput := work.FactoryWorkItem{
	...
	slice := BuildFactoryWorldWorkstationRequestProjectionSlice(interfaces.FactoryWorldState{
		...
		CompletedDispatches: []interfaces.FactoryWorldDispatchCompletion{{
			...
			ConsumedInputs: []interfaces.WorkstationInput{{
				...
				PlaceID:  workItemPlaceID(completedInput),
		...
		ActiveDispatches: map[string]interfaces.FactoryWorldDispatch{
			"dispatch-pending": {
				...
				Inputs: []interfaces.WorkstationInput{{
					...
					PlaceID:  workItemPlaceID(workItem),
					WorkItem: &workItem,
				}},
	...
	state := interfaces.FactoryWorldState{
		...
		ActiveDispatches: map[string]interfaces.FactoryWorldDispatch{
			"dispatch-copied-optionals": {
				...
				Workstation:              interfaces.FactoryWorkstationRef{ID: "review", Name: "Review"},
				StartedAt:                time.Date(2026, 4, 24, 12, 0, 0, 0, time.UTC),
				Inputs:                   []interfaces.WorkstationInput{{TokenID: "token-copied-optionals", PlaceID: workItemPlaceID(workItem), WorkItem: &workItem}},
				WorkItemIDs:              []string{workItem.ID},
...
func TestBuildFactoryWorldWorkstationRequestProjectionSlice_UsesDispatchTimeConsumedPayloadSnapshot(t *testing.T) {
	...
	latest := initial
	latest.State = "done"
	latest.Content = []work.WorkContentPart{{
		Type: work.WorkContentPartTypeText,
		Text: "latest payload after downstream update",
	}}

	var lineage work.WorkPayloadLineageProjection
	...
	slice := BuildFactoryWorldWorkstationRequestProjectionSlice(interfaces.FactoryWorldState{
		...
		CompletedDispatches: []interfaces.FactoryWorldDispatchCompletion{{
			...
			ConsumedInputs: []interfaces.WorkstationInput{{
				...
				PlaceID:  workItemPlaceID(initial),
				WorkItem: &latest,
			}},
			InputWorkItems: []work.FactoryWorkItem{latest},
		}},
	})
	...
	if resolved.PayloadStatus == nil || *resolved.PayloadStatus != WorkstationFactoryWorldWorkItemRefPayloadStatus("RESOLVED") {
		t.Fatalf("resolved consumed payload status = %#v, want RESOLVED", resolved.PayloadStatus)
	}
	assertGeneratedWorkContentParts(t, resolved.Content, []work.WorkContentPart{{
		Type: work.WorkContentPartTypeText,
		Text: "dispatch-time payload",
	}})
}
...
func TestBuildFactoryWorldWorkstationRequestProjectionSlice_ReplayFixtureProjectsHistoricalConsumedPayloads(t *testing.T) {
	events := loadWorkstationProjectionReplayFixtureEvents(t, "pkg", "services", "recordings", "internal", "projections", "testdata", "work-payload-lineage-replay.jsonl")
	...
	if followUp.PayloadStatus == nil || *followUp.PayloadStatus != WorkstationFactoryWorldWorkItemRefPayloadStatus("RESOLVED") {
	...
	assertGeneratedWorkContentParts(t, followUp.Content, []work.WorkContentPart{{
		Type: work.WorkContentPartTypeText,
		Text: "child-v1",
	}})
	...
	if missing.PayloadStatus == nil || *missing.PayloadStatus != WorkstationFactoryWorldWorkItemRefPayloadStatus("UNAVAILABLE") {
...
func requireWorkstationProjectionRequests(t *testing.T, slice WorkstationFactoryWorldWorkstationRequestProjectionSlice, want int) map[string]WorkstationFactoryWorldWorkstationRequestView {
	t.Helper()
	if slice.WorkstationRequestsByDispatchId == nil {
		t.Fatal("workstation request slice missing generated projection map")
	}
...
func appendEmptyAndMultiPartProjectionFixtures(state *interfaces.FactoryWorldState) {
	emptyItem := work.FactoryWorkItem{
		ID:          "work-empty",
		WorkTypeID:  "task",
	...
	state.WorkItemsByID[emptyItem.ID] = work.FactoryWorkItem{
		ID:          emptyItem.ID,
		WorkTypeID:  emptyItem.WorkTypeID,
		DisplayName: emptyItem.DisplayName,
		TraceID:     emptyItem.TraceID,
		State:       emptyItem.State,
	}

	multiPartItem := work.FactoryWorkItem{
		ID:          "work-multi",
		WorkTypeID:  "task",
...
func assertProjectedNilAndMultiPartContent(
	t *testing.T,
	requests map[string]WorkstationFactoryWorldWorkstationRequestView,
) {
	t.Helper()
	...
	assertGeneratedWorkContentParts(t, (*completedItems)[0].Content, []work.WorkContentPart{{
		Type: work.WorkContentPartTypeText,
	...
	assertGeneratedWorkContentParts(t, (*multiItems)[0].Content, []work.WorkContentPart{{
		Type: work.WorkContentPartTypeText,
		Text: "first multi-part payload",
	}, {
		Type:  work.WorkContentPartTypeImage,
...
func loadWorkstationProjectionReplayFixtureEvents(t *testing.T, rel ...string) []interfaces.FactoryEvent {
	t.Helper()
...
func lastWorkstationProjectionFixtureTick(events []interfaces.FactoryEvent) int {
	tick := 0
...
func workstationRequestProjectionStateFixture() interfaces.FactoryWorldState {
	...
	activeWork := work.FactoryWorkItem{
		ID:                     "work-active",
		WorkTypeID:             "task",
		State:                  "queued",
	...
	completedOutput := work.FactoryWorkItem{ID: "work-completed-output", WorkTypeID: "task", DisplayName: "Completed story", ChainingTraceDepth: 4, CurrentChainingTraceID: "chain-completed", PreviousChainingTraceIDs: []string{"chain-parent-a", "chain-parent-z"}, TraceID: "chain-completed", State: "done"}
	var lineage work.WorkPayloadLineageProjection
	...
	lineage.RecordDispatchOutputSnapshot(4, "dispatch-completed", []work.FactoryWorkItem{completedInput}, completedOutput, 0)
	return interfaces.FactoryWorldState{
		...
		ActiveDispatches: map[string]interfaces.FactoryWorldDispatch{
			"dispatch-active": {
				...
				Inputs:      []interfaces.WorkstationInput{{TokenID: "token-active", PlaceID: workItemPlaceID(activeWork), WorkItem: &activeWork}},
		...
		CompletedDispatches: []interfaces.FactoryWorldDispatchCompletion{{
			...
			ConsumedInputs: []interfaces.WorkstationInput{{TokenID: "token-completed", PlaceID: workItemPlaceID(completedInput), WorkItem: &completedInput}},
...
func assertActiveProjectionRequest(t *testing.T, active WorkstationFactoryWorldWorkstationRequestView) {
	t.Helper()
	...
	if active.Request.InputWorkItems == nil || (*active.Request.InputWorkItems)[0].PayloadStatus == nil || *(*active.Request.InputWorkItems)[0].PayloadStatus != WorkstationFactoryWorldWorkItemRefPayloadStatus("RESOLVED") {
		t.Fatalf("active consumed payload status = %#v, want resolved lineage payload", active.Request.InputWorkItems)
	}
	assertGeneratedWorkContentParts(t, (*active.Request.InputWorkItems)[0].Content, []work.WorkContentPart{{
		Type: work.WorkContentPartTypeText,
		Text: "Active dispatch payload",
	}})
...
func assertCompletedProjectionRequest(t *testing.T, completed WorkstationFactoryWorldWorkstationRequestView) {
	t.Helper()
	...
	if (*completed.Request.InputWorkItems)[0].PayloadStatus == nil || *(*completed.Request.InputWorkItems)[0].PayloadStatus != WorkstationFactoryWorldWorkItemRefPayloadStatus("RESOLVED") {
		t.Fatalf("completed consumed payload status = %#v, want resolved lineage payload", completed.Request.InputWorkItems)
	}
	assertGeneratedWorkContentParts(t, (*completed.Request.InputWorkItems)[0].Content, []work.WorkContentPart{{
		Type: work.WorkContentPartTypeText,
		Text: "Completed dispatch payload",
	}})
	...
	if outputItems := *completed.Response.OutputWorkItems; outputItems[0].LineageContinuity == nil || *outputItems[0].LineageContinuity != WorkstationFactoryWorldWorkItemRefLineageContinuity("NEW_DOWNSTREAM_WORK") {
...
func assertProjectionRoundTripDetachesState(t *testing.T, state interfaces.FactoryWorldState, slice WorkstationFactoryWorldWorkstationRequestProjectionSlice) {
	t.Helper()
	encoded, err := json.Marshal(slice)
	if err != nil {
		t.Fatalf("Marshal(slice): %v", err)
	}
	var roundTripped WorkstationFactoryWorldWorkstationRequestProjectionSlice
...
func scriptProjectionStateFixture() interfaces.FactoryWorldState {
	...
	workItem := work.FactoryWorkItem{ID: "work-scripted", WorkTypeID: "task", DisplayName: "Scripted story", TraceID: "trace-scripted", State: "init"}
	exitCode := 124
	return interfaces.FactoryWorldState{
		...
		ActiveDispatches: map[string]interfaces.FactoryWorldDispatch{
			scriptProjectionActiveDispatchID: {
				...
				StartedAt: t0, Inputs: []interfaces.WorkstationInput{{TokenID: "token-script-active", PlaceID: workItemPlaceID(workItem), WorkItem: &workItem}}, WorkItemIDs: []string{workItem.ID}, TraceIDs: []string{workItem.TraceID},
			},
		},
		CompletedDispatches: []interfaces.FactoryWorldDispatchCompletion{{
			...
			ConsumedInputs: []interfaces.WorkstationInput{{TokenID: "token-script-completed", PlaceID: workItemPlaceID(workItem), WorkItem: &workItem}},
...
func assertActiveScriptProjection(t *testing.T, active WorkstationFactoryWorldWorkstationRequestView) {
	t.Helper()
...
func assertCompletedScriptProjection(t *testing.T, completed WorkstationFactoryWorldWorkstationRequestView) {
	t.Helper()
...
func completedInputSlice(item work.FactoryWorkItem) []work.FactoryWorkItem {
	return []work.FactoryWorkItem{item}
}
...
func TestWorkItemRefsForAPIProjection_FilterCustomerWorkAndPreserveLineage(t *testing.T) {
	...
	refsForItems := workItemRefsForItems([]work.FactoryWorkItem{
		itemsByID["work-2"],
		itemsByID["time-1"],
		itemsByID["work-2"],
		itemsByID["work-1"],
