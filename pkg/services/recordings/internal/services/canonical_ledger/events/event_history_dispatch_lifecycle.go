...
// ErrReconnectCursorNotFound retains the ledger implementation's local name
// while the public error identity belongs to the Recordings service root.
var ErrReconnectCursorNotFound = recordings.ErrReconnectCursorNotFound
...
func latestCanonicalEventTime(events []interfaces.FactoryEvent) time.Time {
	var latest time.Time
	for _, event := range events {
		if event.Context.EventTime.After(latest) {
			latest = event.Context.EventTime
		}
	}
	return latest.UTC()
}
