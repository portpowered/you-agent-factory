...
func assertPackageTargetManifestRegistration(t *testing.T) {
	...
	for _, row := range manifest.Packages {
		...
		if row.Destination != recordingsOwner {
			t.Fatalf("package-target manifest destination = %q, want %q", row.Destination, recordingsOwner)
		}
		return
	}
...
func assertOwnershipInventoryRegistration(t *testing.T) {
	...
	for _, row := range inventory.Packages {
		...
		if row.Destination != recordingsOwner {
			t.Fatalf("ownership inventory destination = %q, want %q", row.Destination, recordingsOwner)
		}
