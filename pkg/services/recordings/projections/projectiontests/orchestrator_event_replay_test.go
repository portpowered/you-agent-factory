...
func TestReconstructFactoryWorldState_SessionResultUpdatedMatchesSessionResultProjection(t *testing.T) {
	...
	ownerProjection := factory.SessionResultProjection{
		Durable: factory.SessionResult{
			SessionID:     sessionID,
			ResultStatus:  factory.ResultStatusFinal,
			PrimaryResult: resultParts,
			ArtifactIDs:   []string{resultArtifact.ID},
			ArtifactRefs:  []interfaces.FactoryArtifactRef{resultArtifact},
		},
		Updated: factory.SessionResultUpdatedPayload{
