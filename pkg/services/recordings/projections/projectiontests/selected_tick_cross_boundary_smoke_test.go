...
type selectedTickWorkstationProjector struct {
	result   recordings.WorkstationFactoryWorldWorkstationRequestProjectionSlice
	received interfaces.FactoryWorldState
}

var _ recordings.WorkstationRequestProjector = (*selectedTickWorkstationProjector)(nil)

func (projector *selectedTickWorkstationProjector) ProjectWorkstationRequests(
	state interfaces.FactoryWorldState,
) recordings.WorkstationFactoryWorldWorkstationRequestProjectionSlice {
	projector.received = state
	return projector.result
}

func selectedTickWorkstationProjectionFixture(
	t0 time.Time,
) recordings.WorkstationFactoryWorldWorkstationRequestProjectionSlice {
	completedStartedAt := t0.Add(5 * time.Second)
	completedAt := t0.Add(8 * time.Second)
	completedDuration := int64(875)
	requests := map[string]recordings.WorkstationFactoryWorldWorkstationRequestView{
		"dispatch-runtime-pending": {
			DispatchId: "dispatch-runtime-pending",
			Counts: recordings.WorkstationFactoryWorldWorkstationRequestCountView{
				DispatchedCount: 1,
			},
		},
		"dispatch-runtime-completed": {
			DispatchId: "dispatch-runtime-completed",
			Counts: recordings.WorkstationFactoryWorldWorkstationRequestCountView{
				DispatchedCount: 1,
				RespondedCount:  1,
			},
			...
			Response: &recordings.WorkstationFactoryWorldWorkstationRequestResponseView{
				EndTime:        &completedAt,
				DurationMillis: &completedDuration,
			},
		},
		"dispatch-runtime-failed": {
			DispatchId: "dispatch-runtime-failed",
			Counts: recordings.WorkstationFactoryWorldWorkstationRequestCountView{
				DispatchedCount: 1,
				ErroredCount:    1,
			},
	...
	return recordings.WorkstationFactoryWorldWorkstationRequestProjectionSlice{
		WorkstationRequestsByDispatchId: &requests,
	}
}
...
func assertSelectedTickWorkstationRequests(
	...
	if active.Response != nil {
		t.Fatalf("active request = %#v, want no response yet", active)
	}
