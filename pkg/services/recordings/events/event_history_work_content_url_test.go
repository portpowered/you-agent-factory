...
func TestFactoryEventHistory_RecordWorkRequest_SerializesContentURLNotMaterializedPaths(t *testing.T) {
	...
	// Represent a dispatch-only path without constructing the Work
	// implementation that creates it. Event history must never persist it.
	materializedPath := filepath.Join(t.TempDir(), "workcontent-dispatch-only.png")
	...
	history := newTestFactoryEventHistory(eventHistoryProjectionNet(), func() time.Time {
