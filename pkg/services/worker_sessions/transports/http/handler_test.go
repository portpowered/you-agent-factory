...
func TestListWorkerSessionsTranslatesPositiveLimit(t *testing.T) {
	...
	handler := NewHandler(NewAdapter(service, workServiceStub{}), zap.NewNop())
	recorder := httptest.NewRecorder()
	handler.ListWorkerSessions(recorder, httptest.NewRequest(http.MethodGet, "/worker-sessions?limit=2", nil), factoryapi.ListWorkerSessionsParams{Limit: &limit})
...
func TestListWorkerSessionsRejectsNonPositiveLimit(t *testing.T) {
	...
	handler := NewHandler(NewAdapter(service, workServiceStub{}), zap.NewNop())
	recorder := httptest.NewRecorder()
	handler.ListWorkerSessions(recorder, httptest.NewRequest(http.MethodGet, "/worker-sessions?limit=0", nil), factoryapi.ListWorkerSessionsParams{Limit: &limit})
...
func TestListWorkerSessionsMapsInvalidScopeAndStateErrors(t *testing.T) {
	for _, testCase := range []struct {
		...
		{name: "scope", err: workersessions.ErrInvalidObservationScope, params: func() factoryapi.ListWorkerSessionsParams {
			scope := factoryapi.ListWorkerSessionsParamsScope("unexpected")
			return factoryapi.ListWorkerSessionsParams{Scope: &scope}
		}()},
		{name: "state", err: workersessions.ErrInvalidState, params: func() factoryapi.ListWorkerSessionsParams {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			handler := NewHandler(NewAdapter(service, workServiceStub{}), zap.NewNop())
			recorder := httptest.NewRecorder()
			handler.ListWorkerSessions(recorder, httptest.NewRequest(http.MethodGet, "/worker-sessions", nil), testCase.params)
			if recorder.Code != http.StatusBadRequest {
				t.Fatalf("status = %d, want 400; body=%s", recorder.Code, recorder.Body.String())
			}
			var response factoryapi.ErrorResponse
...
func TestListWorkerSessionsProjectsFleetAttributionAndUnavailableFacts(t *testing.T) {
	...
	workResults := map[string]work.ReadModel{
	...
	handler := NewHandler(NewAdapter(service, workServiceStub{getResults: workResults}), zap.NewNop())
	recorder := httptest.NewRecorder()
	handler.ListWorkerSessions(recorder, httptest.NewRequest(http.MethodGet, "/worker-sessions", nil), factoryapi.ListWorkerSessionsParams{})
	...
	if response.Sessions[0].WorkId == nil || *response.Sessions[0].WorkId != "work-a" || response.Sessions[0].WorkName == nil || *response.Sessions[0].WorkName != "Build API" {
		t.Fatalf("running attribution = %#v, want work-a/Build API", response.Sessions[0])
	}
	if response.Sessions[1].Failure == nil || response.Sessions[1].Failure.Kind != string(workersessions.FailureCauseTimeout) || response.Sessions[1].DurationMillis == nil || *response.Sessions[1].DurationMillis != duration.Milliseconds() {
		t.Fatalf("failed lifecycle facts = %#v, want timeout and duration", response.Sessions[1])
	}
	if response.Sessions[1].WorkId == nil || *response.Sessions[1].WorkId != "work-b" || response.Sessions[1].WorkName == nil || *response.Sessions[1].WorkName != "Review UI" {
		t.Fatalf("failed attribution = %#v, want work-b/Review UI", response.Sessions[1])
	}
	if response.Sessions[2].WorkId != nil || response.Sessions[2].WorkName != nil || response.Sessions[2].ProviderSession != nil || response.Sessions[2].DurationMillis != nil || response.Sessions[2].Failure != nil {
		t.Fatalf("unavailable optional facts = %#v, want explicit nulls", response.Sessions[2])
	}
}
