...
func testWorkerExecutionHandoffMapsWorkerOutcomes(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "context cancellation", result: workers.ExecuteResult{Outcome: workers.ExecutionOutcomeAccepted}, executeErr: context.Canceled, terminal: workers.WorkstationDispatchTerminalOutcomeCanceled, outcome: workers.OutcomeCanceled, cancellation: workers.DispatchCancellationReasonCanceled, wantError: context.Canceled},
