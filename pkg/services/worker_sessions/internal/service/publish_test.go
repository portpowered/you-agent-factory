...
func continuationOrder(waitBeforeCompletion bool) string {
	if waitBeforeCompletion {
		return "waiter-before-completion"
	}
	return "completion-before-await"
}

func assertProviderFailureCause(t *testing.T, cause *workersessions.FailureCause) {
	t.Helper()
	if cause.Kind != workersessions.FailureCauseWorkersExecutionFailure || cause.Detail != "family=terminal type=permanent_bad_request" || cause.ProviderContinuationFailureKind != providers.ContinuationFailureKindStale {
		t.Fatalf("successor failure cause = %#v, want exact stale continuation classification", cause)
	}
}
