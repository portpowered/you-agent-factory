...
// selected owner. Live close/cancel/terminate preserve the established close
// characterization; pause and resume use the live lifecycle-control owner.
...
func (s *Service) canonicalDurableReads() (durableexecution.CanonicalReadService, error) {
	execution, err := s.durableExecution()
	if err != nil {
		return nil, err
	}
	reads, ok := execution.(durableexecution.CanonicalReadService)
	if !ok || reads == nil {
...
func (s *Service) canonicalDurableResults() (durableexecution.CanonicalResultService, error) {
	execution, err := s.durableExecution()
	if err != nil {
		return nil, err
	}
	results, ok := execution.(durableexecution.CanonicalResultService)
	if !ok || results == nil {
...
func (s *Service) canonicalDurableDispatches() (durableexecution.CanonicalDispatchService, error) {
	execution, err := s.durableExecution()
	if err != nil {
		return nil, err
	}
	dispatches, ok := execution.(durableexecution.CanonicalDispatchService)
...
func (s *Service) canonicalDurableResponses() (durableexecution.CanonicalResponseService, error) {
	execution, err := s.durableExecution()
	if err != nil {
		return nil, err
	}
	responses, ok := execution.(durableexecution.CanonicalResponseService)
	if !ok || responses == nil {
...
func (s *Service) controlCanonicalLive(
	...
	if request.Operation == factorysessions.SessionControlClose ||
		request.Operation == factorysessions.SessionControlCancel ||
		request.Operation == factorysessions.SessionControlTerminate {
...
func (s *Service) canonicalDurableControls() (durableexecution.CanonicalControlService, error) {
	execution, err := s.durableExecution()
	if err != nil {
		return nil, err
	}
	controls, ok := execution.(durableexecution.CanonicalControlService)
	if !ok || controls == nil {
