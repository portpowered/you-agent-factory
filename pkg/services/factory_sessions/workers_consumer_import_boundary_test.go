...
// TestProductionPackagesImportWorkersOnlyThroughRoot seals the CUT-SES-WRK
// consumer edge: Factory Sessions production packages must depend on Workers
// only through the published service root, not nested implementation packages.
func TestProductionPackagesImportWorkersOnlyThroughRoot(t *testing.T) {
	t.Parallel()
