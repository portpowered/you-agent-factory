...
type sessionInvocationTelemetry struct {
	...
	packagedConfig *PackagedInvocationTelemetry
}
...
func NewSessionInvocationTelemetry(
	recordMetric func(SessionInvocationMetric),
	recordLog func(SessionInvocationLogRecord),
	packaged *PackagedInvocationTelemetry,
) SessionInvocationTelemetry {
	return &sessionInvocationTelemetry{
		recordMetric:   recordMetric,
		recordLog:      recordLog,
		packagedConfig: packaged,
	}
}
...
type SessionInvocationPackagedTelemetry interface {
	...
	PackagedInvocationCompleted(string, SessionInvocationWaitInput, work.PrimaryResultSelection)
	PackagedInvocationFailed(string, SessionInvocationWaitInput, SessionInvocationSpecialFailure)
}

func (t *sessionInvocationTelemetry) NormalizationAttempt(cfg *interfaces.FactoryConfig, source work.InputSourceLabel) {
	t.metric(InvocationMetricNormalizationAttempts, invocationMetricLabels(cfg, source))
}

func (t *sessionInvocationTelemetry) NormalizationFailure(cfg *interfaces.FactoryConfig, source work.InputSourceLabel, err error) {
...
func (t *sessionInvocationTelemetry) NormalizationSuccess(cfg *interfaces.FactoryConfig, source work.InputSourceLabel) {
	t.metric(InvocationMetricNormalizationSuccess, invocationMetricLabels(cfg, source))
}

func (t *sessionInvocationTelemetry) InterpolationFailure(cfg *interfaces.FactoryConfig, source work.InputSourceLabel, err error) {
...
func (t *sessionInvocationTelemetry) SubmissionFailure(cfg *interfaces.FactoryConfig, source work.InputSourceLabel, _ error) {
	t.metric(InvocationMetricFailure, invocationMetricLabels(cfg, source))
}

func (t *sessionInvocationTelemetry) InvocationSubmitted(cfg *interfaces.FactoryConfig, source work.InputSourceLabel) {
...
func (t *sessionInvocationTelemetry) InvocationCompleted(cfg *interfaces.FactoryConfig, source work.InputSourceLabel, result []work.WorkContentPart) {
...
func (t *sessionInvocationTelemetry) InvocationFailed(cfg *interfaces.FactoryConfig, source work.InputSourceLabel, errorCode string) {
	t.metric(InvocationMetricFailure, invocationMetricLabels(cfg, source))
	if errorCode == string(work.PrimaryResultErrorCodeUnresolved) {
...
func (t *sessionInvocationTelemetry) LogArgumentFailure(
	...
	normalized *work.NormalizedArguments,
	err error,
	failureClass string,
) {
...
func (t *sessionInvocationTelemetry) LogSubmissionFailure(sessionID string, source work.InputSourceLabel, cfg *interfaces.FactoryConfig, err error) {
...
func (t *sessionInvocationTelemetry) LogInvocationCompleted(sessionID string, input SessionInvocationWaitInput, selection work.PrimaryResultSelection) {
...
func (t *sessionInvocationTelemetry) PackagedInvocationCompleted(sessionID string, input SessionInvocationWaitInput, selection work.PrimaryResultSelection) {
	packaged := t.packaged(input.FactoryConfig)
	if packaged == nil {
		return
	}
...
func (t *sessionInvocationTelemetry) metric(name string, labels map[string]string) {
	if t == nil || t.recordMetric == nil || strings.TrimSpace(name) == "" {
		return
	}
	t.recordMetric(SessionInvocationMetric{Name: name, Labels: cloneMetricLabels(labels)})
}

func (t *sessionInvocationTelemetry) log(level, message string, fields map[string]any, err error) {
	if t == nil || t.recordLog == nil {
		return
	}
	t.recordLog(SessionInvocationLogRecord{Level: level, Message: message, Fields: fields, Error: err})
}

func (t *sessionInvocationTelemetry) packaged(cfg *interfaces.FactoryConfig) *PackagedInvocationTelemetry {
	if t == nil || t.packagedConfig == nil || t.packagedConfig.Active == nil || !t.packagedConfig.Active(cfg) {
		return nil
	}
	return t.packagedConfig
}

func (t *sessionInvocationTelemetry) packagedMetric(name string, source work.InputSourceLabel, extra map[string]string) {
	packaged := t.packagedConfig
...
func addArgumentErrorFields(fields map[string]any, normalized *work.NormalizedArguments, argumentErr *work.ArgumentError) {
...
func invocationLogFields(sessionID string, source work.InputSourceLabel, policy *interfaces.InvocationReturnConfig, cfg *interfaces.FactoryConfig) map[string]any {
	fields := map[string]any{
...
func invocationMetricLabels(cfg *interfaces.FactoryConfig, source work.InputSourceLabel) map[string]string {
...
func invocationFactoryLabels(cfg *interfaces.FactoryConfig) map[string]string {
	...
	if value := work.InvocationSignatureHash(cfg.InvocationSignature); value != "" {
...
func invocationArgumentSourceKinds(normalized *work.NormalizedArguments, parameter string) string {
...
func invocationArgumentRedactionState(normalized *work.NormalizedArguments, parameter string) (bool, int) {
	argument := invocationNormalizedArgument(normalized, parameter)
	if argument == nil {
		return false, 0
	}
...
func invocationNormalizedArgument(normalized *work.NormalizedArguments, parameter string) *work.NormalizedArgument {
