...
func newResponseStream() *responsestream.SessionResponseStream {
	return responsestream.NewSessionResponseStream(platformclock.Real{})
}

func newStreamSet() *responsestream.StreamSet {
	return responsestream.NewStreamSet(platformclock.Real{})
}

func newStreamSetWithFactory(
	newStream func() *responsestream.SessionResponseStream,
) *responsestream.StreamSet {
	return responsestream.NewStreamSetWithFactory(newStream, platformclock.Real{})
}
...
func TestSessionResponseStreamWithClock_RequiresSelectedClock(t *testing.T) {
	t.Parallel()

	if stream := responsestream.NewSessionResponseStreamWithClock(nil, responsestream.RetentionLimits{}); stream != nil {
		t.Fatalf("stream = %#v, want nil when no clock was selected", stream)
	}
}
