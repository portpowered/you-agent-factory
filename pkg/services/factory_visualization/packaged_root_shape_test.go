...
// TestPackagedRootShapeMatchesCanonicalServiceLayout seals FUN-visualization
// story 005: Factory Visualization ships only wire/, internal/, and transports/
...
func TestPackagedRootShapeMatchesCanonicalServiceLayout(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(serviceRoot, "service")); err == nil {
		t.Fatal("pkg/services/factory_visualization/service must not exist")
...
func serviceRootDir(t *testing.T) string {
	...
	if strings.HasSuffix(filepath.ToSlash(root), "/pkg/services/factory_visualization") {
		return root
	}
	t.Fatalf("unexpected test working directory %q; run from pkg/services/factory_visualization", root)
	return ""
}
