...
func TestHumanApprovalCompatibilityWrappersDelegateToApprovalTransport(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.HasSuffix(r.URL.Path, "/approvals") {
			_ = json.NewEncoder(w).Encode(factoryapi.ListHumanApprovalsResponse{Approvals: []factoryapi.HumanApproval{{ApprovalId: "approval-1", SessionId: "session-1", Status: "PENDING"}}})
			return
		}
		_ = json.NewEncoder(w).Encode(factoryapi.HumanApproval{ApprovalId: "approval-1", SessionId: "session-1", Status: "PENDING"})
	}))
	defer server.Close()

	var listJSON bytes.Buffer
	if err := NewListHumanApprovals(testHTTPProtocol(t))(ListHumanApprovalsConfig{Context: context.Background(), Server: server.URL, SessionID: "session-1", JSON: true, Output: &listJSON}); err != nil {
	...
	if err := ListHumanApprovals(ListHumanApprovalsConfig{Context: context.Background(), Server: server.URL, SessionID: "session-1", JSON: true, Output: &directList, HTTP: testHTTPProtocol(t)}); err != nil {
	...
	if err := NewShowHumanApproval(testHTTPProtocol(t))(ShowHumanApprovalConfig{Context: context.Background(), Server: server.URL, SessionID: "session-1", ApprovalID: "approval-1", JSON: true, Output: &showJSON}); err != nil {
	...
	if err := ShowHumanApproval(ShowHumanApprovalConfig{Context: context.Background(), Server: server.URL, SessionID: "session-1", ApprovalID: "approval-1", JSON: true, Output: &directShow, HTTP: testHTTPProtocol(t)}); err != nil {
	...
	if len(requestPaths) != 4 || requestPaths[0] != "/factory-sessions/session-1/approvals" || requestPaths[1] != "/factory-sessions/session-1/approvals" || requestPaths[2] != "/factory-sessions/session-1/approvals/approval-1" || requestPaths[3] != "/factory-sessions/session-1/approvals/approval-1" {
