...
func TestListHumanApprovalsJSONUsesSessionScopedAPIEnvelope(t *testing.T) {
	...
	err := workcli.ListHumanApprovals(workcli.ListHumanApprovalsConfig{Context: context.Background(), Server: server.URL, SessionID: "session/beta", JSON: true, Output: &output, Diagnostics: &diagnostics, Verbose: true, HTTP: testHTTPProtocol(t)})
	if err != nil {
...
func TestShowHumanApprovalHumanOutputAndNextCommandDiagnostics(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.EscapedPath()
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(factoryapi.HumanApproval{ApprovalId: "approval/1", SessionId: "session/beta", DispatchId: "dispatch-1", WorkstationId: "approve", WorkstationName: "Approve release", Status: "PENDING", Decisions: []factoryapi.HumanApprovalDecisions{"APPROVE", "REJECT"}, WorkIds: []string{"work-1", "work-2"}})
	}))
	defer server.Close()

	var output, diagnostics bytes.Buffer
	err := workcli.ShowHumanApproval(workcli.ShowHumanApprovalConfig{Context: context.Background(), Server: server.URL, SessionID: "session/beta", ApprovalID: "approval/1", Output: &output, Diagnostics: &diagnostics, HTTP: testHTTPProtocol(t)})
	if err != nil {
...
func TestListHumanApprovalsEmptyResponseIsDeterministic(t *testing.T) {
	...
	err := workcli.ListHumanApprovals(workcli.ListHumanApprovalsConfig{Context: context.Background(), Server: server.URL, JSON: true, Output: &output, HTTP: testHTTPProtocol(t)})
	if err != nil {
		t.Fatalf("ListHumanApprovals: %v", err)
	}
