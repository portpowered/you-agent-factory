...
func assertPreparedMixedMediaParts(
	t *testing.T,
	parts []work.WorkContentPart,
	image work.StageContentResult,
	document work.StageContentResult,
) {
	t.Helper()
	if len(parts) != 3 {
...
func TestPublicWorkRootStagingSeamPreservesPrepareMixedMedia(t *testing.T) {
	ctx := context.Background()
	staging := newPublicWorkRootStaging(t, nil)
	...
	assertPreparedMixedMediaParts(t, parts, image, document)
}

func TestPublicWorkRootStagingSeamRejectsInvalidStageRequests(t *testing.T) {
	ctx := context.Background()
	tests := []struct {
		name    string
...
func TestPublicWorkRootStagingSeamPrepareRejectsMediaTypeMismatch(t *testing.T) {
	ctx := context.Background()
	staging := newPublicWorkRootStaging(t, nil)
...
// materialization and state-access. It also re-proves issuance, prepare, tamper,
// and expiry cleanup through the published Work root staging seam.
func TestPublicWorkRootStagingSeamSealsContentStagingCutWithoutMaterializationOrRuntime(t *testing.T) {
	...
	clock.now = clock.now.Add(2 * time.Minute)
	if _, err := staging.ResolveContent(ctx, staged.StagedFileRef); !errors.Is(err, work.ErrStagedContentExpired) {
