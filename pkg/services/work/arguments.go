...
// RuntimeInvocationArguments converts normalized arguments into the metadata
// carried on submitted Work.
func RuntimeInvocationArguments(
	...
	for _, parameter := range signature.Parameters {
		...
		if name != "" {
			valueModes[name] = NormalizeInvocationValueMode(parameter.ValueMode)
		}
	}
	args := &InvocationArguments{Arguments: make(map[string]InvocationArgument, len(normalized.Arguments))}
	for name, argument := range normalized.Arguments {
		next := InvocationArgument{
			Values:    append([]string(nil), argument.Values...),
			ValueMode: valueModes[name], Sensitive: argument.Sensitive,
		}
		for _, source := range argument.Sources {
			next.Sources = append(next.Sources, InvocationArgumentSource{
				Kind: string(source.Kind), Name: source.Name, Redact: source.Redact,
			})
		}
...
// InvocationSignatureHash returns a stable digest for one authored signature.
...
func buildSignatureIndex(signature *InvocationSignatureConfig) (signatureIndex, error) {
...
// NormalizeInvocationValueMode applies the canonical default for an authored
// invocation parameter value mode.
func NormalizeInvocationValueMode(valueMode string) string {
	return normalizedValueMode(valueMode)
}
