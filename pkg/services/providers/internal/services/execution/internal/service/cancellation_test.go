...
func TestExecutePreservesDeclaredFailureSessionWhenContextEndsDuringAttempt(t *testing.T) {
	t.Parallel()
	...
	nativeSession := &providers.SessionRef{
		Provider: providers.IDCodex,
		Kind:     providers.SessionIDKind,
		ID:       "declared-canceled-failure-session",
	}
	executionService := mustExecutionService(t, func(
		context.Context,
		providers.ExecuteRequest,
	...
	if !errors.As(executeErr, &failure) ||
		failure.SessionRef == nil ||
		failure.SessionRef.ID != nativeSession.ID {
		t.Fatalf("Execute() error = %#v, want cancellation with declared failure session", executeErr)
	}
	failure.SessionRef.ID = "caller-mutated"
	if nativeSession.ID != "declared-canceled-failure-session" {
		t.Fatalf("adapter session = %#v, want unchanged", nativeSession)
	}
}
