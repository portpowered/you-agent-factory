...
// IntegrationDependencies are the Providers collaborators used by Codex's
// registry-backed integration on the neutral conductor path.
type IntegrationDependencies struct {
	ProvidersService providers.Service
}

// Integration routes Codex through the Providers root execution boundary.
type Integration struct {
	providers providers.Service
}

// NewIntegration constructs Codex's registry-backed integration.
...
func (*Integration) Identity() inference.Identity {
	return inference.Identity(providers.IDCodex)
}
...
// Invoke executes Codex through providers.Service and publishes progress
	...
	if reference := requestedSession(request); reference != nil {
		...
		if continued.Outcome != providers.ContinuationOutcomeResumed {
			return providers.ExecuteResult{}, providers.ExecuteFailure{
				...
				Message: "Codex cannot continue the requested Provider Session",
			}
		}
...
func requestedSession(request inference.InvocationRequest) *providers.SessionRef {
	if session := request.ProviderSession(); session != nil &&
		strings.TrimSpace(session.Provider()) == string(providers.IDCodex) &&
...
func failureFromExecuteError(err error, session *inference.ProviderSession) inference.Failure {
	...
	return inference.NewFailure(inference.FailureInput{
		...
		Message:         "Codex invocation failed.",
		ProviderSession: session,
	})
}
