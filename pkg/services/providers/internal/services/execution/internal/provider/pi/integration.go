...
// IntegrationDependencies are the Providers collaborators used by Pi's
// registry-backed integration on the neutral conductor path.
type IntegrationDependencies struct {
	ProvidersService providers.Service
}

// Integration routes Pi through the Providers root execution boundary.
type Integration struct {
	providers providers.Service
}

// NewIntegration constructs Pi's registry-backed integration.
...
// Invoke executes Pi through providers.Service and publishes progress before
// closing with exactly one terminal outcome.
func (i *Integration) Invoke(
	...
	if i == nil || i.providers == nil {
		failure := inference.NewFailure(inference.FailureInput{
			...
			Message: "Pi Providers service is unavailable",
		})
...
func requestedSession(request inference.InvocationRequest) *providers.SessionRef {
	if session := request.ProviderSession(); session != nil &&
		workers.CanonicalProviderSessionProvider(session.Provider()) == string(modelprovider.ProviderPi) &&
...
func failureFromExecuteError(request inference.InvocationRequest, err error) inference.Failure {
	...
	if errors.As(err, &failure) {
		...
		case inference.FailureInvalidRequest:
			if message == "" {
				message = "Pi rejected the request as invalid."
			}
		}
