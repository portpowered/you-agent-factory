...
func (s *Service) RunScriptPoller(
	...
	commandReq, err := ScriptPollerCommandRequest(runtimeCfg, workstation, workerDef, s.resolveTemplates)
	if err != nil {
		return err
	}
...
func (s *Service) scriptPollerExecutionTimeout(workstation interfaces.FactoryWorkstationConfig, workerDef *interfaces.FactoryWorkerConfig) (time.Duration, error) {
	if s == nil || s.executionPolicy == nil {
