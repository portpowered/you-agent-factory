...
type CheckpointStore interface {
	...
	Save(string, Checkpoint) error
}

type fileCheckpointStore struct{ files CheckpointFileSystem }
...
func NewCheckpointStore(files CheckpointFileSystem) (CheckpointStore, error) {
	if files == nil {
		return nil, errors.New("hosted linear checkpoint filesystem is required")
	}
	return fileCheckpointStore{files: files}, nil
}
...
func RunPollCycle(
	...
	if checkpoints == nil {
		return CycleResult{}, errors.New("hosted linear checkpoint store is required")
	}
	checkpoint, err := checkpoints.Load(checkpointPath)
	if err != nil {
...
func (s fileCheckpointStore) Save(path string, checkpoint Checkpoint) error {
	if strings.TrimSpace(path) == "" || checkpoint.IssueID == "" || checkpoint.UpdatedAt == "" {
		return nil
	}
	if err := s.files.MkdirAll(filepath.Dir(path), 0o755); err != nil {
	...
	tmpPath := path + ".tmp"
	if err := s.files.WriteFile(tmpPath, data, 0o600); err != nil {
...
func resolveSecretRef(
	...
	if getenv == nil {
		return "", fmt.Errorf("secret environment reader is required")
	}
	if readFile == nil {
		return "", fmt.Errorf("secret file reader is required")
	}

	if value := strings.TrimSpace(getenv(SecretEnvName(ref))); value != "" {
		return value, nil
	}
...
func (c Client) fetchIssuesPage(ctx context.Context, apiKey, cursor string, filter linearIssueFilter) (linearIssuePage, error) {
	if c.HTTPClient == nil {
		return linearIssuePage{}, fmt.Errorf("linear HTTP client is required")
	}
