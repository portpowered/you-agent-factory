...
import (
	...
	"github.com/portpowered/infinite-you/pkg/services/workers/providercompat/adapter"
	"github.com/portpowered/infinite-you/pkg/services/workers/providercompat/adapter/testkit"
)
...
func TestFinalOnlyAdapterConformance(t *testing.T) {
	session := providers.SessionMetadata{Provider: "fixture", Kind: "session_id", ID: "session-final-9"}
	testkit.RunFinalOnly(t, testkit.FinalOnlyFixture{
		...
		Success:    workers.CommandResult{Stdout: []byte(`{"content":"Complete response","session":"session-final-9"}`)},
		Failures: []testkit.FinalOnlyFailureCase{
			{
				Name:                    "empty final output is a normalized failure",
				Result:                  workers.CommandResult{Stdout: []byte(`{"content":"","session":"session-final-9"}`)},
				ExpectedProviderSession: &session,
			},
			{
				Name:   "malformed final output is a normalized failure",
				Result: workers.CommandResult{Stdout: []byte(`{"content":"` + finalOnlySecret)},
			},
		},
...
func (finalOnlyAdapter) ParseFinal(_ context.Context, input adapter.FinalParseContext) (adapter.FinalParseResult, error) {
	...
	if strings.TrimSpace(native.Session) != "" {
		session = &providers.SessionMetadata{Provider: "fixture", Kind: "session_id", ID: native.Session}
	}
	...
	return adapter.FinalParseResult{
		Response: workerexecution.InferenceResponse{Content: native.Content, Continuation: testkit.ContinuationForSession(session)},
