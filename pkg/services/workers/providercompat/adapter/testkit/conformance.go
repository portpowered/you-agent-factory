...
type FullStreamFixture struct {
	...
	FinalResult         workers.CommandResult
...
func verifyAuthoritativeFinal(t *testing.T, fixture FullStreamFixture) {
	...
	gotSession := sessionMetadataFromContinuation(result.Response.Continuation)
	if gotSession == nil || *gotSession != fixture.Expected.ProviderSession {
		t.Fatalf("provider session = %#v, want %#v", gotSession, fixture.Expected.ProviderSession)
	}
...
func parseRetryFailure(t *testing.T, providerAdapter adapter.Adapter, observations []adapter.Observation) error {
	...
	_, parseErr := providerAdapter.ParseFinal(context.Background(), adapter.FinalParseContext{
		CommandResult: workers.CommandResult{Stdout: observationsForStream(observations, adapter.OutputStreamStdout), ExitCode: 75},
		FlushReason:   adapter.FlushReasonTerminated,
	})
	if parseErr == nil {
