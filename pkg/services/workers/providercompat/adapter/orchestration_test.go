...
func TestExecuteFlushesBeforeReturningEveryCommandOutcome(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "process failure", prepare: func() (context.Context, *kernelRunner) {
			return context.Background(), &kernelRunner{result: workers.CommandResult{ExitCode: 7}}
...
func (r *kernelRunner) Run(_ context.Context, request workers.CommandRequest, observe func(adapter.Observation) error) (workers.CommandResult, error) {
	r.calls++
	r.request = request
