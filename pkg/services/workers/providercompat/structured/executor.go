...
func (r commandRunner) Run(
	...
	case workerprovider.InferenceProgressPublishingCommandRunner, *workerprovider.InferenceProgressPublishingCommandRunner,
		workers.ExecCommandRunner, *workers.ExecCommandRunner,
		workers.StreamingAdaptedCommandRunner, *workers.StreamingAdaptedCommandRunner:
...
func runObservedCommand(
	...
	runner := workers.StreamingExecCommandRunner{Runner: baseRunner, Logger: logger, Observer: func(stream string, chunk []byte) {
		mu.Lock()
		defer mu.Unlock()
		if observeErr == nil {
