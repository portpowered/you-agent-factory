	...
	for _, tc := range testCases {
		...
		t.Run(entryLabel, func(t *testing.T) {
			...
			if providerErr.Continuation != nil {
				t.Fatalf("%s expected provider continuation to be nil, got %#v", entryLabel, providerErr.Continuation)
			}
...
func TestScriptWrapProvider_Infer_CodexExitFailurePreservesSessionMetadata(t *testing.T) {
	...
	if providerErr.Continuation == nil {
		t.Fatal("expected provider continuation on failure")
	}
	if providerErr.Continuation.ProviderSessionID != "sess_codex_error_123" {
		t.Fatalf("provider session id = %q, want %q", providerErr.Continuation.ProviderSessionID, "sess_codex_error_123")
	}
}
