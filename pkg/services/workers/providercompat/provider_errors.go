...
type ProviderError = workers.ProviderError
...
func NewProviderError(errorType workerexecution.WorkFailureType, message string, cause error) *ProviderError {
	return workers.NewProviderError(errorType, message, cause)
}
...
func newProviderErrorFromResultWithDiagnostics(result ProviderFailureResult, cause error, session *providers.SessionMetadata, diagnostics *workerexecution.WorkDiagnostics) *ProviderError {
	err := NewProviderErrorFromResult(result, cause)
	err.Continuation = providers.ContinuationFromSessionMetadata(session)
	err.Diagnostics = workerexecution.CloneWorkDiagnostics(diagnostics)
	return err
}

func NewProviderErrorWithSession(errorType workerexecution.WorkFailureType, message string, cause error, session *providers.SessionMetadata) *ProviderError {
	err := NewProviderError(errorType, message, cause)
	err.Continuation = providers.ContinuationFromSessionMetadata(session)
	return err
}

func newProviderErrorWithDiagnostics(errorType workerexecution.WorkFailureType, message string, cause error, session *providers.SessionMetadata, diagnostics *workerexecution.WorkDiagnostics) *ProviderError {
...
func NormalizeProviderExecutionError(err error) *ProviderError {
	return workers.NormalizeProviderExecutionError(err)
}
...
func validateProviderErrorCorpusEntry(entry ProviderErrorCorpusEntry) error {
	...
	if entry.Provider == modelprovider.ProviderClaude && entry.ExpectedMessage == "" {
