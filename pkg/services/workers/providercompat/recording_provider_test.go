...
func TestRecordingProviderRequiresInjectedClock(t *testing.T) {
	t.Parallel()
	provider := NewRecordingProvider(&recordingProviderFake{}, func(workerexecution.InferenceEvent) {}, nil)
	_, err := provider.Infer(context.Background(), recordingProviderDispatch())
	var providerErr *ProviderError
	if !errors.As(err, &providerErr) || !strings.Contains(providerErr.Message, "clock is required") {
		t.Fatalf("Infer() error = %#v, want missing-clock failure", err)
	}
}
...
func TestRecordingProvider_Infer_SuccessEmitsRequestAndResponseEventsInOrder(t *testing.T) {
	...
	provider := NewRecordingProvider(fake, events.record, sequenceClock(
...
func TestRecordingProvider_Infer_SuccessPreservesProviderSessionAndSafeDiagnostics(t *testing.T) {
	t.Parallel()
	respSession := &providers.SessionMetadata{Provider: "claude", Kind: "session_id", ID: "sess-123"}
	fake := &recordingProviderFake{
		responses: []workerexecution.InferenceResponse{{
			Content:      "provider response",
			Continuation: providers.ContinuationFromSessionMetadata(respSession),
			Diagnostics: &workerexecution.WorkDiagnostics{
...
func TestRecordingProvider_Infer_CursorSessionMetadataIsCanonicalizedInEvents(t *testing.T) {
	t.Parallel()
	respSession := &providers.SessionMetadata{
		Provider: "cursor",
		Kind:     "session_id",
...
func TestRecordingProvider_Infer_FailureZeroExitCodeStillPreservesProviderSessionAndSafeDiagnostics(t *testing.T) {
	...
	providerErr := NewProviderErrorWithSession(
		...
		"provider timed out",
		nil,
		&providers.SessionMetadata{Provider: "claude", Kind: "session_id", ID: "sess-456"},
	)
	providerErr.Diagnostics = &workerexecution.WorkDiagnostics{
		Provider: &workerexecution.ProviderDiagnostic{
			Provider: "claude",
...
func TestRecordingProvider_Infer_MissingInnerProviderEmitsMisconfiguredFailureEvent(t *testing.T) {
	t.Parallel()
	events := &recordingEvents{}
	provider := NewRecordingProvider(nil, events.record, sequenceClock(
		time.Date(2026, 4, 18, 12, 0, 0, 0, time.UTC),
		11*time.Millisecond,
	))
...
func assertInferenceResponseEvent(t *testing.T, event workerexecution.InferenceEvent) factoryapi.InferenceResponseEventPayload {
	...
	public, err := apisurface.FactoryEventToAPI(canonical)
	if err != nil {
		t.Fatalf("response payload map: %v", err)
	}
	payload, err := public.Payload.AsInferenceResponseEventPayload()
	if err != nil {
...
func TestInferenceProgressPublishingCommandRunner_NormalizesCodexStructuredEvents(t *testing.T) {
	...
	if finalFragment.Continuation == nil || finalFragment.Continuation.ProviderSessionID != "sess-codex-1" {
		t.Fatalf("final provider session = %#v, want session propagated", finalFragment.Continuation)
	}
}
...
func TestInferenceProgressPublishingCommandRunner_MapsUnknownAndMalformedCodexEventsToBoundedDiagnostics(t *testing.T) {
	...
	if published[4].Continuation == nil || published[4].Continuation.ProviderSessionID != "sess-codex-2" {
		t.Fatalf("final provider session = %#v, want session carried across malformed frames", published[4].Continuation)
	}
}
...
func TestInferenceProgressPublishingCommandRunner_MapsFailureCancelAndTruncation(t *testing.T) {
	...
	if published[5].Continuation == nil || published[5].Continuation.ProviderSessionID != "sess-codex-3" {
		t.Fatalf("cancel provider session = %#v, want session propagated", published[5].Continuation)
	}
}
...
func assertCodexStartedFragment(t *testing.T, fragment *InferenceProgressFragment, sessionID string, workID string) {
	...
	if fragment.Continuation == nil || fragment.Continuation.ProviderSessionID != sessionID {
		t.Fatalf("start provider session = %#v, want %s", fragment.Continuation, sessionID)
	}
