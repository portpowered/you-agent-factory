...
func assertGPT56SolFailureMetadata(t *testing.T, providerErr *ProviderError, entry ProviderErrorCorpusEntry, result CommandResult) {
	...
	if providerErr.Continuation == nil || providerErr.Continuation.ProviderSessionID != "sess-codex-gpt-5-6-sol" {
		t.Fatalf("provider continuation = %#v, want captured Codex session", providerErr.Continuation)
	}
