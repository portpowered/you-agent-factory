...
type RecordingProvider struct {
	inner    providercontract.LegacyProvider
...
func inferenceResponseEvent(req workerexecution.ProviderInferenceRequest, resp workerexecution.InferenceResponse, err error, attempt int, inferenceRequestID string, duration time.Duration, eventTime time.Time) workerexecution.InferenceEvent {
	...
	if err != nil {
		...
		payload.Continuation = cloneContinuation(providerContinuationFromInferenceError(err))
		...
		payload.Continuation = cloneContinuation(resp.Continuation)
		payload.Diagnostics = safeInferenceDiagnosticsEventPayload(
...
func providerContinuationFromInferenceError(err error) *providers.ContinuationRef {
	var providerErr *ProviderError
	if !errors.As(err, &providerErr) {
		return nil
	}
...
var _ providercontract.LegacyProvider = (*RecordingProvider)(nil)
