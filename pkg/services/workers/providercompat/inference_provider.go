...
import (
	...
	provideradapter "github.com/portpowered/infinite-you/pkg/services/workers/providercompat/adapter"
	"github.com/portpowered/infinite-you/pkg/services/workers/providercompat/commandenv"
	providercontract "github.com/portpowered/infinite-you/pkg/services/workers/providercompat/inferencecontract"
...
type ResponseStreamExecutionResult struct {
	...
	FailureSession            *providers.SessionMetadata
...
func (p *ScriptWrapProvider) Execute(ctx context.Context, req workerexecution.RunnerExecutionRequest) (workerexecution.RunnerExecutionResult, error) {
	...
	if err != nil {
		...
		p.publishFailureFragment(req.Dispatch.DispatchID, providers.SessionMetadataFromContinuation(providerErr.Continuation), providerErr)
...
func (p *ScriptWrapProvider) executeStructuredResponseStream(
	...
	responseSession := providers.SessionMetadataFromContinuation(result.Response.Continuation)
	logger.Info("inferencer: request completed",
		appendProviderSessionLogFields(providerLogFields(req, "output_len", len(result.Response.Content)), responseSession)...)
	p.publishCompletedFragment(req.Dispatch.DispatchID, responseSession)
...
func normalizeProviderExecutionError(provider string, result CommandResult, err error, session *providers.SessionMetadata, diagnostics *workerexecution.WorkDiagnostics) *ProviderError {
	switch {
...
func normalizeProviderExitFailure(provider string, result CommandResult, session *providers.SessionMetadata, diagnostics *workerexecution.WorkDiagnostics) *ProviderError {
...
func providerErrorFromAdapterFailure(
	...
	return &ProviderError{
		...
		Continuation: providers.ContinuationFromSessionMetadata(failure.ProviderSession),
...
func (p *ScriptWrapProvider) publishCompletedFragment(dispatchID string, providerSession *providers.SessionMetadata) {
...
func (p *ScriptWrapProvider) publishFailureFragment(dispatchID string, providerSession *providers.SessionMetadata, err error) {
...
func providerSessionFromCommandResult(provider string, result CommandResult) *providers.SessionMetadata {
...
func effectiveProviderSession(req workerexecution.ProviderInferenceRequest, result CommandResult) *providers.SessionMetadata {
	...
	if (req.ModelProvider == string(modelprovider.ProviderClaude) || req.ModelProvider == string(modelprovider.ProviderAntigravity)) && req.SessionID != "" {
		return &providers.SessionMetadata{
			Provider: providers.CanonicalProviderSessionProvider(req.ModelProvider),
			Kind:     providerSessionKindSessionID,
			ID:       req.SessionID,
		}
	}
...
var _ providercontract.LegacyProvider = (*ScriptWrapProvider)(nil)
