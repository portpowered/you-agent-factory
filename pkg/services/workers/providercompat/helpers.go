...
import (
	...
	"github.com/portpowered/infinite-you/pkg/services/workers/providercompat/agypty"
...
func (f *Factory) New(
	...
	provider := NewScriptWrapProviderWithDependencies(
		skipPermissions,
		logger,
		workers.CommandRunnerWithLogging(f.commandRunner, logger, f.commandClock),
		nil,
		progressPublisher,
...
func providerFailureLogFields(
	...
	fields = appendProviderSessionLogFields(fields, providers.SessionMetadataFromContinuation(providerErr.Continuation))
...
func appendProviderSessionLogFields(fields []any, session *providers.SessionMetadata) []any {
	if session == nil {
		return fields
	}
	return append(fields,
		"provider_session_provider", providers.CanonicalProviderSessionProvider(session.Provider),
