...
func TestScriptWrapProvider_Infer_ClaudePayloadUsesExpectedCommandArgsAndEnv(t *testing.T) {
	...
	if resp.Continuation.Provider != string(modelprovider.ProviderClaude) {
		t.Fatalf("provider session provider = %q, want %q", resp.Continuation.Provider, modelprovider.ProviderClaude)
	}
	if resp.Continuation.Kind != providerSessionKindSessionID {
		t.Fatalf("provider session kind = %q, want %q", resp.Continuation.Kind, providerSessionKindSessionID)
	}
	if resp.Continuation.ProviderSessionID != "claude-session-123" {
		t.Fatalf("provider session id = %q, want %q", resp.Continuation.ProviderSessionID, "claude-session-123")
	}
...
func TestScriptWrapProvider_Infer_ClaudeWithoutSessionLeavesMetadataNil(t *testing.T) {
	...
	if resp.Continuation != nil {
		t.Fatalf("expected provider session to be nil, got %#v", resp.Continuation)
	}
}
...
func TestScriptWrapProvider_Infer_ClaudeExitFailurePreservesConfiguredSessionMetadata(t *testing.T) {
	...
	if providerErr.Continuation.Provider != string(modelprovider.ProviderClaude) {
		t.Fatalf("provider session provider = %q, want %q", providerErr.Continuation.Provider, modelprovider.ProviderClaude)
	}
	if providerErr.Continuation.ProviderSessionID != "claude-session-123" {
		t.Fatalf("provider session id = %q, want %q", providerErr.Continuation.ProviderSessionID, "claude-session-123")
	}
}
...
func TestScriptWrapProvider_Infer_CodexPayloadUsesExpectedCommandArgsStdinAndEnv(t *testing.T) {
	...
	if resp.Continuation.Provider != string(modelprovider.ProviderCodex) {
		t.Fatalf("provider session provider = %q, want %q", resp.Continuation.Provider, modelprovider.ProviderCodex)
	}
	if resp.Continuation.Kind != providerSessionKindSessionID {
		t.Fatalf("provider session kind = %q, want %q", resp.Continuation.Kind, providerSessionKindSessionID)
	}
	if resp.Continuation.ProviderSessionID != "sess_codex_123" {
		t.Fatalf("provider session id = %q, want %q", resp.Continuation.ProviderSessionID, "sess_codex_123")
	}
