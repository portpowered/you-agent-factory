...
func TestStructuredDecoderBoundsUnsafeDiagnosticsAndRecovers(t *testing.T) {
	...
	if findDraft(decoded.Drafts, factorysessions.ResponseEventKindMessage, factorysessions.ResponseEventPhaseCompleted) == nil {
...
func TestStructuredDecoderFlushesUnterminatedSnapshotAndTerminal(t *testing.T) {
	...
	message := findDraft(flushed.Drafts, factorysessions.ResponseEventKindMessage, factorysessions.ResponseEventPhaseCompleted)
	terminal := findDraft(flushed.Drafts, factorysessions.ResponseEventKindRun, factorysessions.ResponseEventPhaseCompleted)
...
func TestOpenCodeAuthoritativeFinalResultsPreserveContentBeyondPublicationLimit(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			message := findDraft(result.Drafts, factorysessions.ResponseEventKindMessage, factorysessions.ResponseEventPhaseCompleted)
			if tc.wantDraft {
...
func assertPublishedMessageIsBounded(t *testing.T, draft *factorysessions.ResponseEventDraft, authoritative string) {
	t.Helper()
	if draft == nil {
...
func TestStructuredExplicitFailureWinsOverZeroExitAndNeverLeaksNativeBody(t *testing.T) {
	...
	errorDraft := findDraft(decoded.Drafts, factorysessions.ResponseEventKindError, factorysessions.ResponseEventPhaseFailed)
	if errorDraft == nil {
...
func assertStructuredDrafts(t *testing.T, drafts []factorysessions.ResponseEventDraft) {
	t.Helper()
	want := []struct {
		...
		{factorysessions.ResponseEventKindUsage, factorysessions.ResponseEventPhaseUpdated},
		{factorysessions.ResponseEventKindRun, factorysessions.ResponseEventPhaseCompleted},
	}
...
func assertStructuredDraft(t *testing.T, index int, draft factorysessions.ResponseEventDraft, kind factorysessions.ResponseEventKind, phase factorysessions.ResponseEventPhase, terminal bool) {
	t.Helper()
	...
	if draft.Provenance.Fidelity != factorysessions.ResponseEventFidelityNormalized {
		t.Fatalf("draft[%d] provenance = %#v", index, draft.Provenance)
	}
}

func assertTerminalProvenance(t *testing.T, provenance factorysessions.ResponseEventProvenance) {
	t.Helper()
	if provenance.Delivery != factorysessions.ResponseEventDeliverySynthesized || provenance.Fidelity != factorysessions.ResponseEventFidelityLifecycleOnly {
