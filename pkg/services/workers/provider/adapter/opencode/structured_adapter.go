...
func (*NegotiatedAdapter) Identity() adapter.Identity {
	return adapter.Identity(modelprovider.ProviderOpenCode)
}
...
func (a *NegotiatedAdapter) BuildCommand(_ context.Context, input adapter.CommandContext) (adapter.CommandBuildResult, error) {
	...
	command.Args = args
	command.Env = structuredCommandEnv(request.ProcessEnvironment, request.EnvVars)
...
func structuredCommandEnv(processEnvironment []string, overrides map[string]string) []string {
	...
	return workerprocess.MergeCommandEnv(processEnvironment, workerprocess.CommandEnvEntriesFromMap(overrides), automation)
}
