...
func TestAdapterFinalOnlyConformance(t *testing.T) {
	...
	providerAdapter := agy.NewAdapterWithDependencies(
		factoryRoot, mock, "agy", agypty.DefaultSessionConfig(), executableDependencies(nil),
	)
...
func TestAdapterBuildCommandUsesTypedArgvWorkspaceAndEnvironment(t *testing.T) {
	...
	providerAdapter := agy.NewAdapterWithExecutable(
		factoryRoot, executable, executableDependencies(nil, executable),
	)
...
func TestAdapterBuildCommandResolvesBareExecutableThroughInjectedSearchPath(t *testing.T) {
	t.Parallel()
	...
	providerAdapter := agy.NewAdapterWithDependencies(
		...
		"agy",
		agypty.SessionConfig{},
		executableDependencies(map[string]string{"agy": resolved}, resolved),
	)
	built, err := providerAdapter.BuildCommand(context.Background(), adapter.CommandContext{
		Request: workerexecution.ProviderInferenceRequest{WorkingDirectory: ".", UserMessage: "hello"},
	})
	if err != nil {
		t.Fatalf("BuildCommand() error = %v", err)
	}
	if built.Request.Command != resolved {
		t.Fatalf("command = %q, want injected search result %q", built.Request.Command, resolved)
	}
}

func TestAdapterBuildCommandFailsClosedWithoutExecutableEffects(t *testing.T) {
	t.Parallel()

	request := adapter.CommandContext{Request: workerexecution.ProviderInferenceRequest{
		WorkingDirectory: ".", UserMessage: "hello",
	}}
	if _, err := agy.NewAdapter(t.TempDir()).BuildCommand(context.Background(), request); err == nil || !strings.Contains(err.Error(), "executable locator is required") {
		t.Fatalf("missing locator error = %v", err)
	}
	locatorOnly := agy.ExecutableDependencies{Locator: fakeExecutableLocator{}}
	if _, err := agy.NewAdapterWithDependencies(t.TempDir(), nil, "agy", agypty.SessionConfig{}, locatorOnly).BuildCommand(context.Background(), request); err == nil || !strings.Contains(err.Error(), "executable path inspector is required") {
...
func assertAdapterMetacharacterExecuteResult(
	...
	if message.Kind != factorysessions.ResponseEventKindMessage || message.Phase != factorysessions.ResponseEventPhaseCompleted {
