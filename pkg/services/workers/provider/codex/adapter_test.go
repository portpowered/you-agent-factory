...
func TestDecoderMapsThreadTurnAndCompletedAgentMessageExactly(t *testing.T) {
	...
	wantKinds := []factorysessions.ResponseEventKind{factorysessions.ResponseEventKindSession, factorysessions.ResponseEventKindTurn, factorysessions.ResponseEventKindMessage, factorysessions.ResponseEventKindUsage, factorysessions.ResponseEventKindTurn}
	assertDraftKindsAndSession(t, drafts, wantKinds, "thread-codex-123")
	message := drafts[2]
	if message.ItemID != "item-message-1" || message.Phase != factorysessions.ResponseEventPhaseCompleted || message.TurnID != drafts[1].TurnID {
	...
	var usage factorysessions.ResponseEventUsage
...
func assertDraftKindsAndSession(t *testing.T, drafts []factorysessions.ResponseEventDraft, wantKinds []factorysessions.ResponseEventKind, wantSession string) {
	t.Helper()
...
func TestDecoderPreservesEverySupportedItemSemanticAndIdentity(t *testing.T) {
	...
	assertItemLifecycle(t, byID["reason-1"], factorysessions.ResponseEventKindReasoning, factorysessions.ResponseEventPhaseStarted, factorysessions.ResponseEventPhaseCompleted)
	assertItemLifecycle(t, byID["command-1"], factorysessions.ResponseEventKindTool, factorysessions.ResponseEventPhaseStarted, factorysessions.ResponseEventPhaseCompleted)
	assertItemLifecycle(t, byID["mcp-1"], factorysessions.ResponseEventKindTool, factorysessions.ResponseEventPhaseStarted, factorysessions.ResponseEventPhaseCompleted)
	assertItemLifecycle(t, byID["collab-1"], factorysessions.ResponseEventKindTool, factorysessions.ResponseEventPhaseStarted, factorysessions.ResponseEventPhaseCompleted)
	assertItemLifecycle(t, byID["web-1"], factorysessions.ResponseEventKindTool, factorysessions.ResponseEventPhaseStarted, factorysessions.ResponseEventPhaseCompleted)
	assertItemLifecycle(t, byID["error-1"], factorysessions.ResponseEventKindError, factorysessions.ResponseEventPhaseUpdated, factorysessions.ResponseEventPhaseUpdated, factorysessions.ResponseEventPhaseUpdated)
	assertItemLifecycle(t, byID["plan-1"], factorysessions.ResponseEventKindPlan, factorysessions.ResponseEventPhaseUpdated, factorysessions.ResponseEventPhaseUpdated, factorysessions.ResponseEventPhaseUpdated)
	assertItemLifecycle(t, byID["message-1"], factorysessions.ResponseEventKindMessage, factorysessions.ResponseEventPhaseCompleted)
	assertItemLifecycle(t, byID["files-1"], factorysessions.ResponseEventKindFileChange, factorysessions.ResponseEventPhaseUpdated)
...
func TestDecoderMapsErrorItemAsBoundedNonTerminalObservation(t *testing.T) {
	...
	if draft.Kind != factorysessions.ResponseEventKindError || draft.Phase != factorysessions.ResponseEventPhaseUpdated || draft.ItemID != "error-bounded" {
...
func TestDecoderMapsFullUsageAndSelectsOneTypedTerminalFactAtFlush(t *testing.T) {
	...
	if draft.Kind != factorysessions.ResponseEventKindError || draft.Phase != factorysessions.ResponseEventPhaseFailed || draft.ProviderSessionRef != "thread-terminal-1" {
	...
	runResponseAdapterConformance(t, responseAdapterFixture{
		...
		Expected: responseAdapterExpected{
			...
			Drafts: []draftExpectation{
				{Kind: factorysessions.ResponseEventKindSession, Phase: factorysessions.ResponseEventPhaseStarted, ProviderRef: threadID},
				{Kind: factorysessions.ResponseEventKindTurn, Phase: factorysessions.ResponseEventPhaseStarted, ProviderRef: threadID},
				{Kind: factorysessions.ResponseEventKindTool, Phase: factorysessions.ResponseEventPhaseStarted, ItemID: "command-1", ProviderRef: threadID},
				{Kind: factorysessions.ResponseEventKindTool, Phase: factorysessions.ResponseEventPhaseDelta, ItemID: "command-1", ProviderRef: threadID},
				{Kind: factorysessions.ResponseEventKindTool, Phase: factorysessions.ResponseEventPhaseCompleted, ItemID: "command-1", ProviderRef: threadID},
				{Kind: factorysessions.ResponseEventKindMessage, Phase: factorysessions.ResponseEventPhaseCompleted, ItemID: "message-1", ProviderRef: threadID},
				{Kind: factorysessions.ResponseEventKindUsage, Phase: factorysessions.ResponseEventPhaseUpdated, ProviderRef: threadID},
				{Kind: factorysessions.ResponseEventKindTurn, Phase: factorysessions.ResponseEventPhaseCompleted, ProviderRef: threadID},
			},
...
func TestResponseAdapterBuildCommandPreservesOrderedImagesAndExecutionControls(t *testing.T) {
	...
	request := workerexecution.ProviderInferenceRequest{
		...
		InputTokens: []any{factoryruntime.RuntimeToken{ID: "token-images", Color: factoryruntime.RuntimeTokenColor{Content: []work.WorkContentPart{
			{Type: work.WorkContentPartTypeImage, File: first},
			{Type: work.WorkContentPartTypeImage, File: second},
		}}}},
	}
	built, err := codex.NewResponseAdapter().BuildCommand(context.Background(), adapter.CommandContext{
		Request:         request,
		...
		ContentMaterializer: work.ContentMaterializeFunc(func(
			...
			firstURL, firstErr := work.FilesystemPathToContentURL(first)
			if firstErr != nil {
...
func decodeAdapter(t *testing.T, providerAdapter adapter.Adapter, observations []adapter.Observation, reason adapter.FlushReason) ([]factorysessions.ResponseEventDraft, []adapter.Diagnostic) {
	t.Helper()
...
func completedMessage(drafts []factorysessions.ResponseEventDraft) *factorysessions.ResponseEventDraft {
	for index := range drafts {
		if drafts[index].Kind == factorysessions.ResponseEventKindMessage && drafts[index].Phase == factorysessions.ResponseEventPhaseCompleted {
			return &drafts[index]
		}
	}
	return nil
...
func messagePayloadText(t *testing.T, draft factorysessions.ResponseEventDraft) string {
	t.Helper()
	var payload factorysessions.ResponseEventMessage
	decodePayload(t, draft, &payload)
...
func decodeFixture(t *testing.T, fixture []byte) []factorysessions.ResponseEventDraft {
	t.Helper()
...
func draftsByItemID(drafts []factorysessions.ResponseEventDraft) map[string][]factorysessions.ResponseEventDraft {
	result := make(map[string][]factorysessions.ResponseEventDraft)
...
func itemIDOrder(drafts []factorysessions.ResponseEventDraft) []string {
	var result []string
	for _, draft := range drafts {
		if draft.ItemID != "" {
...
func assertItemLifecycle(t *testing.T, drafts []factorysessions.ResponseEventDraft, kind factorysessions.ResponseEventKind, phases ...factorysessions.ResponseEventPhase) {
	t.Helper()
	got := make([]factorysessions.ResponseEventPhase, len(drafts))
	...
	for index, draft := range drafts {
		...
		if draft.Provenance.NativeEventType == "item.completed" {
			completedSnapshots++
			if draft.Provenance.Representation != factorysessions.ResponseEventRepresentationSnapshot {
...
func assertItemPayloads(t *testing.T, byID map[string][]factorysessions.ResponseEventDraft) {
	t.Helper()
...
func assertErrorItemPayload(t *testing.T, drafts []factorysessions.ResponseEventDraft) {
	t.Helper()
...
func assertReasoningCommandAndFilePayloads(t *testing.T, byID map[string][]factorysessions.ResponseEventDraft) {
	t.Helper()
	var reasoning factorysessions.ResponseEventReasoning
	decodePayload(t, byID["reason-1"][1], &reasoning)
	if reasoning.Summary != "The provider adapter owns native decoding." {
		t.Fatalf("reasoning = %#v", reasoning)
	}

	var command factorysessions.ResponseEventTool
	...
	var files factorysessions.ResponseEventFileChange
...
func assertToolAndPlanPayloads(t *testing.T, byID map[string][]factorysessions.ResponseEventDraft) {
	t.Helper()
	var mcp factorysessions.ResponseEventTool
	...
	var collaboration factorysessions.ResponseEventTool
	...
	var plan factorysessions.ResponseEventPlan
...
func decodePayload(t *testing.T, draft factorysessions.ResponseEventDraft, target any) {
	t.Helper()
