...
func (p *ScriptWrapProvider) commandExec() CommandRunner {
	...
	return workerprocess.CommandRunnerWithLogging(nil, p.Logger, nil)
}
...
func NewScriptWrapProviderWithDependencies(
	...
	provider := &ScriptWrapProvider{
		...
		responseStreamExecutor: responseStreamExecutor,
		openCodeResolver:       openCodeResolver,
	...
	if provider.openCodeResolver == nil {
		provider.openCodeResolverErr = errors.New("opencode capability resolver is required")
	}
	return provider
}
...
func (p *ScriptWrapProvider) Execute(ctx context.Context, req workerexecution.RunnerExecutionRequest) (workerexecution.RunnerExecutionResult, error) {
	...
	if strings.EqualFold(strings.TrimSpace(req.ModelProvider), string(modelprovider.ProviderOpenCode)) {
	...
	cursorProvider := req.ModelProvider == string(modelprovider.ProviderCursor)
	if err != nil {
...
func (p *ScriptWrapProvider) executeNegotiatedOpenCode(
	...
	decision, err := p.openCodeResolver.Resolve(ctx, string(modelprovider.ProviderOpenCode))
	if err != nil {
...
func (p *ScriptWrapProvider) openCodeAdapterRunner() provideradapter.StreamingCommandRunner {
	...
	case workerprocess.ExecCommandRunner, *workerprocess.ExecCommandRunner:
		return execAdapterRunner{runner: runner, logger: logging.EnsureLogger(p.Logger)}
	case InferenceProgressPublishingCommandRunner:
		return execAdapterRunner{runner: runner.Runner, logger: logging.EnsureLogger(runner.Logger)}
	case *InferenceProgressPublishingCommandRunner:
		if runner != nil {
			return execAdapterRunner{runner: runner.Runner, logger: logging.EnsureLogger(runner.Logger)}
		}
	...
	return func(decoded provideradapter.DecodeResult) {
		...
		for _, diagnostic := range decoded.Diagnostics {
			p.progressPublisher(InferenceProgressFragment{
				...
				Metadata: map[string]string{"runner_id": string(modelprovider.ProviderOpenCode), "diagnostic_code": diagnostic.Code},
			})
		}
	}
}
...
func (p *ScriptWrapProvider) publishOpenCodeCapability(
	...
	metadata := map[string]string{
		"runner_id": string(modelprovider.ProviderOpenCode), "selected_mode": mode, "fidelity": fidelity,
	}
	if diagnostic != nil {
...
func normalizeProviderExecutionError(provider string, result CommandResult, err error, session *workerexecution.ProviderSessionMetadata, diagnostics *workerexecution.WorkDiagnostics) *ProviderError {
	...
	if normalizedProvider == string(modelprovider.ProviderCursor) {
...
func effectiveProviderSession(req workerexecution.ProviderInferenceRequest, result CommandResult) *workerexecution.ProviderSessionMetadata {
	...
	if (req.ModelProvider == string(modelprovider.ProviderClaude) || req.ModelProvider == string(modelprovider.ProviderCursor) || req.ModelProvider == string(modelprovider.ProviderOpenCode) || req.ModelProvider == string(modelprovider.ProviderPi)) && req.SessionID != "" {
		return &workerexecution.ProviderSessionMetadata{
