...
func TestResponseEventDecoder_CorrelatesSafeToolLifecycleFixture(t *testing.T) {
	...
	assertCursorToolDraft(t, result.Drafts[1], factorysessions.ResponseEventPhaseStarted, "call-read-1", "readToolCall", "started")
	assertCursorToolDraft(t, result.Drafts[2], factorysessions.ResponseEventPhaseCompleted, "call-read-1", "readToolCall", "completed")
	assertCursorToolDraft(t, result.Drafts[3], factorysessions.ResponseEventPhaseStarted, "call-shell-2", "shell", "started")
	assertCursorToolDraft(t, result.Drafts[4], factorysessions.ResponseEventPhaseFailed, "call-shell-2", "shell", "failed")

	var readStarted, readCompleted, shellStarted, shellCompleted factorysessions.ResponseEventTool
...
func TestResponseEventDecoder_RedactsCredentialVariantsInArgumentsAndResults(t *testing.T) {
	...
	var started, completed factorysessions.ResponseEventTool
...
func TestResponseEventDecoder_MapsExplicitToolFailureAndCancellationStatuses(t *testing.T) {
	...
	assertCursorToolDraft(t, result.Drafts[1], factorysessions.ResponseEventPhaseFailed, "call-fail", "writeToolCall", "failed")
	assertCursorToolDraft(t, result.Drafts[3], factorysessions.ResponseEventPhaseCanceled, "call-cancel", "searchToolCall", "canceled")
}
...
func TestResponseEventDecoder_ReconnectRetainsCorrelationForRecoveredCompletion(t *testing.T) {
	...
	if gap := cursorDraftOfKind(result.Drafts, factorysessions.ResponseEventKindStreamGap); gap != nil {
		t.Fatalf("recovered completion also emitted a gap: %#v", *gap)
	}
	tools := cursorDraftsOfKind(result.Drafts, factorysessions.ResponseEventKindTool)
	if len(tools) != 2 {
		t.Fatalf("tool drafts = %#v, want one recovered lifecycle", tools)
	}
	assertCursorToolDraft(t, tools[0], factorysessions.ResponseEventPhaseStarted, "call-recovered", "readToolCall", "started")
	assertCursorToolDraft(t, tools[1], factorysessions.ResponseEventPhaseCompleted, "call-recovered", "readToolCall", "completed")
}
...
func TestResponseEventDecoder_ReconnectReportsEveryStillUnresolvedToolAsGap(t *testing.T) {
	...
	gaps := cursorDraftsOfKind(result.Drafts, factorysessions.ResponseEventKindStreamGap)
	if len(gaps) != 2 {
	...
	if last := result.Drafts[len(result.Drafts)-1]; last.Kind != factorysessions.ResponseEventKindMessage || last.Phase != factorysessions.ResponseEventPhaseCompleted {
...
func TestResponseEventDecoder_TerminalFailureReportsUnresolvedToolGap(t *testing.T) {
	...
	if result.Drafts[2].Kind != factorysessions.ResponseEventKindError || result.Drafts[2].Phase != factorysessions.ResponseEventPhaseFailed {
...
func TestResponseEventDecoder_ExplicitCancellationClosesUnresolvedToolsAsCanceled(t *testing.T) {
	...
	if result.Drafts[2].Kind != factorysessions.ResponseEventKindRun || result.Drafts[2].Phase != factorysessions.ResponseEventPhaseCanceled {
...
func TestResponseEventDecoder_FlushClosesEveryUnresolvedToolFromBoundaryEvidence(t *testing.T) {
	...
	testCases := []struct {
		...
		wantKind  factorysessions.ResponseEventKind
		wantPhase factorysessions.ResponseEventPhase
		wantGap   string
	}{
		{name: "CompletedWithoutTerminalRecord", reason: adapter.FlushReasonCompleted, wantKind: factorysessions.ResponseEventKindStreamGap, wantPhase: factorysessions.ResponseEventPhaseUpdated, wantGap: cursorToolGapFlush},
		{name: "ProcessTerminated", reason: adapter.FlushReasonTerminated, wantKind: factorysessions.ResponseEventKindStreamGap, wantPhase: factorysessions.ResponseEventPhaseUpdated, wantGap: cursorToolGapTerminated},
		{name: "ProviderCanceled", reason: adapter.FlushReasonCanceled, wantKind: factorysessions.ResponseEventKindTool, wantPhase: factorysessions.ResponseEventPhaseCanceled},
	}
...
func TestResponseEventDecoder_TerminalFailureAndCancellationDoNotCompleteMessage(t *testing.T) {
	testCases := []struct {
		...
		{name: "Failure", record: `{"type":"result","subtype":"api_error","is_error":true,"result":"Provider unavailable","session_id":"cursor-result-session"}`, wantKind: factorysessions.ResponseEventKindError, wantPhase: factorysessions.ResponseEventPhaseFailed},
		{name: "ErrorFlaggedSuccess", record: `{"type":"result","subtype":"success","is_error":true,"result":"Request timed out","session_id":"cursor-result-session"}`, wantKind: factorysessions.ResponseEventKindError, wantPhase: factorysessions.ResponseEventPhaseFailed},
		{name: "Canceled", record: `{"type":"result","subtype":"canceled","is_error":true,"result":"User canceled","session_id":"cursor-result-session"}`, wantKind: factorysessions.ResponseEventKindRun, wantPhase: factorysessions.ResponseEventPhaseCanceled},
	}
...
func TestResponseEventDecoder_TerminalSnapshotIsBounded(t *testing.T) {
	...
	if result.Drafts[0].Provenance.Fidelity != factorysessions.ResponseEventFidelityLossy {
...
func cursorDraftsOfKind(drafts []factorysessions.ResponseEventDraft, kind factorysessions.ResponseEventKind) []factorysessions.ResponseEventDraft {
	var matching []factorysessions.ResponseEventDraft
...
func cursorDraftOfKind(drafts []factorysessions.ResponseEventDraft, kind factorysessions.ResponseEventKind) *factorysessions.ResponseEventDraft {
...
func assertCursorSessionDraft(t *testing.T, draft factorysessions.ResponseEventDraft) {
	t.Helper()
	if draft.Kind != factorysessions.ResponseEventKindSession || draft.Phase != factorysessions.ResponseEventPhaseStarted {
...
func assertCursorMessageDelta(t *testing.T, draft factorysessions.ResponseEventDraft, wantText string) {
	t.Helper()
	if draft.Kind != factorysessions.ResponseEventKindMessage || draft.Phase != factorysessions.ResponseEventPhaseDelta {
	...
	if draft.Provenance.Provider != "cursor" || draft.Provenance.NativeEventType != "assistant" || draft.Provenance.Representation != factorysessions.ResponseEventRepresentationDelta {
		t.Fatalf("message provenance = %#v", draft.Provenance)
	}
	var payload factorysessions.ResponseEventMessageDelta
	if err := json.Unmarshal(draft.Payload, &payload); err != nil {
		t.Fatalf("decode message payload: %v", err)
	}
	if payload.ContentBlockKind != factorysessions.ResponseEventContentBlockText || payload.TextDelta != wantText {
		t.Fatalf("message payload = %#v, want text %q", payload, wantText)
	}
}

func assertCursorMessageSnapshot(t *testing.T, draft factorysessions.ResponseEventDraft, wantText string) {
	t.Helper()
	...
	if draft.Provenance.Provider != "cursor" || draft.Provenance.NativeEventType != "result" || draft.Provenance.NativeEventSubtype != "success" || draft.Provenance.Representation != factorysessions.ResponseEventRepresentationSnapshot {
		t.Fatalf("message snapshot provenance = %#v", draft.Provenance)
	}
	var payload factorysessions.ResponseEventMessage
	decodeCursorPayload(t, draft, &payload)
	if payload.Role != "assistant" || len(payload.ContentBlocks) != 1 || payload.ContentBlocks[0].Kind != factorysessions.ResponseEventContentBlockText || payload.ContentBlocks[0].Text != wantText {
		t.Fatalf("message snapshot payload = %#v, want %q", payload, wantText)
	}
}

func reconstructCursorMessage(t *testing.T, drafts []factorysessions.ResponseEventDraft) string {
	t.Helper()
...
func decodeCursorPayload(t *testing.T, draft factorysessions.ResponseEventDraft, target any) {
	t.Helper()
...
func assertCursorToolDraft(t *testing.T, draft factorysessions.ResponseEventDraft, phase factorysessions.ResponseEventPhase, callID, name, status string) {
	t.Helper()
	if draft.Kind != factorysessions.ResponseEventKindTool || draft.Phase != phase {
	...
	if draft.Provenance.Provider != "cursor" || draft.Provenance.NativeEventType != "tool_call" || draft.Provenance.Representation != factorysessions.ResponseEventRepresentationNotification {
...
func assertCursorToolGapDraft(t *testing.T, draft factorysessions.ResponseEventDraft, callID, reason string) {
	t.Helper()
	if draft.Kind != factorysessions.ResponseEventKindStreamGap || draft.Phase != factorysessions.ResponseEventPhaseUpdated || draft.ItemID != "cursor-tool/"+callID {
		t.Fatalf("tool gap correlation = %#v", draft)
	}
	if draft.Provenance.Provider != "cursor" || draft.Provenance.Delivery != factorysessions.ResponseEventDeliverySynthesized {
...
func assertCursorSynthesizedToolCancellation(t *testing.T, draft factorysessions.ResponseEventDraft, callID, name, nativeType string) {
	t.Helper()
	if draft.Kind != factorysessions.ResponseEventKindTool || draft.Phase != factorysessions.ResponseEventPhaseCanceled || draft.ItemID != "cursor-tool/"+callID {
		t.Fatalf("synthesized tool cancellation = %#v", draft)
	}
	if draft.Provenance.Provider != "cursor" || draft.Provenance.Delivery != factorysessions.ResponseEventDeliverySynthesized || draft.Provenance.NativeEventType != nativeType {
...
func decodeCursorToolPayload(t *testing.T, draft factorysessions.ResponseEventDraft, target *factorysessions.ResponseEventTool) {
	t.Helper()
	if err := json.Unmarshal(draft.Payload, target); err != nil {
