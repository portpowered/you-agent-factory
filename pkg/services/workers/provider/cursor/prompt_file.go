...
func (a *Adapter) materializePrompt(
	ctx context.Context,
	...
	operatingSystem := strings.ToLower(strings.TrimSpace(a.operatingSystem))
	if operatingSystem == "" {
	...
	path, cleanup, err := writePromptFile(ctx, a.temporaryFiles, directory, prompt)
	if err != nil {
		return nil, nil, err
	}
