...
func TestAdapterBuildCommandRequestsStructuredPartialMessages(t *testing.T) {
	...
	built, err := claude.NewAdapter().BuildCommand(context.Background(), adapter.CommandContext{
		Request: workerexecution.ProviderInferenceRequest{
			ModelProvider: string(modelprovider.ProviderClaude), Model: "claude-sonnet-4",
...
func TestDecoderInvalidToolInputDoesNotLeakOrStopLaterMessage(t *testing.T) {
	...
	completed := findDraft(drafts, factorysessions.ResponseEventKindMessage, factorysessions.ResponseEventPhaseCompleted)
...
func assertCanonicalDrafts(t *testing.T, drafts []factorysessions.ResponseEventDraft) {
	t.Helper()
	for index, draft := range drafts {
		if err := factorysessions.ValidateResponseEventDraft(draft); err != nil {
			t.Fatalf("draft[%d] invalid: %v", index, err)
...
func assertControlObservations(t *testing.T, drafts []factorysessions.ResponseEventDraft) {
	t.Helper()
	retry := findDraft(drafts, factorysessions.ResponseEventKindError, factorysessions.ResponseEventPhaseUpdated)
	if retry == nil {
	...
	compaction := findDraft(drafts, factorysessions.ResponseEventKindProgress, factorysessions.ResponseEventPhaseUpdated)
	if compaction == nil {
		t.Fatalf("compaction observation missing: %#v", drafts)
	}
}

func assertControlAttribution(t *testing.T, drafts []factorysessions.ResponseEventDraft) {
	t.Helper()
	completed := draftsByKindAndPhase(drafts, factorysessions.ResponseEventKindMessage, factorysessions.ResponseEventPhaseCompleted)
	...
	delta := findDraft(drafts, factorysessions.ResponseEventKindMessage, factorysessions.ResponseEventPhaseDelta)
...
func assertControlOutputSafe(t *testing.T, drafts []factorysessions.ResponseEventDraft, diagnostics []adapter.Diagnostic) {
	t.Helper()
	encoded, err := json.Marshal(struct {
...
func TestDecoderOmitsOutOfRangeRetryMetadata(t *testing.T) {
	...
	retry := findDraft(result.Drafts, factorysessions.ResponseEventKindError, factorysessions.ResponseEventPhaseUpdated)
	if retry == nil {
...
func TestDecoderCompleteToolSnapshotReusesLogicalToolIdentity(t *testing.T) {
	...
	assertValidDrafts(t, drafts, diagnostics)
	completedTools := draftsByKindAndPhase(drafts, factorysessions.ResponseEventKindTool, factorysessions.ResponseEventPhaseCompleted)
	...
	var authoritative factorysessions.ResponseEventTool
	...
	if got := len(draftsByKindAndPhase(drafts, factorysessions.ResponseEventKindMessage, factorysessions.ResponseEventPhaseCompleted)); got != 1 {
...
func decodeFixture(t *testing.T, path string, keepFinalRecordUnterminated bool) ([]factorysessions.ResponseEventDraft, []adapter.Diagnostic) {
	t.Helper()
...
func assertValidDrafts(t *testing.T, drafts []factorysessions.ResponseEventDraft, diagnostics []adapter.Diagnostic) {
	t.Helper()
	if len(diagnostics) != 0 {
		t.Fatalf("diagnostics = %#v, want none", diagnostics)
	}
...
func draftsByKindAndPhase(drafts []factorysessions.ResponseEventDraft, kind factorysessions.ResponseEventKind, phase factorysessions.ResponseEventPhase) []factorysessions.ResponseEventDraft {
...
func assertMessageContent(t *testing.T, draft factorysessions.ResponseEventDraft, itemID string, texts []string) {
	t.Helper()
...
func assertMessageDrafts(t *testing.T, drafts []factorysessions.ResponseEventDraft) {
	t.Helper()
	...
	var completed factorysessions.ResponseEventMessage
...
func assertToolDrafts(t *testing.T, drafts []factorysessions.ResponseEventDraft) {
	t.Helper()
...
func findDraft(drafts []factorysessions.ResponseEventDraft, kind factorysessions.ResponseEventKind, phase factorysessions.ResponseEventPhase) *factorysessions.ResponseEventDraft {
