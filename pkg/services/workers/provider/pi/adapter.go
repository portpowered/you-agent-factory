...
func (*Adapter) Identity() adapter.Identity { return adapter.Identity(modelprovider.ProviderPi) }
...
func (*Adapter) BuildCommand(_ context.Context, input adapter.CommandContext) (adapter.CommandBuildResult, error) {
	...
	command.Command = string(modelprovider.ProviderPi)
	command.Args = args
...
func providerSession(sessionID string) *workerexecution.ProviderSessionMetadata {
	...
	return &workerexecution.ProviderSessionMetadata{
		Provider: string(modelprovider.ProviderPi), Kind: providerSessionKind, ID: sessionID,
	}
}
