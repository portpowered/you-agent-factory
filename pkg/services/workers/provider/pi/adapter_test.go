...
func TestAdapterBuildCommandRequestsNonInteractiveJSONMode(t *testing.T) {
	...
	built, err := pi.NewAdapter().BuildCommand(context.Background(), adapter.CommandContext{
		Request: workerexecution.ProviderInferenceRequest{
			ModelProvider: string(modelprovider.ProviderPi),
...
func TestSelectableInvocationIntegrationReturnsAuthoritativeTerminalResult(t *testing.T) {
	...
	if !publishedDraft(published, factorysessions.ResponseEventKindMessage, factorysessions.ResponseEventPhaseCompleted, "msg-production") {
...
func TestAdapterSelectableInvocationFailureIsNormalized(t *testing.T) {
	...
	provider := workerprovider.NewScriptWrapProviderWithDependencies(false, nil, &recordingRunner{
	...
	}, nil, func(workerprovider.InferenceProgressFragment) {}, structured.NewExecutor(), "", nil, nil)
