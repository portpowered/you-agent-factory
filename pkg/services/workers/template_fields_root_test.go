...
func TestResolveTemplateFields_ProjectVariableFallsBackToContextThenNeutralDefault(t *testing.T) {
	...
	resolved, err := workerprompting.ResolveTemplateFields(
		...
		tokens,
		&workers.Context{ProjectID: "analytics-platform"},
		"",
	)
	if err != nil {
	...
	if resolved.Env["PROJECT"] != workers.DefaultProjectID {
		t.Fatalf("expected neutral project fallback %q, got %s", workers.DefaultProjectID, resolved.Env["PROJECT"])
	}
	if resolved.Env["CONTEXT_PROJECT"] != workers.DefaultProjectID {
		t.Fatalf("expected neutral context project fallback %q, got %s", workers.DefaultProjectID, resolved.Env["CONTEXT_PROJECT"])
	}
}
...
func TestApplyResolvedFields_NilResolved(t *testing.T) {
	base := &workers.Context{WorkDirectory: "/base"}
