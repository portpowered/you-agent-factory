...
type codexWorktreePreparer struct{}

func (codexWorktreePreparer) Prepare(
	_ context.Context,
	factoryRoot string,
	worktreeName string,
...
func newCodexWorktreeTestWorkstationExecutor(
	...
	return &executor.WorkstationExecutor{
		...
		WorktreePreparer: codexWorktreePreparer{},
	}
}
...
func TestWorkstationExecutor_LegacyClaudeWorktree_SkipsCodexFactoryPreparation(t *testing.T) {
	...
	result, err := we.Execute(context.Background(), work.WorkDispatch{
		...
		InputTokens: executor.InputTokens(factoryruntime.RuntimeToken{
			ID:    "tok-1",
			Color: factoryruntime.RuntimeTokenColor{WorkID: "work-1", Tags: map[string]string{"branch": "my-feature-branch"}},
		}),
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
...
func TestWorkstationExecutor_CodexWorktreePreparation_SkipsWhenWorkingDirectoryAuthored(t *testing.T) {
	...
	_, err := we.Execute(context.Background(), work.WorkDispatch{
		...
		InputTokens: executor.InputTokens(factoryruntime.RuntimeToken{
			ID:    "tok-1",
			Color: factoryruntime.RuntimeTokenColor{WorkID: "work-1", Tags: map[string]string{"branch": "feature-a"}},
		}),
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
