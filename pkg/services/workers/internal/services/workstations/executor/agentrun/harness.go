...
func NewLibraryHarnessAdapter(toolFileSystem workerexecution.AgentToolFileSystem) *LibraryHarnessAdapter {
	return &LibraryHarnessAdapter{toolFileSystem: toolFileSystem}
}
...
func (a *LibraryHarnessAdapter) Execute(ctx context.Context, input HarnessInput) (HarnessResult, error) {
	...
	if workerconfig.AgentToolsAllowExecution(policy) && a.toolFileSystem == nil {
		return HarnessResult{}, ErrToolFileSystemRequired
	}
