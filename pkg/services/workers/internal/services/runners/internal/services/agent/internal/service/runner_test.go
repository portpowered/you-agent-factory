...
func TestExecutePreservesProviderOutcomeAndStructuredDiagnostics(t *testing.T) {
	...
	if result.Diagnostics == nil || result.Diagnostics.Command == nil || result.Diagnostics.Command.Duration != 1500*time.Millisecond || result.Diagnostics.Panic == nil || result.Diagnostics.Provider == nil || result.Diagnostics.Provider.ResponseMetadata["phase"] != "complete" {
		t.Fatalf("runner diagnostics = %#v, want structured command, panic, and metadata", result.Diagnostics)
	}
}
