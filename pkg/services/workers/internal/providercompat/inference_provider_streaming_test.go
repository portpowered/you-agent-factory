...
func TestScriptWrapProvider_Infer_ClaudeCompletionPublisherPreservesFinalResponse(t *testing.T) {
	...
	if published[0].Continuation == nil || published[0].Continuation.ProviderSessionID != "claude-session-123" {
		t.Fatalf("provider continuation = %#v, want claude-session-123", published[0].Continuation)
	}
}
...
func assertEquivalentInferenceResponse(t *testing.T, got, want workerexecution.InferenceResponse) {
	...
	if !reflect.DeepEqual(got.Continuation, want.Continuation) {
		t.Fatalf("provider continuation = %#v, want %#v", got.Continuation, want.Continuation)
	}
