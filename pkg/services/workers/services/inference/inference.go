...
func WorkContentFromInferenceOutput(raw string, operation interfaces.ModelOperation) ([]work.WorkContentPart, error) {
	...
	if err := json.Unmarshal([]byte(trimmed), &content); err == nil {
		return orderWorkContentByOperationOutputs(work.SupportedContentParts(content), operation), nil
	}
	var envelope struct {
		Content []work.WorkContentPart `json:"content"`
	}
	if err := json.Unmarshal([]byte(trimmed), &envelope); err == nil && envelope.Content != nil {
		return orderWorkContentByOperationOutputs(work.SupportedContentParts(envelope.Content), operation), nil
	}
...
func resolveModelOperationBinding(binding interfaces.ModelOperationBinding, inputTokens []workerexecution.Token) workerexecution.ResolvedModelOperationBinding {
...
func findMatchingInputContentPart(inputTokens []workerexecution.Token, selector *interfaces.ModelOperationBindingSelector) (work.WorkContentPart, bool) {
