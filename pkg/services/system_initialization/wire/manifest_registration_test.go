...
func TestManifestRegistration_BootstrapWirePackageIsRegistered(t *testing.T) {
	t.Helper()
...
func assertPackageTargetManifestRegistration(t *testing.T) {
	...
	for _, packagePath := range manifest.Inventory {
		if packagePath == bootstrapWirePackagePath {
			foundInventory = true
			break
		}
	}
	if !foundInventory {
		t.Fatalf("package-target manifest inventory missing %q", bootstrapWirePackagePath)
	}

	for _, row := range manifest.Packages {
		if row.PackagePath != bootstrapWirePackagePath {
			continue
		}
	...
	t.Fatalf("package-target manifest packages missing %q", bootstrapWirePackagePath)
}
...
func assertOwnershipInventoryRegistration(t *testing.T) {
	...
	t.Fatalf("ownership inventory packages missing %q", bootstrapWirePackagePath)
}
