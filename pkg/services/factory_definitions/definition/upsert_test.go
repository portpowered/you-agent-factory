...
func TestSaveUpsertNamedAndActivate_RestoresExistingLayoutOnActivationFailure(t *testing.T) {
	...
	svc := newTestService(host, host.activationGateway())
	candidate := editableFactoryFromPayload(t, namedFactoryPayload(t, "alpha-updated"))
...
func editableFactoryFromPayload(t *testing.T, payload []byte) factorydefinitions.EditableFactory {
	t.Helper()
	var metadata struct {
		Name    string                             `json:"name"`
		Version *factorydefinitions.FactoryVersion `json:"version"`
	}
	if err := json.Unmarshal(payload, &metadata); err != nil {
		t.Fatalf("decode Factory metadata: %v", err)
	}
