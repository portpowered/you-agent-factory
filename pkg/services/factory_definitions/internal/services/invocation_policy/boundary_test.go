...
const invocationPolicyPublicPackage = "github.com/portpowered/infinite-you/pkg/services/factory_definitions/internal/services/invocation_policy"

var invocationPolicyForbiddenImportRoots = []string{
...
var invocationPolicyAllowedPublicTypeImportPrefixes = []string{
...
func TestPackageBoundary_PublicSurfaceDoesNotImportForbiddenOwnership(t *testing.T) {
	...
	assertPackageDirectImportsForbidden(t, invocationPolicyPublicPackage, invocationPolicyForbiddenImportRoots)
}

func TestPackageBoundary_PublicSurfaceDeclaresOnlyInvocationPolicyVocabulary(t *testing.T) {
	t.Parallel()
...
func assertTypeExprUsesAllowedImports(t *testing.T, expr ast.Expr) {
	...
	case *ast.Ident:
		...
		t.Fatalf("unexpected bare identifier %s on invocation_policy public surface; use factory_definitions vocabulary", typed.Name)
	case *ast.SelectorExpr:
		...
		if prefix == "" {
			t.Fatalf("could not resolve selector %s on invocation_policy public surface", exprString(expr))
		}
		for _, allowed := range invocationPolicyAllowedPublicTypeImportPrefixes {
			if prefix == allowed {
				return
			}
		}
		t.Fatalf("invocation_policy public surface type %s must use only factory_definitions root imports", exprString(expr))
	default:
		t.Fatalf("unexpected type expression %T on invocation_policy public surface", expr)
	}
}
...
func TestPackageBoundary_WireSurfaceDoesNotConstructRuntimeOrSelectSiblingLeases(t *testing.T) {
	...
	wirePackage := invocationPolicyPublicPackage + "/wire"
	assertPackageDirectImportsForbidden(t, wirePackage, invocationPolicyForbiddenImportRoots)
}
