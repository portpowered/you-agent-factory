...
func NewExplicit(
	...
	captureLoaded := NewLoaded(mapSnapshotObject, mergeRuntime...)
	return func(
		factoryDir string,
...
func CaptureLoaded(
	...
	factoryWithRuntime := factoryConfig
	if len(mergeRuntime) > 0 && mergeRuntime[0] != nil {
		var err error
		factoryWithRuntime, err = mergeRuntime[0](factoryConfig, source)
		if err != nil {
...
func factorySnapshotMetadata(
	...
	result["runtime_config_hash"] = snapshotSHA256(struct {
		...
		Workers      map[string]factorydefinitions.FactoryWorkerConfig      `json:"workers,omitempty"`
