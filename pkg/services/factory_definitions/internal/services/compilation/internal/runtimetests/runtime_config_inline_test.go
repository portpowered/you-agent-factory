...
import (
	...
	factoryruntimeconfig "github.com/portpowered/infinite-you/pkg/services/factory_definitions/internal/services/compilation/internal/runtimeconfig"
)
...
func TestRuntimeConfigMerge_MergesRuntimeDefinitionsOntoCanonicalConfig(t *testing.T) {
	inlined, err := factoryruntimeconfig.Merge(canonicalMergeFactoryConfig(), canonicalMergeRuntimeDefinitions())
	if err != nil {
...
func TestRuntimeConfigMerge_PreservesCanonicalWorkstationKindForMapping(t *testing.T) {
	...
	if len(merged.Inputs) != 1 ||
		merged.Inputs[0].WorkTypeName != "story" ||
		merged.Inputs[0].StateName != "ready" {
		t.Fatalf("merged workstation inputs = %+v, want story:ready", merged.Inputs)
	}
}

func TestRuntimeConfigMerge_UsesCanonicalDefinitionsWhenRuntimeDefinitionsAreMissing(t *testing.T) {
	inlined, err := factoryruntimeconfig.Merge(canonicalMergeFactoryConfig(), emptyRuntimeDefinitionLookup())
	if err != nil {
		t.Fatalf("runtimeconfig.Merge: %v", err)
	}
...
func TestNewLoadedFactoryConfig_LoadsCanonicalConfigWithoutRuntimeConfig(t *testing.T) {
	loaded, err := NewLoadedFactoryConfig(
		"factory-dir",
		canonicalMergeFactoryConfig(),
		emptyRuntimeDefinitionLookup(),
	)
	if err != nil {
		t.Fatalf("NewLoadedFactoryConfig: %v", err)
	}
