...
func TestMapNamedFactoryDir_HierarchicalScopedLayout(t *testing.T) {
	...
	got, err := factorydefinitions.MapDir(rootDir, "@you/goal")
	if err != nil {
	...
	segments, err := factorydefinitions.PathSegments("@you/goal")
	if err != nil {
	...
	roundTrip, err := factorydefinitions.NameFromPathSegments(segments)
	if err != nil {
...
func TestValidateNamedFactoryName_RejectsPathTraversal(t *testing.T) {
	err := factorydefinitions.ValidateName("../beta")
	if err == nil {
		t.Fatal("expected invalid named-factory segment to fail")
	}
	if !errors.Is(err, factorydefinitions.ErrInvalidName) {
