...
const compilationPublicPackage = "github.com/portpowered/infinite-you/pkg/services/factory_definitions/internal/services/compilation"

var compilationForbiddenImportRoots = []string{
...
func TestPackageBoundary_PublicSurfaceDoesNotImportForbiddenOwnership(t *testing.T) {
	...
	assertPackageDirectImportsForbidden(t, compilationPublicPackage, compilationForbiddenImportRoots)
}

func TestPackageBoundary_PublicSurfaceDeclaresOnlyCTRDEFCompileVocabulary(t *testing.T) {
	t.Parallel()
...
func assertTypeExprUsesAllowedImports(t *testing.T, expr ast.Expr) {
	...
	case *ast.Ident:
		...
		t.Fatalf("unexpected bare identifier %s on compilation public surface; use factory_definitions or contracts vocabulary", typed.Name)
	case *ast.SelectorExpr:
		...
		if prefix == "" {
			t.Fatalf("could not resolve selector %s on compilation public surface", exprString(expr))
		}
		for _, allowed := range compilationAllowedPublicTypeImportPrefixes {
			if prefix == allowed {
				return
			}
		}
...
func TestPackageBoundary_WireSurfaceDoesNotConstructRuntimeOrSelectSiblingLeases(t *testing.T) {
	...
	wirePackage := compilationPublicPackage + "/wire"
	assertPackageDirectImportsForbidden(t, wirePackage, compilationForbiddenImportRoots)
}
