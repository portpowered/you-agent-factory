...
func TestValidate_AllowsRecurringControllerWithoutTerminalState(t *testing.T) {
	cfg := &factorydefinitions.FactoryConfig{
		WorkTypes: []factorydefinitions.WorkTypeConfig{
			{
				Name: "controller",
	...
	if result := Validate(cfg); result.HasBlockingTargets() {
		t.Fatalf("recurring controller validation = %#v, want no blocking targets", result.BlockingTargets())
	}
}

func TestValidate_AllowsCompositeParentWithoutTerminalState(t *testing.T) {
	cfg := &factorydefinitions.FactoryConfig{
		WorkTypes: []factorydefinitions.WorkTypeConfig{
			{
				Name: "request",
	...
	if result := Validate(cfg); result.HasBlockingTargets() {
		t.Fatalf("composite parent validation = %#v, want no blocking targets", result.BlockingTargets())
	}
}
...
func TestValidate_RejectsNonTerminalWorkTypeWithoutDelegatedOutcome(t *testing.T) {
	...
	for _, target := range result.BlockingTargets() {
		if target.Code == CodeWorkTypeMissingCompletionState {
			return
		}
	}
	t.Fatalf("validation targets = %#v, want missing completion-state target", result.BlockingTargets())
}
