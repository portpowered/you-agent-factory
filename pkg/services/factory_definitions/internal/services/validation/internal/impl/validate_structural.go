...
func ManagedRuntimeDependencyTargets(cfg *factorydefinitions.FactoryConfig) []Target {
	...
	resourceByName := make(map[string]factorydefinitions.ResourceConfig, len(cfg.Resources))
	var targets []Target
...
func managedRuntimeResourceTargets(index int, resource factorydefinitions.ResourceConfig) []Target {
	if strings.TrimSpace(resource.Type) != factorydefinitions.ResourceTypeModel {
		return nil
	}
...
func managedRuntimeWorkerTargets(
	...
	resourceByName map[string]factorydefinitions.ResourceConfig,
) []Target {
	if !factorydefinitions.IsInferenceWorkerType(worker.Type) {
		return nil
	}
	if strings.TrimSpace(worker.ModelLocality) != factorydefinitions.ModelLocalityLocal {
		return nil
	}
	...
	var matchedResource *factorydefinitions.ResourceConfig
	for _, requirement := range worker.Resources {
		resource, ok := resourceByName[strings.TrimSpace(requirement.Name)]
		if !ok || strings.TrimSpace(resource.Type) != factorydefinitions.ResourceTypeModel {
			continue
		}
