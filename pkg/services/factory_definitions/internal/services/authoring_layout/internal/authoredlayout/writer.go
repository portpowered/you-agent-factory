...
func NewWriter(
	...
	var normalize func(*factorydefinitions.FactoryWorkstationConfig)
	if len(normalizeWorkstation) > 0 {
		normalize = normalizeWorkstation[0]
	}
	return &Writer{
		...
		normalizeWorkstation: normalize,
	}
}
...
func (w *Writer) writeWorkstation(
	...
	definition, promptContent := workstationForExpansion(workstation, w.normalizeWorkstation)
...
func workstationForExpansion(
	...
	if normalizeWorkstation != nil {
		normalizeWorkstation(&definition)
	}
	promptContent := ""
	if definition.PromptFile != "" {
