...
func TestCatalogListsAndResolvesDetachedDefinitionsInPublicNameOrder(t *testing.T) {
	t.Parallel()

	service, err := distributionpackagedcatalog.New([]factorydefinitions.PackagedDefinition{
		{
...
func TestCatalogUnknownNameReportsStablePublicInventory(t *testing.T) {
	t.Parallel()
...
func TestCatalogBlankNameFailsWithUnknownIdentity(t *testing.T) {
	t.Parallel()
	...
	if !errors.Is(err, factorydefinitions.ErrUnknownPackagedFactoryIdentity) {
		t.Fatalf("resolve blank name error = %v, want ErrUnknownPackagedFactoryIdentity", err)
	}
}
