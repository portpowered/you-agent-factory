...
func TestValidate_CanonicalFindingsAndStableIdentity(t *testing.T) {
	t.Run("canonical findings match config validation", func(t *testing.T) {
		...
		cfg, err := factorymapping.FactoryConfigFromOpenAPI(apiFactory)
		if err != nil {
			t.Fatalf("FactoryConfigFromOpenAPI: %v", err)
		}
		explicit := factoryvalidation.Validate(&cfg)
		findings := factoryvalidation.New(nil).Validate(t.Context(), &cfg, nil).Targets
		...
		for i, target := range explicit.Targets {
			if findings[i].Code != target.Code {
				t.Fatalf("finding[%d].Code = %q, want %q", i, findings[i].Code, target.Code)
			}
		}
	...
	t.Run("legacy example without ids remains valid", func(t *testing.T) {
		...
		loaded, err := factorydefinitioncomposition.LoadCanonicalJSON(
			payload,
			nil,
		)
		if err != nil {
