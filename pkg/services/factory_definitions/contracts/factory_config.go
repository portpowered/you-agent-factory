...
// NameValueConfig carries customer-facing copy with a required base fallback
// and optional exact locale overrides. ID remains opaque metadata.
...
// NameValueValidationError identifies the exact NameValue field that failed
// semantic validation.
...
// ValidateNameValue enforces the semantic constraints that OpenAPI 3.0 cannot
// express for locale-keyed maps.
func ValidateNameValue(value NameValueConfig) error {
	...
	if err := validateNameValueLocaleList(value.Locales); err != nil {
		return err
	}
	return validateNameValueLocaleValues(value.Values)
}
...
// ResolveNameValue returns only an exact canonical override. Any absent,
// differently cased, or less-specific locale falls back to the base value.
...
func validateNameValueLocaleList(locales []string) error {
...
func validateNameValueLocaleValues(values map[string]string) error {
