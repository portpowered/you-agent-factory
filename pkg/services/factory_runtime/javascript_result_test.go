...
func TestWorkArtifactProjection_UsesCompletedVerificationAndResultFallbacks(t *testing.T) {
	...
	token := workerexecution.Token{ID: "token-1", PlaceID: "story:init", Color: workerexecution.Color{
...
func TestWorkArtifactProjection_FallsBackToPlaceTopologyAndHandlesEmptyInput(t *testing.T) {
	...
	net := &factory.RuntimeNet{
		...
		Transitions: map[string]*factory.PetriTransition{
			"z-review": {ID: "z-review", Name: "z-review", InputArcs: []factory.PetriArc{{PlaceID: "other:init"}}, ExpectedArtifacts: []work.ExpectedArtifactDeclaration{{Name: "other", Pattern: "other.txt"}}},
			"a-review": {ID: "a-review", Name: "a-review", InputArcs: []factory.PetriArc{{PlaceID: "story:init"}}, ExpectedArtifacts: []work.ExpectedArtifactDeclaration{{Name: "review", Pattern: "review.txt"}}},
			"nil":      nil,
		},
	}
...
func tokenPointer(token workerexecution.Token) *workerexecution.Token {
	return &token
}
