	...
	err := walkProductionPkgFiles(func(path, rel string, file *ast.File, fset *token.FileSet) error {
		...
		return scanTransitionExhaustionUses(file, fset, transitionImportAliases(file), func(funcName string, line int) error {
			...
			if len(allowedFunctions) == 0 {
				return fmt.Errorf("%s:%d introduces factoryruntime.PetriTransitionExhaustion outside the approved expiry/circuit-breaker sites", path, line)
			}
			if funcName == "" {
				return fmt.Errorf("%s:%d uses factoryruntime.PetriTransitionExhaustion outside the approved file-level declaration site", path, line)
			}
			return fmt.Errorf("%s:%d uses factoryruntime.PetriTransitionExhaustion in %s; only approved expiry/circuit-breaker sites may keep it", path, line, funcName)
		})
	})
	if err != nil {
		t.Fatal(err)
...
func transitionImportAliases(file *ast.File) map[string]struct{} {
	...
	for _, imp := range file.Imports {
		if imp.Path == nil || imp.Path.Value != `factoryruntime "github.com/portpowered/infinite-you/pkg/services/factory_runtime"` {
			continue
		}
		name := "petri"
		if imp.Name != nil {
