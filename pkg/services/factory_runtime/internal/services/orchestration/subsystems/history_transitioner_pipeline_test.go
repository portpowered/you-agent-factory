...
func TestHistoryTransitionerPipeline_TerminalResultOutcomeRoutes(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assertTerminalResultCancellation(t, completed, test.wantCancellation)
		})
	}
}

func assertTerminalResultCancellation(t *testing.T, completed interfaces.CompletedDispatch, want workerexecution.DispatchCancellationReason) {
	t.Helper()
	if want == "" {
	...
	if completed.Cancellation == nil || completed.Cancellation.Reason != want {
		t.Fatalf("completion cancellation = %#v, want %s", completed.Cancellation, want)
	}
}
