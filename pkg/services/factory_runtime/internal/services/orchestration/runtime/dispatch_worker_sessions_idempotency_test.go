...
func requireRecordedCancellationProjection(t *testing.T, base time.Time, completed interfaces.FactoryWorldDispatchCompletion) {
	t.Helper()
...
func requireRecordedOutcomeMappings(t *testing.T) {
	t.Helper()
	if recordedObservationState(string(workers.OutcomeAccepted)) != workersessions.StateCompleted ||
		recordedObservationState(string(workers.OutcomeContinue)) != workersessions.StateCompleted ||
		recordedObservationState(string(workers.OutcomeRejected)) != workersessions.StateFailed ||
		recordedObservationState("unknown") != workersessions.StateFailed {
