...
func TestRecordedDispatchFailureProjection(t *testing.T) {
	...
	providerMetadata := &providers.SessionMetadata{Provider: "codex", Kind: providers.SessionIDKind, ID: "provider-session-1"}
...
func TestRecordedWorkerSessionObservation_ProjectsRestartInterruptionAsTerminalFailure(t *testing.T) {
	...
	events := []interfaces.FactoryEvent{
		...
		},
		{
			...
			Payload: mustMarshalRecordedTest(t, interfaces.DispatchWorkerSessionAssociationEventPayload{WorkerSessionID: "worker-restarted"}),
		},
		{
			Context: interfaces.FactoryEventContext{
				Tick: 2, Sequence: 3, EventTime: base.Add(2 * time.Second),
...
func newRecordedExactObservationFixture(t *testing.T) recordedExactObservationFixture {
	...
	providerMetadata := &providers.SessionMetadata{Provider: string(providers.IDCodex), Kind: providers.SessionIDKind, ID: "provider-session-exact"}
