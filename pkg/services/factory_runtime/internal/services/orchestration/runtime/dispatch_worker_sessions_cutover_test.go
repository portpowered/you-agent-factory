...
func TestRecordedWorkerSessionObservationConfirmationFollowsCompletedFlushWatermark(t *testing.T) {
	...
	listBeforeFlush, err := service.ListObservations(context.Background(), workersessions.ListObservationsRequest{WorkID: workID})
	if err != nil {
		t.Fatalf("ListObservations(before flush) error = %v", err)
	}
	if len(listBeforeFlush.Observations) != 1 {
		t.Fatalf("ListObservations(before flush) count = %d, want 1", len(listBeforeFlush.Observations))
	}
	before := listBeforeFlush.Observations[0]
	...
	showBeforeFlush, err := service.GetObservationByWorkerSessionID(context.Background(), workersessions.GetObservationByWorkerSessionIDRequest{WorkerSessionID: workerSessionID})
	if err != nil {
		t.Fatalf("GetObservationByWorkerSessionID(before flush) error = %v", err)
	}
	...
	listAfterFlush, err := service.ListObservations(context.Background(), workersessions.ListObservationsRequest{WorkID: workID})
	if err != nil {
		t.Fatalf("ListObservations(after flush) error = %v", err)
	}
	showAfterFlush, err := service.GetObservationByWorkerSessionID(context.Background(), workersessions.GetObservationByWorkerSessionIDRequest{WorkerSessionID: workerSessionID})
	if err != nil {
		t.Fatalf("GetObservationByWorkerSessionID(after flush) error = %v", err)
	}
	if listAfterFlush.Observations[0].ConfirmationState != workersessions.ConfirmationStateConfirmed || showAfterFlush.ConfirmationState != workersessions.ConfirmationStateConfirmed {
		t.Fatalf("after flush confirmation = list:%q show:%q, want CONFIRMED for both surfaces", listAfterFlush.Observations[0].ConfirmationState, showAfterFlush.ConfirmationState)
	}
...
func TestProcessLocalWorkerSessionObservationsRemainUnconfirmed(t *testing.T) {
	live := &processLocalWorkerSessionService{
		topLevelResult: workersessions.ListWorkerSessionObservationsResult{Observations: []workersessions.Observation{{
			WorkerSessionID:   "worker-live-running",
			AttemptID:         "attempt-live-running",
			State:             workersessions.StateRunning,
			ConfirmationState: workersessions.ConfirmationStateConfirmed,
		}}},
		getByWorkerResult: workersessions.Observation{
			WorkerSessionID:   "worker-live-terminal",
			AttemptID:         "attempt-live-terminal",
...
func TestRecordedWorkerSessionObservationReplayUsesWatermarkForConfirmation(t *testing.T) {
	...
	replayEvents := []interfaces.FactoryEvent{
		{
			Context: interfaces.FactoryEventContext{Tick: 1, Sequence: 1, EventTime: base, DispatchID: stringPointerForRecordedTest(dispatchID)},
			Id:      "worker-replay-request",
			Type:    interfaces.FactoryEventTypeDispatchRequest,
		},
		{
