...
func TestRestoredDispatchRequestEventPreservesRestartMetadataAndResources(t *testing.T) {
	...
	if event.Type != interfaces.FactoryEventTypeDispatchRequest || stringPointerValue(event.Context.DispatchID) != dispatch.DispatchID {
		t.Fatalf("restored dispatch request event = %#v, want dispatch request for %q", event, dispatch.DispatchID)
	}
	if payload.TransitionID != dispatch.TransitionID || payload.Metadata == nil ||
		stringPointerValue(payload.Metadata.RunnerID) != dispatch.RunnerID ||
		payload.Metadata.RunnerSelectionSource == nil || *payload.Metadata.RunnerSelectionSource != dispatch.RunnerSelectionSource {
		t.Fatalf("restored dispatch metadata = %#v, want runner facts from %#v", payload.Metadata, dispatch)
	}
	if payload.Resources == nil || len(*payload.Resources) != 1 || (*payload.Resources)[0].Name != "gpu" {
		t.Fatalf("restored dispatch resources = %#v, want one gpu resource", payload.Resources)
	}
	if len(payload.Inputs) != 1 || payload.Inputs[0].WorkID != "work-restart" {
		t.Fatalf("restored dispatch Work inputs = %#v, want one deduplicated Work input", payload.Inputs)
	}
