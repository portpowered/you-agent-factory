...
func TestNew_RestoresOtherWorkWhenCompletedCronWorkHasNoOccupancy(t *testing.T) {
	...
	cronWork := work.FactoryWorkItem{
		...
		TraceID:    "trace-cron-recovery",
		Tags: map[string]string{
			interfaces.TimeWorkTagKeySource:          interfaces.TimeWorkSourceCron,
			interfaces.TimeWorkTagKeyCronWorkstation: "cron-refresh",
		},
	}
	recoverableWork := work.FactoryWorkItem{
		ID: "recoverable-work", WorkTypeID: "task", State: "init", TraceID: "trace-recoverable",
	}
	restored := &interfaces.FactoryWorldState{
		WorkItemsByID: map[string]work.FactoryWorkItem{
			cronWork.ID:        cronWork,
			recoverableWork.ID: recoverableWork,
		},
		...
		// An empty occupancy projection is omitted by JSON serialization in
		// legacy recordings; exercise that nil-map restore shape here.
		PlaceOccupancyByID: nil,
		CompletedDispatches: []interfaces.FactoryWorldDispatchCompletion{{
			DispatchID:   "dispatch-cron-recovery",
			TransitionID: "cron-refresh",
			WorkItemIDs:  []string{cronWork.ID},
			ConsumedInputs: []interfaces.WorkstationInput{{
				TokenID: cronWork.ID, PlaceID: interfaces.SystemTimePendingPlaceID, WorkItem: &cronWork,
			}},
			InputWorkItems: []work.FactoryWorkItem{cronWork},
	...
	if warning.fields["session_id"] != "session-cron-recovery" ||
		warning.fields["work_id"] != cronWork.ID ||
		warning.fields["disposition"] != restoredLegacyAutomationDisposition {
		t.Fatalf("restore warning fields = %#v, want safe session/work/disposition context", warning.fields)
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			restored: &interfaces.FactoryWorldState{
				...
				ActiveWorkItemsByID: map[string]work.FactoryWorkItem{"cron-work": cronWork("cron-work")},
				PlaceOccupancyByID:  map[string]interfaces.FactoryPlaceOccupancy{},
			},
		...
		},
		{
			...
			restored: &interfaces.FactoryWorldState{
				WorkItemsByID: map[string]work.FactoryWorkItem{
					"time-lookalike": {ID: "time-lookalike", WorkTypeID: "task", State: "init"},
				},
				ActiveWorkItemsByID: map[string]work.FactoryWorkItem{
					"time-lookalike": {ID: "time-lookalike", WorkTypeID: "task", State: "init"},
				},
				PlaceOccupancyByID: map[string]interfaces.FactoryPlaceOccupancy{},
				CompletedDispatches: []interfaces.FactoryWorldDispatchCompletion{{
					DispatchID: "dispatch-lookalike", WorkItemIDs: []string{"time-lookalike"},
					Result: interfaces.WorkstationResult{Outcome: string(workerexecution.OutcomeAccepted)},
				}},
			},
			net: buildSimpleNet(),
...
func buildCronRestoreNet() *state.Net {
	...
	net.Places[interfaces.SystemTimePendingPlaceID] = &petri.Place{
		ID: interfaces.SystemTimePendingPlaceID, TypeID: interfaces.SystemTimeWorkTypeID, State: interfaces.SystemTimePendingState,
	}
	...
	net.Transitions["cron-refresh"] = &petri.Transition{
		...
		InputArcs: []petri.Arc{{
			ID: "cron-input", Name: "cron-input", PlaceID: interfaces.SystemTimePendingPlaceID,
			Direction: petri.ArcInput, Mode: interfaces.ArcModeConsume,
