...
func New(
	places map[string]*petri.Place,
	workTypes map[string]*state.WorkType,
	workIDGenerator *petri.WorkIDGenerator,
) *Transformer {
	return &Transformer{
		places:    places,
		workTypes: workTypes,
		workIDGen: workIDGenerator,
	}
}
...
func (t *Transformer) InitialTokenFromSubmit(req work.SubmitRequest, now time.Time) (*factorytoken.Token, error) {
	...
	return &factorytoken.Token{
		...
		Color: factorytoken.Color{
			...
			Tags:                     work.CloneTags(req.Tags),
			Relations:                work.CloneRelations(req.Relations),
			Content:                  cloneWorkContent(req.Content),
			Payload:                  work.ClonePayload(req.Payload),
...
func applyWorkerOutputContent(color *factorytoken.Color, output string) error {
	...
	content, err := work.ContentFromWorkerOutput(output)
	if err != nil {
...
func ApplyPreservedInputToColor(
	...
	color.Payload = work.ClonePayload(source.Payload)
	...
	if len(color.Tags) == 0 && len(source.Tags) > 0 {
		color.Tags = work.CloneTags(source.Tags)
	}
