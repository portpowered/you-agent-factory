...
func provideRuntimeHostRoot(cfg *runtimehost.Config) (composebridge.Root, error) {
	...
	configured, err := service.ConfigWithWorkerApplication(service.FactoryServiceConfigFromRuntimeHost(cfg))
	if err != nil {
		return composebridge.Root{}, err
	}
	if runtimeCfg := service.RuntimeHostConfigFromFactoryService(configured); runtimeCfg != cfg {
		*cfg = *runtimeCfg
	}
