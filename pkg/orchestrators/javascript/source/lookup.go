...
func factoryWorkflowCandidate(factoryName, factoryDir string, jsCfg *interfaces.FactoryOrchestratorJavaScriptConfig) (lookupCandidate, bool) {
	...
	reader := validation.FileSourceReader(factoryDir)
...
func resolveExplicitWorkflowFileRef(ctx Context, trimmed string) (Resolution, bool) {
	...
	for _, readerCfg := range readers {
		reader := validation.FileSourceReader(readerCfg.root)
...
func resolutionWithLoadIssues(requestKind Kind, requestValue string, stage LookupStage, sourceRef string, issues []validation.Issue) Resolution {
...
func dialectForLoaded(loaded validation.LoadedSource) string {
	switch strings.TrimSpace(loaded.Format) {
	case validation.FormatTypeScript:
		return "typescript"
	default:
