...
func TestResolve_WorkflowName_UsesProjectClaudeWorkflowsFirst(t *testing.T) {
	...
	resolution := source.Resolve(source.Request{
	...
	if resolution.LookupStage != source.LookupStageProjectClaude {
		t.Fatalf("lookup stage = %q, want %q", resolution.LookupStage, source.LookupStageProjectClaude)
	}
	if resolution.SourceRef != source.ProjectClaudeWorkflowsDir+"/review.js" {
...
func TestResolve_WorkflowName_PrefersProjectOverGlobal(t *testing.T) {
	projectRoot := t.TempDir()
	homeDir := t.TempDir()
	projectDir := filepath.Join(projectRoot, source.ProjectClaudeWorkflowsDir)
...
func TestResolve_WorkflowName_ReportsConflictForMultipleJavaScriptFactories(t *testing.T) {
	...
	if len(resolution.Diagnostics) == 0 || resolution.Diagnostics[0].Code != source.CodeSourceConflict {
...
func TestResolve_WorkflowFile_BypassesOrderedLookup(t *testing.T) {
	...
	ctx := testContext(t, projectRoot)
	resolution := source.Resolve(source.Request{
		Kind:  source.KindWorkflowFile,
		Value: interfaces.FactoryDir + "/workflows/review.js",
	}, ctx)
	if !resolution.Found || resolution.LookupStage != source.LookupStageExplicitSourceKind {
...
func TestResolve_FactoryID_ResolvesJavaScriptFactory(t *testing.T) {
	...
	ctx := testContext(t, projectRoot)
	resolution := source.Resolve(source.Request{
		Kind:  source.KindFactoryID,
		Value: "review-flow",
	}, ctx)
	if !resolution.Found || resolution.LookupStage != source.LookupStageExplicitFactory {
...
func TestResolve_InlineWorkflow_ComputesStableHash(t *testing.T) {
	...
	first := source.Resolve(source.Request{
		Kind:         source.KindInlineWorkflow,
		InlineSource: validWorkflowSource,
	}, ctx)
	second := source.Resolve(source.Request{
		Kind:         source.KindInlineWorkflow,
		InlineSource: validWorkflowSource,
	}, ctx)
...
func TestResolve_ArtifactRoot_RejectsSymlinkThatResolvesInsideRepo(t *testing.T) {
	...
	if resolution.ArtifactRoot.Diagnostic == nil || resolution.ArtifactRoot.Diagnostic.Code != source.CodeArtifactRootInsideRepo {
...
func TestResolve_ArtifactRoot_RejectsRelativeAndInsideRepoPaths(t *testing.T) {
	...
	relative := source.Resolve(source.Request{
		Kind:         source.KindInlineWorkflow,
		InlineSource: validWorkflowSource,
	...
	inside := source.Resolve(source.Request{
		Kind:         source.KindInlineWorkflow,
		InlineSource: validWorkflowSource,
	...
	outside := source.Resolve(source.Request{
		Kind:         source.KindInlineWorkflow,
		InlineSource: validWorkflowSource,
...
func TestCLINormalizeRequest_MatchesAPISurface(t *testing.T) {
	...
	req := source.Request{Kind: source.KindWorkflowName, Value: "review"}
...
func testContext(t *testing.T, projectRoot string) source.Context {
	t.Helper()
	homeDir := t.TempDir()
	return testContextWithHome(t, projectRoot, homeDir)
}

func testContextWithHome(t *testing.T, projectRoot, homeDir string) source.Context {
	t.Helper()
	...
	return source.Context{
		...
		ProjectWorkflowRoot: filepath.Join(projectRoot, source.ProjectClaudeWorkflowsDir),
