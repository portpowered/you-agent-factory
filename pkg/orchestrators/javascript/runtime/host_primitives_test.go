...
func assertProgressPhaseAndLogRecords(t *testing.T, records []workflowruntime.RuntimeRecord) {
	t.Helper()
...
func assertProgressArtifactRecord(t *testing.T, record workflowruntime.RuntimeRecord, sessionID string) string {
	t.Helper()
	artifact := record.Artifact
	if artifact == nil {
		t.Fatal("expected artifact record")
	}
	wantURI := workflowresult.FormatArtifactURI(sessionID, "artifact-1")
...
func assertProgressCheckpointRecord(t *testing.T, record workflowruntime.RuntimeRecord, wantURI string) {
	t.Helper()
