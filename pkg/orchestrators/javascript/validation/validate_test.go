...
func TestValidate_AcceptsSupportedWorkflowGlobals(t *testing.T) {
	result := validation.Validate(validation.Request{
...
func TestValidate_RejectsSyntaxErrorWithSourceLocation(t *testing.T) {
	...
	if issue.Code != validation.CodeSyntaxError {
		t.Fatalf("issue code = %q, want %q", issue.Code, validation.CodeSyntaxError)
	}
	if issue.Line <= 0 {
...
func TestValidate_RejectsInvalidArgsSchema(t *testing.T) {
	...
	if result.Issues[0].Code != validation.CodeInvalidArgsSchema {
		t.Fatalf("issue code = %q, want %q", result.Issues[0].Code, validation.CodeInvalidArgsSchema)
	}
}
...
func TestValidate_RejectsForbiddenHostAccess(t *testing.T) {
	...
	if result.Issues[0].Code != validation.CodeForbiddenHostAccess {
		t.Fatalf("issue code = %q, want %q", result.Issues[0].Code, validation.CodeForbiddenHostAccess)
	}
}
...
func TestWorkflowSourceTargets_RejectsFileBackedSyntaxError(t *testing.T) {
	...
	if targets[0].Code != validation.CodeSyntaxError {
		t.Fatalf("target code = %q, want %q", targets[0].Code, validation.CodeSyntaxError)
	}
}
...
func TestValidate_RejectsInvalidPrimitiveCallShapes(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			name: "invalid meta",
			src:  `meta("bad");`,
			code: validation.CodeInvalidMetadata,
		},
		{
			name: "invalid workflow artifact",
			src:  `workflow.artifact("log");`,
			code: validation.CodeUnsupportedPrimitive,
		},
		{
...
func TestValidateFactory_RejectsInlineForbiddenHostAccess(t *testing.T) {
	...
	if result.Targets[0].Code != validation.CodeForbiddenHostAccess {
		t.Fatalf("target code = %q, want %q", result.Targets[0].Code, validation.CodeForbiddenHostAccess)
	}
}
