...
func TestLoad_JavaScriptWorkflowPreservesSourceHash(t *testing.T) {
	...
	loaded, issues := validation.Load(validation.LoadRequest{
		SourceRef: "review.js",
		Content:   source,
	})
	if len(issues) > 0 {
	...
	if loaded.Format != validation.FormatJavaScript {
		t.Fatalf("format = %q, want %q", loaded.Format, validation.FormatJavaScript)
	}
	if loaded.SourceRef != "review.js" {
		t.Fatalf("source ref = %q, want review.js", loaded.SourceRef)
	}
	wantHash := validation.SourceHash([]byte(source))
...
`
	...
	if loaded.Format != validation.FormatTypeScript {
		t.Fatalf("format = %q, want %q", loaded.Format, validation.FormatTypeScript)
	}

	result := validation.ValidateLoaded(loaded, validation.Request{
		SourceRef: "review.ts",
	})
	if result.HasIssues() {
...
phase("setup");
`
	_, issues := validation.Load(validation.LoadRequest{
		SourceRef: "unsafe.ts",
		Content:   source,
	})
	if len(issues) == 0 {
	...
	if issues[0].Code != validation.CodeUnsupportedLoader {
		t.Fatalf("issue code = %q, want %q", issues[0].Code, validation.CodeUnsupportedLoader)
	}
	if issues[0].Line != 1 {
...
func TestLoad_SourceHashStableForUnchangedSource(t *testing.T) {
	source := validWorkflowSource
	first, _ := validation.Load(validation.LoadRequest{SourceRef: "review.js", Content: source})
	second, _ := validation.Load(validation.LoadRequest{SourceRef: "review.js", Content: source})
...
`
	...
	result := validation.ValidateLoaded(loaded, validation.Request{SourceRef: "broken.ts"})
	if !result.HasIssues() {
...
`
	...
	reader := validation.FileSourceReader(dir)
...
func TestWorkflowSourceTargets_RejectsTypeScriptImportBeforeRuntime(t *testing.T) {
	...
	if targets[0].Code != validation.CodeUnsupportedLoader {
		t.Fatalf("target code = %q, want %q", targets[0].Code, validation.CodeUnsupportedLoader)
	}
}
...
func TestWorkflowSourceTargets_RejectsSourceHashMismatch(t *testing.T) {
	...
	if targets[0].Code != validation.CodeSourceHashMismatch {
		t.Fatalf("target code = %q, want %q", targets[0].Code, validation.CodeSourceHashMismatch)
	}
}
