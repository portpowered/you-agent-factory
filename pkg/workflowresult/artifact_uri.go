...
func ParseArtifactURI(raw string) (ParsedArtifactURI, []Issue) {
	trimmed := strings.TrimSpace(raw)
	if trimmed == "" {
		return ParsedArtifactURI{}, []Issue{malformedArtifactURI(raw, "artifact URI is required")}
	}
	parsed, err := url.Parse(trimmed)
	if err != nil || parsed.Scheme != ArtifactURIScheme {
		return ParsedArtifactURI{}, []Issue{malformedArtifactURI(raw, "artifact URI must use the you-artifact scheme")}
	}
	if parsed.Host != "" && parsed.Host != "sessions" {
		return ParsedArtifactURI{}, []Issue{malformedArtifactURI(raw, "artifact URI host must be sessions")}
	}
	if strings.Contains(parsed.Path, "..") {
		return ParsedArtifactURI{}, []Issue{{
	...
	if !ok {
		return ParsedArtifactURI{}, []Issue{malformedArtifactURI(raw, "artifact URI must match you-artifact://sessions/{session_id}/artifacts/{artifact_id}")}
	}
	if issue := validateArtifactID("sessionId", sessionID); issue != nil {
		return ParsedArtifactURI{}, []Issue{*issue}
	}
...
// requested session.
func ValidateArtifactURIForSession(raw, expectedSessionID string) []Issue {
	...
	if expected == "" {
		return []Issue{malformedArtifactURI(raw, "expected session id is required")}
	}
...
func validateArtifactID(field, value string) *Issue {
...
func malformedArtifactURI(raw, message string) Issue {
	return Issue{
