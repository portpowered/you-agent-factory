...
const fixturesRelativeDir = "pkg/config/systemconfig/testdata/fixtures"

func TestProjectInputInventory_RecordsTolerantUnknownFieldPolicy(t *testing.T) {
	t.Parallel()

	inventory := systemconfig.ProjectInputInventory()
	if inventory.FormatVersion != systemconfig.InputInventoryFormatVersion {
		t.Fatalf("FormatVersion = %q, want %q", inventory.FormatVersion, systemconfig.InputInventoryFormatVersion)
	}
...
func runEnsureScopeCase(t *testing.T, inputCase systemconfig.InputCase) {
	t.Helper()
...
func assertScopeExpectation(t *testing.T, resolved systemconfig.ResolvedBackendScope, want *systemconfig.ScopeExpectation) {
	t.Helper()
	...
	if want.RequireLocalUUID && !systemconfig.IsLocalBackendScopeID(resolved.BackendScopeID) {
...
func assertPersistedFileExpectation(
	t *testing.T,
	configPath string,
	resolved systemconfig.ResolvedBackendScope,
	want *systemconfig.PersistedFileExpectation,
) {
	t.Helper()
...
func TestMarshalInputInventoryJSON_IsByteIdenticalAcrossRepeatedProjections(t *testing.T) {
	...
	first := systemconfig.ProjectInputInventory()
	second := systemconfig.ProjectInputInventory()

	firstJSON, err := systemconfig.MarshalInputInventoryJSON(first)
	if err != nil {
		t.Fatalf("first MarshalInputInventoryJSON() error = %v", err)
	}
	secondJSON, err := systemconfig.MarshalInputInventoryJSON(second)
	if err != nil {
...
func TestProjectInputInventory_MatchesCommittedBaseline(t *testing.T) {
	inventory := systemconfig.ProjectInputInventory()
	got, err := systemconfig.MarshalInputInventoryJSON(inventory)
	if err != nil {
		t.Fatalf("MarshalInputInventoryJSON() error = %v", err)
	}

	fixturePath := testutil.MustRepoPath(t, systemconfig.InputIndexBaselineRelativePath)
	...
	t.Fatalf(
		...
		systemconfig.InputIndexBaselineRelativePath,
		len(want),
		len(got),
	)
}
