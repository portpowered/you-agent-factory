...
func TestEnsurePackagedFactories_InstallsAssembledScriptsThinExecutableAndPreservesEdits(t *testing.T) {
	...
	root := t.TempDir()
	created, err := ensurePackagedFactories(root, []factorypackages.Definition{definition})
	if err != nil {
		t.Fatalf("ensurePackagedFactories(create): %v", err)
	}
	if len(created) != 1 || created[0].Outcome != PackagedFactoryCreated {
	...
	skipped, err := ensurePackagedFactories(root, []factorypackages.Definition{definition})
	if err != nil {
		t.Fatalf("ensurePackagedFactories(rerun): %v", err)
	}
	if len(skipped) != 1 || skipped[0].Outcome != PackagedFactorySkipped {
