...
// variables, applies flag overrides, and returns resolved defaults.
func ResolveFromHome(homeDir string, flags FlagOverrides) (ResolvedDefaults, error) {
	...
	return Resolve(ResolveInput{
		File: fileDefaults,
		Env: Defaults{
			WorkerModelProvider: strings.TrimSpace(os.Getenv(EnvDefaultWorkerModelProvider)),
			WorkerModel:         strings.TrimSpace(os.Getenv(EnvDefaultWorkerModel)),
		},
		Flag: Defaults{
