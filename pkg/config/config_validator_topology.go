...
func (cv *ConfigValidator) ruleWorkTypeHandlingBehavior(cfg *interfaces.FactoryConfig) []Finding {
	return handlingBehaviorTargetsToFindings(validation.WorkTypeHandlingBehaviorTargets(
		cfg,
		validation.WorkTypeHandlingBehaviorOptions{
			RequireDefault: cv.requireDefaultHandlingWorkType,
		},
	))
}

func handlingBehaviorTargetsToFindings(targets []validation.Target) []Finding {
	if len(targets) == 0 {
		return nil
	}
