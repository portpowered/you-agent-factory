...
func TestWireBuildFactoryService_MatchesInjectFactoryServiceInvalidConfig(t *testing.T) {
	...
	_, errRun := defaultBuildFactoryService(ctx, cfg)

	if errWire == nil {
		t.Fatal("expected InjectFactoryService to fail without factory.json")
	}
	if errRun == nil {
		t.Fatal("expected defaultBuildFactoryService to fail without factory.json")
	}
	if errRun.Error() != errWire.Error() {
		t.Fatalf("defaultBuildFactoryService error = %q, want %q", errRun, errWire)
	}
}
