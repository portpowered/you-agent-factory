...
func assembleMaterializationFixture(t *testing.T) []byte {
  ...
  "workstations": [{"name":"draft","worker":"author","inputs":[{"workType":"task","state":"init"}],"outputs":[{"workType":"task","state":"complete"}],"type":"MODEL_WORKSTATION","promptFile":"prompts/workstation.md"}]
}`)
...
func loadAssembledFixture(t *testing.T, assembled []byte) loadedPromptFixture {
	t.Helper()
	loaded, err := load.LoadFromCanonicalJSON(assembled, load.LoadOptions{})
	if err != nil {
...
func persistAssembledFixture(t *testing.T, assembled []byte) string {
	t.Helper()
	factoryDir, err := persist.PersistNamedFactory(t.TempDir(), "prompt-fixture", assembled)
	if err != nil {
...
func loadMaterializedFixture(t *testing.T, factoryDir string) loadedPromptFixture {
	t.Helper()
	loaded, err := load.LoadFromFactoryDir(factoryDir, nil)
	if err != nil {
