...
func TestSessionOwner_BeforeSubmitRunsAfterNormalizationAndBeforeWorkSubmission(t *testing.T) {
	...
	if _, err := owner.InvokeFactorySession(context.Background(), "session-1", sessionOwnerInvocationRequest(factoryapi.InvocationRequest{Args: &map[string]any{"input": "hello"}})); err != nil {
		t.Fatalf("InvokeFactorySession: %v", err)
	}
