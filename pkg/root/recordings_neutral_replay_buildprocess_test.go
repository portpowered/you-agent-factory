...
type neutralReplayBuildProcessLedger struct {
	recordings.Ledger
}

func TestBuildProcessComposesRecordingsNeutralReplayTypedFailures(t *testing.T) {
	t.Parallel()

	if _, err := BuildProcess(context.Background(), serviceedges.Edges{}); err != nil {
		t.Fatalf("BuildProcess() error = %v", err)
	}

	service := wiredRecordingsServiceForNeutralReplay(t)
	recording := finalizedNeutralReplayRecording(t, service)
	assertNeutralReplayTypedFailures(t, service, recording)
}

func wiredRecordingsServiceForNeutralReplay(t *testing.T) recordings.Service {
	t.Helper()

	return recordingsservice.NewService(
		&neutralReplayBuildProcessLedger{},
...
func finalizedNeutralReplayRecording(
	...
	for index, event := range []recordings.CanonicalEvent{
		rootNeutralReplayEvent("root-neutral-replay-1", 0, scope),
		rootNeutralReplayEvent("root-neutral-replay-2", 1, scope),
	} {
...
func assertNeutralReplayTypedFailures(
	...
	for _, err := range []error{
		...
		assertRootBoundedReplayError(t, err)
	}
}

func assertRootBoundedReplayError(t *testing.T, err error) {
	t.Helper()
	message := err.Error()
	if len(message) > 120 {
