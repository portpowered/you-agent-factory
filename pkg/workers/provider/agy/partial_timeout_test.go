...
func assertPartialTimeoutMessageDraft(t *testing.T, draft responseevents.Draft, wantText string) {
	t.Helper()
...
func filterMessageDrafts(drafts []responseevents.Draft) []responseevents.Draft {
	filtered := make([]responseevents.Draft, 0, len(drafts))
