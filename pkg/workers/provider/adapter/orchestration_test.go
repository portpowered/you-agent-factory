...
func (d *kernelDecoder) Flush(_ context.Context, input adapter.FlushContext) (adapter.DecodeResult, error) {
	...
	payload, err := json.Marshal(responseevents.MessagePayload{Role: "assistant", ContentBlocks: []responseevents.ContentBlock{{Kind: responseevents.ContentBlockText, Text: string(content)}}})
	if err != nil {
	...
	return adapter.DecodeResult{Drafts: []responseevents.Draft{{
		...
		Provenance: responseevents.Provenance{Provider: "opaque-fixture", NativeEventType: string(nativeType), Delivery: responseevents.DeliveryNativeStream, Representation: responseevents.RepresentationSnapshot, Fidelity: responseevents.FidelityNormalized},
		Payload:    payload,
	}}}, nil
}
