...
func TestStartCronIntervalWatcher_SubmitsAtConfiguredInterval(t *testing.T) {
	start := time.Date(2026, time.April, 18, 12, 30, 0, 0, time.UTC)
	fakeClock := clockwork.NewFakeClockAt(start)
	requests := make(chan work.WorkRequest, 4)
	...
	var sidecars sync.WaitGroup
	if err := svc.StartCronIntervalWatcher(runCtx, &sidecars, runtimeCfg, "factory-alpha", func(_ context.Context, request work.WorkRequest) error {
		requests <- request
		return nil
...
func TestStartCronIntervalWatcher_RejectsInvalidDependenciesAndInterval(t *testing.T) {
	...
	var sidecars sync.WaitGroup
	if err := svc.StartCronIntervalWatcher(context.Background(), &sidecars, runtimeCfg, "factory-alpha", func(context.Context, work.WorkRequest) error {
		return nil
