// Package compatibility owns worker/workstation behavior pairing policy.
...
type Workstation struct {
	Name           string
	Type           string
	Kind           workertaxonomy.WorkstationKind
...
func EffectiveWorkstationTypeForCompatibility(workstation Workstation) string {
	...
	if workstation.Kind == workertaxonomy.WorkstationKindPoller {
		return ""
	}
...
func ExpectedWorkerBehaviorClassForWorkstation(workstation Workstation, workerType string) (WorkerWorkstationBehaviorClass, bool) {
	...
	if workertaxonomy.IsPollerRunPublicWorkstationType(workstationType, workstation.Kind) {
		return WorkerWorkstationBehaviorPoller, true
	}

	switch workertaxonomy.PublicWorkstationTypeFromInternalRuntime(workstationType, workerType, workstation.Kind) {
...
func EffectiveWorkstationBehaviorClass(workstationType string, kind workertaxonomy.WorkstationKind, hasWorker bool) string {
	if class := workertaxonomy.ProjectWorkstationBehaviorClass(workstationType, kind); class != "" {
		return class
	}
	if !hasWorker {
		return ""
	}
	if kind == workertaxonomy.WorkstationKindPoller {
		return workertaxonomy.WorkstationTypePoller
	}
	if strings.TrimSpace(workstationType) == "" && (kind == "" || kind == workertaxonomy.WorkstationKindStandard) {
...
func IsLegacyGrandfatheredWorkerWorkstationPair(workerType, workstationType string, kind workertaxonomy.WorkstationKind) bool {
	...
	if wsTrimmed == "" && (kind == "" || kind == workertaxonomy.WorkstationKindStandard) {
		if workertaxonomy.IsInferenceWorkerType(workerType) || workerTrimmed == workertaxonomy.WorkerTypeScript {
			return true
		}
	}
	if !workertaxonomy.IsPollerRunWorkstationType(workstationType, kind) {
		return false
	}
...
func RequiresWorkerWorkstationBehaviorCompatibility(workstationType string, kind workertaxonomy.WorkstationKind, workerTypeName string) bool {
...
func CompatibleWorkerWorkstationBehavior(workerType, workstationType string, kind workertaxonomy.WorkstationKind) bool {
...
func displayWorkstationTaxonomyValue(workstationType string, kind workertaxonomy.WorkstationKind) string {
