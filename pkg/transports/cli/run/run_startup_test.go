...
func TestRun_ShutdownOutputReportsResolvedAutoGeneratedRecordingPath(t *testing.T) {
	...
	var out bytes.Buffer
	err := Run(context.Background(), ensureTestRecordingsCLI(RunConfig{
		StartupOutput: &out,
		HomeDir:       "/tmp",
...
func TestRun_StartupFailureDoesNotReportRecordingSuccess(t *testing.T) {
	originalBuilder := openTestRuntimeRunner
	defer func() {
		openTestRuntimeRunner = originalBuilder
	}()
	...
	openTestRuntimeRunner = func(_ context.Context, _ *testRuntimeSelections, _ serviceedges.Edges) (factoryServiceRunner, error) {
		return stubFactoryService{run: func(context.Context) error { return wantErr }}, nil
	}
	...
	err := Run(context.Background(), ensureTestRecordingsCLI(RunConfig{
		...
		RecordingTargetPlanner: recordings.LiveRecordingTargetPlannerFunc(func(recordings.LiveRecordingTargetRequest) (recordings.LiveRecordingTarget, error) {
			return recordings.LiveRecordingTarget{
				ServicePath:  "/tmp/service-recording.json",
				ReportedPath: "/tmp/reported-recording.json",
			}, nil
		}),
	}))
	...
	if strings.Contains(out.String(), "Recording saved:") {
		t.Fatalf("startup failure output = %q, want no success recording message", out.String())
	}
}
