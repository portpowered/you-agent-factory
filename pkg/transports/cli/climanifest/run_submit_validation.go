...
var runSubmitFamilySpecs = []struct {
...
}{
	{
		...
		flags: []string{
			...
			"server", "skip-permissions", "verbose", "with-mock-workers", "work",
		},
	},
	{
...
func validateRunSubmitCommand(record Command, id, path, operationID string, flagNames []string) error {
	...
	if len(record.Channels.Input) == 0 || len(record.Channels.Output) == 0 || len(record.Outputs) == 0 || len(record.Exits) == 0 || len(record.Constraints.Runtime) == 0 {
		return fmt.Errorf("command %q has incomplete execution metadata", id)
	}
