...
func (f *FactorySessionSSEFixture) RootMockFactory() *testutil.MockFactory {
	return &testutil.MockFactory{
		SessionFactories: map[string]*testutil.MockFactory{
			f.SessionID: f.sessionFactoryForGeneration(
...
func (f *FactorySessionSSEFixture) ReplaceStreamGeneration(
	t *testing.T,
	root *testutil.MockFactory,
) []factoryapi.FactoryEvent {
	t.Helper()
	replacement := factorySessionSSEFixtureReplacementRetainedEvents(t)
	root.SessionFactories[f.SessionID] = f.sessionFactoryForGeneration(
		factorySessionSSEFixtureNextGenerationID,
		replacement,
		make(chan factoryapi.FactoryEvent, 4),
	)
	return replacement
}

func (f *FactorySessionSSEFixture) sessionFactoryForGeneration(
	...
	return &testutil.MockFactory{
		FactoryEventStream: &interfaces.FactoryEventStream{
			...
			History:             append([]factoryapi.FactoryEvent(nil), retained...),
			Events:              live,
		},
	}
}
