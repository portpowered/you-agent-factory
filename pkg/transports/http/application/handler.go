...
func (handler *Handler) Bind(opened factorysessions.RuntimeHTTPServices) (http.Handler, error) {
	...
	if opened.WorkerSessions != nil {
		workerSessionsHandler = workersessionshttp.NewHandler(
			workersessionshttp.NewAdapterWithStartAndContinueAndInterrupt(opened.WorkerSessions, opened.WorkerSessions, opened.WorkerSessions, opened.WorkerSessions, opened.Work),
			opened.Logger,
		)
	}
