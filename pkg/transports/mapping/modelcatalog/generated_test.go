...
func TestDetailToGeneratedPreservesCatalogSemantics(t *testing.T) {
	...
	operation := managedruntime.Operation{
		...
		Outputs: []managedruntime.OperationSlot{{Name: "audio", ContentTypes: []string{"AUDIO"}}},
	}
	detail := modelcatalog.Detail{
		Summary: modelcatalog.Summary{
			Name: "VOICE_MODEL", ProviderLocality: managedruntime.LocalityLocal,
			Status: modelcatalog.StatusReady, LoadState: modelcatalog.LoadStateUnloaded,
			Operations: []managedruntime.Operation{operation}, Modalities: []string{"AUDIO", "TEXT"},
			...
			ManagedRuntime: managedruntime.Runtime{
				Identity: "VOICE_MODEL", Locality: managedruntime.LocalityLocal,
				ReadinessState:      managedruntime.ReadinessStateReady,
				LifecycleState:      managedruntime.LifecycleStateInstalled,
				SupportedOperations: []managedruntime.Operation{operation},
				Diagnostics:         map[string]string{"revision": "rev-1"},
			},
		},
		Capabilities: []modelcatalog.Capability{{
			Worker: "voice-worker", ProviderLocality: managedruntime.LocalityLocal,
			ModelProvider: &provider, Operations: []managedruntime.Operation{operation},
