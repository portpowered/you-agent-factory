...
func TestFactoryConfigFromOpenAPIJSON_MapsNonClassifierWithoutOutputs(t *testing.T) {
	cfgJSON := []byte(`{
	...
	if len(workstation.Outputs) != 0 || len(workstation.OnFailure) != 1 {
		t.Fatalf("mapped workstation routing = %#v, want empty outputs and one onFailure route", workstation)
	}
}

func TestFactoryConfigFromOpenAPIJSON_MapsMissingOnFailureWhenSuccessRoutingIsExplicit(t *testing.T) {
	cfgJSON := []byte(`{
	...
	workstation := cfg.Workstations[0]
	if len(workstation.Outputs) != 1 || len(workstation.OnFailure) != 0 {
		t.Fatalf("mapped workstation routing = %#v, want one output and no onFailure route", workstation)
	}
}

func TestFactoryConfigFromOpenAPIJSON_MapsNonClassifierClassificationRoutes(t *testing.T) {
	cfgJSON := []byte(`{
	...
	workstation := cfg.Workstations[0]
	if len(workstation.ClassificationRoutes) != 1 ||
		workstation.ClassificationRoutes[0].Label != "approved" {
		t.Fatalf("mapped classification routes = %#v, want explicit approved route", workstation.ClassificationRoutes)
	}
}
