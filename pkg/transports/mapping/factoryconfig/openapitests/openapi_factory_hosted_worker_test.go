...
func assertRuntimeHostedLinearWorker(t *testing.T, runtimeWorker interfaces.FactoryWorkerConfig) {
	t.Helper()
...
func TestFactoryConfigFromOpenAPIJSON_PreservesHostedLinearWorkerMissingMapping(t *testing.T) {
	cfgJSON := []byte(`{
	...
	if worker.Linear == nil {
		t.Fatal("mapped hosted Linear config is nil, want explicit empty config")
	}
	if worker.Linear.Mapping.WorkType != "" || worker.Linear.Mapping.State != "" {
		t.Fatalf("mapped hosted Linear mapping = %#v, want empty work type and state", worker.Linear.Mapping)
	}
}
