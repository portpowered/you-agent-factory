...
func TestFactoryConfigFromOpenAPIJSON_PreservesMalformedWorkPropagationObject(t *testing.T) {
	cfgJSON := []byte(`{
...
func TestFactoryConfigFromOpenAPIJSON_PreservesUnsupportedWorkPropagationMode(t *testing.T) {
	cfgJSON := []byte(`{
	...
	if cfg.Workstations[0].WorkPropagation == nil ||
		cfg.Workstations[0].WorkPropagation.Mode != interfaces.WorkPropagationMode("preserve_input") {
		t.Fatalf("WorkPropagation = %#v, want authored unsupported mode preserved for owner validation", cfg.Workstations[0].WorkPropagation)
	}
}
