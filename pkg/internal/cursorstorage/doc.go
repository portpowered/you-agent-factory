// Package cursorstorage discovers and parses Cursor cursor-agent CLI session storage in-process.
//
// v1 required backend: cursor-agent CLI storage under ~/.config/cursor/chats or ~/.cursor/chats
// (Linux). Cursor desktop globalStorage is documented as optional/future and is not required for v1.
//
