...
func TestRunChecksRepositoryRequiredScenarioManifest(t *testing.T) {
	...
	if got := stdout.String(); got != "[agent-factory:required-functional] 2 required short customer-boundary scenario(s) are current; 2 reviewed non-required SSE disposition(s) are explicit; the full functional tree is boundary-enforced; 35 unchanged legacy file(s) remain quarantined by the reviewed migration baseline\n" {
