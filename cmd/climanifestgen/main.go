...
const successMessage = "[agent-factory:cli-manifest-generate] representative- and work-family CLI metadata generated"
...
func run(root string, check bool, stdout, stderr io.Writer) int {
	if check {
		...
		if drift.Empty() {
			fmt.Fprintln(stdout, "[agent-factory:cli-manifest-check] representative- and work-family CLI metadata is current")
			return 0
		}
...
func writeDrift(stderr io.Writer, drift climanifestgen.Drift) {
	fmt.Fprintln(stderr, "[agent-factory:cli-manifest-check] representative- and work-family CLI metadata drift detected")
