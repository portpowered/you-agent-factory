...
const successMessage = "[agent-factory:cli-manifest-generate] CLI metadata generated"
...
func run(root string, check bool, stdout, stderr io.Writer) int {
	if check {
		...
		if drift.Empty() {
			fmt.Fprintln(stdout, "[agent-factory:cli-manifest-check] CLI metadata is current")
			return 0
		}
...
func writeDrift(stderr io.Writer, drift climanifestgen.Drift) {
	fmt.Fprintln(stderr, "[agent-factory:cli-manifest-check] CLI metadata drift detected")
