...
func parseConfig() config {
	var cfg config
	flag.StringVar(&cfg.goBinary, "go", "go", "Go executable used to run gocoveragecheck")
	flag.StringVar(&cfg.repositoryRoot, "root", ".", "repository root")
	flag.Float64Var(&cfg.minimumCoverage, "minimum", 75.9, "minimum aggregate unit coverage")
	...
	flag.StringVar(&cfg.logPath, "log", "", "complete command log path")
	flag.Parse()
	return cfg
}
...
func run(cfg config, stdout io.Writer) error {
	...
	for _, path := range []string{cfg.logPath, cfg.profilePath, cfg.coveragePath, cfg.timingPath} {
	...
	if errors.As(runErr, &commandExit) {
	...
	return fmt.Errorf("run unit coverage: %w", runErr)
}
