...
func TestRunWritesFunctionalTimingSummaryOnSuccess(t *testing.T) {
	...
	if slicesContains(gotArgs, "-count=1") {
		t.Fatalf("coverage go test args = %v, want Go default count", gotArgs)
	}
	if !slicesContains(gotArgs, "-parallel=2") {
		t.Fatalf("coverage go test args = %v, want bounded functional in-package parallelism", gotArgs)
	}
