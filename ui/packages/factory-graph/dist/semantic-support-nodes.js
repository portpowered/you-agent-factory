...
export function FactoryGraphWorkTypeNodeView({ data, selected: reactFlowSelected, }) {
    ...
    const content = (_jsxs("span", { "aria-hidden": selectable ? true : undefined, className: "flex min-w-0 items-center gap-1.5 overflow-hidden", "data-work-type-label-zone": true, ...(selectable ? {} : { "aria-label": label, role: "img" }), title: data.validationMessage ?? label, children: [selectable ? null : _jsx("span", { className: "sr-only", children: label }), _jsx(GraphSemanticIcon, { className: classNames("h-3.5 w-3.5 shrink-0", factoryGraphNodeVisualIconClassName(visualState, "text-info")), kind: "work-type", label: workTypeLabel }), _jsxs("span", { className: "grid min-w-0 gap-px overflow-hidden", children: [_jsxs("span", { className: "flex min-w-0 items-start gap-1 overflow-hidden", children: [_jsx("span", { className: factoryGraphNodeWrappedTextClassName("block overflow-hidden text-[0.62rem] font-bold uppercase leading-none text-info"), children: workTypeLabel }), data.isDefaultWorkType ? (_jsx(FactoryGraphNodeBadge, { className: "max-w-full shrink", role: "status", tone: "info", weight: "label", children: defaultWorkTypeLabel(data.locale) })) : null] }), _jsx("strong", { className: factoryGraphNodeWrappedTextClassName("block font-mono text-[0.8rem] font-bold leading-tight text-on-surface"), children: name })] })] }));
...
function FactoryGraphResourceNodeContent({ label, locale, place, resourceLabel, tokenCount, visualState, }) {
