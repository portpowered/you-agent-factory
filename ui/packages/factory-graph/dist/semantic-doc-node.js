...
export function FactoryGraphDocNodeView({ data, selected: reactFlowSelected, }) {
    ...
    return (_jsx(FactoryGraphNodeShell, { className: [
            ...
            }, children: _jsx(FactoryGraphDocNodeContent, { displayLabel: data.displayLabel, docLabel: docLabel, expanded: isExpanded, fileType: data.fileType, targetPath: data.targetPath, visualState: visualState }) })) : (_jsx(FactoryGraphDocNodeContent, { displayLabel: data.displayLabel, docLabel: docLabel, expanded: isExpanded, fileType: data.fileType, targetPath: data.targetPath, visualState: visualState })) }));
}
function FactoryGraphDocNodeContent({ displayLabel, docLabel, expanded, fileType, targetPath, visualState, }) {
    return (_jsxs("div", { className: "grid min-w-0 gap-1 py-1", children: [_jsxs("div", { className: "flex min-w-0 items-center gap-1.5", children: [_jsx("span", { "data-factory-entity-semantic-icon": true, children: _jsx(GraphSemanticIcon, { className: factoryGraphNodeVisualIconClassName(visualState, "text-on-surface-variant"), kind: "doc", label: docLabel }) }), _jsx("span", { className: factoryGraphNodeWrappedTextClassName("block text-sm font-medium text-on-surface"), "data-factory-entity-title": true, children: displayLabel })] }), _jsx("span", { className: factoryGraphNodeWrappedTextClassName("block text-xs text-on-surface-variant"), children: targetPath }), expanded ? (_jsx(FactoryGraphNodeExpandedContent, { family: "doc", children: _jsx("span", { "data-factory-graph-expanded-field": "file-type", children: fileType ?? docLabel }) })) : null] }));
}
