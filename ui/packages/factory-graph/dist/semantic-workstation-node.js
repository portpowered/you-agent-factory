...
export function FactoryGraphWorkstationNodeView({ data, }) {
    ...
    const className = classNames(factoryGraphNodeSurfaceClassName("workstation"), "min-w-0 w-full justify-start overflow-hidden border-2", factoryGraphNodeHoverClassName({ muted: data.muted, selected: data.selectedWorkstation }, "primary"), "border-info-border", presentation.borderClassName, data.active &&
        !data.selectedWorkstation &&
        "border-af-success-border shadow-af-success-chip", data.activeFlow &&
        !data.selectedWorkstation &&
        "agent-flow-node--active ring-2 ring-af-success-border", data.selectedWorkstation && "border-primary shadow-af-accent-selected", data.selectedWorkID !== null &&
...
function Summary({ data, presentation, title, }) {
    return (_jsxs("div", { className: "grid min-w-0 gap-1", "data-workstation-control-role": presentation.controlRole, "data-workstation-runtime-type": presentation.runtimeType, "data-workstation-scheduling-behavior": presentation.schedulingBehavior, children: [_jsx(GraphNodeButton, { "aria-label": data.onSelectWorkstation
                    ...
                    : undefined, "aria-pressed": data.onSelectWorkstation ? data.selectedWorkstation : undefined, className: "flex min-w-0 w-full items-center justify-between gap-2 overflow-hidden", "data-selected-workstation": data.selectedWorkstation ? "true" : undefined, disabled: data.onSelectWorkstation === undefined, onClick: data.onSelectWorkstation
                    ? (event) => {
...
function ActiveContent({ data, entries, presentation, title, }) {
    ...
    return (_jsxs("div", { className: "grid h-full min-w-0 grid-rows-[auto_auto_1fr_auto]", "data-active": data.active ? "true" : undefined, "data-selected-work": data.selectedWorkID !== null ? "true" : undefined, "data-selected-workstation": data.selectedWorkstation ? "true" : undefined, "data-workstation-control-role": presentation.controlRole, "data-workstation-runtime-type": presentation.runtimeType, "data-workstation-scheduling-behavior": presentation.schedulingBehavior, children: [data.onSelectWorkstation ? (_jsx(GraphNodeButton, { "aria-label": selectWorkstationLabel(title, data.locale), "aria-pressed": data.selectedWorkstation, className: "flex min-w-0 w-full items-center justify-between gap-2 overflow-hidden", onClick: (event) => {
                    event.stopPropagation();
                    data.onSelectWorkstation?.(data.workstation.node_id);
                }, title: title, children: header })) : (_jsx("div", { className: "flex min-w-0 w-full items-center justify-between gap-2 overflow-hidden", title: title, children: header })), _jsx(FactoryGraphWorkstationGuardedControlCard, { locale: data.locale, presentation: presentation }), _jsx("ul", { className: "mt-2 grid min-w-0 list-none content-start gap-1 p-0", children: visible.map(({ execution, workItem }) => (_jsx(WorkItem, { data: data, execution: execution, workItem: workItem }, `${execution.dispatch_id}:${workItem.work_id}`))) }), _jsx(Overflow, { total: entries.length, visible: visible.length, locale: data.locale })] }));
}
...
function Header({ compact = false, presentation, title, }) {
    return (_jsxs(_Fragment, { children: [_jsx("span", { className: compact
                    ...
                    : workstationTitleClassName(title), "data-workstation-title": true, children: title }), _jsx("span", { className: "min-w-0 truncate text-[0.62rem] font-semibold leading-tight text-on-surface-subtle", "data-workstation-runtime-label": true, title: presentation.label, children: presentation.label }), presentation.schedulingLabel ? (_jsx("span", { className: "min-w-0 shrink-0 truncate rounded-sm border border-outline-variant bg-surface px-1.5 py-0.5 text-[0.62rem] font-semibold leading-none text-on-surface-subtle", "data-workstation-scheduling-label": true, title: presentation.schedulingLabel, children: presentation.schedulingLabel })) : null] }));
}
