...
import { verifyBentoCardCatalogResponsive } from "./verify-bento-card-catalog-storybook-responsive.mjs";

const uiRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const host = process.env.AGENT_FACTORY_STORYBOOK_HOST ?? "127.0.0.1";
const port = Number(process.env.AGENT_FACTORY_STORYBOOK_PORT ?? "3828");
...
const storyChecks = [
  {
    ...
    verify: verifyBentoCardCatalogResponsive,
    viewports: [
...
async function main() {
  ...
  try {
    ...
    for (const storyCheck of storyChecks) {
      for (const viewport of storyCheck.viewports) {
