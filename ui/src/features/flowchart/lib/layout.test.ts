// biome-ignore-all lint/complexity/noExcessiveLinesPerFunction: existing flowchart layout coverage stayed intact during feature-root migration.
...
describe("buildGraphLayout", () => {
  ...
  it("keeps constraint and limit places as first-class graph nodes", async () => {
    ...
    expect(
      layout.nodes.find((node) => node.nodeId === "place:quality-gate:ready"),
    ).toMatchObject({
      height: 58,
      nodeKind: "constraint",
      width: 156,
    ...
    expect(
      layout.nodes.find((node) => node.nodeId === "place:visit-limit:open"),
    ).toMatchObject({
      height: 58,
      nodeKind: "constraint",
      width: 156,
