...
describe("getWorkOutcomeMessages", () => {
  it("returns English chart and trend labels by default", () => {
    ...
    expect(messages.chart.workTypeFailureLabel("story")).toBe("Work type: story");
    ...
    expect(messages.trends.reworkPointLabel("Review", 2)).toBe(
      "Review: 2 retry or rework events",
    );
  });
  ...
  it("returns zh-CN chart and trend labels for the localized dashboard path", () => {
    ...
    expect(messages.chart.workTypeFailureLabel("story")).toBe("工作类型：story");
