...
const hostedTopologyReplayMessagesByLocale = {
  en: {
    ...
    topology: {
      ...
      activeWorkRows: (count) =>
        count === 1 ? "1 active Work" : `${count} active Work`,
      ...
      legendLabel: "Factory topology legend",
  ...
  "zh-CN": {
    ...
    topology: {
      ...
      activeWorkDuration: (ticks) => `${ticks} 个逻辑时刻`,
      activeWorkOverflow: (count) => `还有 ${count} 个活动工作`,
      activeWorkRows: (count) => `${count} 个活动工作`,
      empty: "此时刻没有可用的工厂拓扑。",
      failed: "无法显示工厂拓扑。",
      hideNodeKinds: "隐藏节点类型",
