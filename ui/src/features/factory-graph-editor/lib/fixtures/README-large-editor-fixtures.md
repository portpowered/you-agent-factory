...
- save-related layout recomputation: 300 ms median for dirty checks plus pending layout application

The 1000 node stress fixture uses a 120000 ms projection budget, a 600 ms save-layout recomputation budget, and looser drag thresholds so severe regressions are caught without blocking ordinary CI on the same thresholds as the 500 node case.
