...
describe("useEditableFactoryGraph document plane projection", () => {
  ...
  it("undoes and redoes mixed topology, field, and layout transactions in LIFO order", () => {
    const { result } = renderEditableFactoryGraphHook({
      currentFactoryDocument: documentFactory,
    });
    ...
    expect(result.current.pendingState.pendingFactoryDefinition).toMatchObject(
      {
        ...
        workstations: [
          {
            body: "Updated document instructions.",
            name: "document-only",
          },
        ],
      },
    ...
    expect(
      result.current.layoutDraftState.layout.nodes?.find(
    ...
    expect(
      result.current.pendingState.pendingFactoryDefinition?.workstations?.[0]
        ?.body,
    ).toBe("Document plane baseline.");
    expect(result.current.pendingState.pendingFactoryDefinition?.resources).toEqual(
      [{ capacity: 2, name: "cache" }],
    );
    ...
    expect(result.current.pendingState.pendingFactoryDefinition).toMatchObject({
      resources: [],
      workstations: [
        { body: "Document plane baseline.", name: "document-only" },
      ],
    });
