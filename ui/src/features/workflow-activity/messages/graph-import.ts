...
const workflowActivityGraphImportMessagesByLocale = {
  en: {
    ...
    importErrorNotPngFile: "Drop a PNG image exported by you-agent-factory.",
    ...
    importErrorUnsupportedSchemaVersion: (schemaVersion) =>
      schemaVersion
        ? `This PNG uses unsupported you-agent-factory factory metadata version ${schemaVersion}.`
  ...
  ja: {
    ...
    dismissAction: "閉じる",
    graphDropHint:
      "you-agent-factory の PNG をこのグラフにドロップしてインポートを開始してください。",
    graphDropReadingMessage: (fileName) =>
    ...
    importErrorEmbeddedMetadataInvalid:
      "埋め込まれた you-agent-factory ファクトリーメタデータが無効なため、現在のファクトリーは変更されませんでした。",
    importErrorFileReadFailed:
      "ブラウザがドロップしたファイルを読み取れませんでした。もう一度 PNG をドロップしてください。",
    importErrorMetadataMissing:
      "この PNG にはインポートに必要な you-agent-factory ファクトリーメタデータが含まれていません。",
    importErrorNotPngFile:
      "you-agent-factory から書き出した PNG 画像をドロップしてください。",
    importErrorPngInvalid:
    ...
    importErrorUnsupportedSchemaVersion: (schemaVersion) =>
      schemaVersion
        ? `この PNG は未対応の you-agent-factory ファクトリーメタデータバージョン ${schemaVersion} を使用しています。`
        : "この PNG は未対応の you-agent-factory ファクトリーメタデータバージョンを使用しています。",
  },
  ko: {
    ...
    importErrorEmbeddedMetadataInvalid:
      "내장된 you-agent-factory 팩토리 메타데이터가 잘못되어 현재 팩토리는 변경되지 않았습니다.",
    importErrorFileReadFailed:
    ...
    importErrorUnsupportedSchemaVersion: (schemaVersion) =>
      schemaVersion
        ? `이 PNG는 지원되지 않는 you-agent-factory 팩토리 메타데이터 버전 ${schemaVersion}을 사용합니다.`
        : "이 PNG는 지원되지 않는 you-agent-factory 팩토리 메타데이터 버전을 사용합니다.",
  },
  "zh-CN": {
    dialogCloseLabel: "关闭对话框",
    dialogFlowLabel: "变更流程",
    dismissAction: "关闭",
    graphDropHint: "将 you-agent-factory PNG 拖放到此图上即可开始导入。",
    graphDropReadingMessage: (fileName) =>
    ...
    importErrorEmbeddedMetadataInvalid:
      "嵌入的 you-agent-factory 工厂元数据无效，因此当前工厂未被更改。",
    importErrorFileReadFailed: "浏览器无法读取拖入的文件。请再次拖入该 PNG。",
    importErrorMetadataMissing:
      "此 PNG 不包含导入所需的 you-agent-factory 工厂元数据。",
    importErrorNotPngFile: "请拖入从 you-agent-factory 导出的 PNG 图片。",
    importErrorPngInvalid:
    ...
    importErrorUnsupportedSchemaVersion: (schemaVersion) =>
      schemaVersion
        ? `此 PNG 使用了不受支持的 you-agent-factory 工厂元数据版本 ${schemaVersion}。`
        : "此 PNG 使用了不受支持的 you-agent-factory 工厂元数据版本。",
  },
