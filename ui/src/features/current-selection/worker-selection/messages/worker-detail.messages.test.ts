...
describe("getWorkerDetailMessages", () => {
  it.each(locales)(
    ...
    (locale) => {
      ...
      expect(messages.editableConfigurationValidationStatus.length).toBeGreaterThan(
        0,
      );
      ...
      expect(
        messages.editableConfigurationNameRequired.length,
      ).toBeGreaterThan(0);
      expect(
      ...
      expect(messages.localizeWorkerType("MODEL_WORKER").length).toBeGreaterThan(
        0,
      );
      expect(messages.localizeModelProvider("CURSOR").length).toBeGreaterThan(0);
