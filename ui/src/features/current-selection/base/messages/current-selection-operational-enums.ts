...
function formatRawOutcomeLabel(rawOutcome: string): string {
  return `Raw outcome: ${rawOutcome}`;
}

function isRepeaterRejectedOutcome(
  value: string,
  workstationKind?: string,
): boolean {
  return (
...
function localizeWorkstationRunOutcome(
  value: string,
  workstationKind: string | undefined,
  localizeOutcome: (value: string) => string,
  repeatedWorkLabel: string,
): WorkstationRunOutcomeDisplay {
  if (isRepeaterRejectedOutcome(value, workstationKind)) {
    return {
      label: repeatedWorkLabel,
      rawOutcomeLabel: formatRawOutcomeLabel(value.trim()),
    };
  }

  return {
    label: localizeOutcome(value),
  };
}

const localizeEnglishOutcome = (value: string) =>
  localizeEnumLabel({
    category: "outcome",
    labels: {
      ACCEPTED: "Accepted",
...
const localizeJapaneseOutcome = (value: string) =>
  localizeEnumLabel({
    category: "outcome",
    labels: {
      ACCEPTED: "受理済み",
...
const localizeKoreanOutcome = (value: string) =>
  localizeEnumLabel({
    category: "outcome",
    labels: {
      ACCEPTED: "수락됨",
...
const localizeSimplifiedChineseOutcome = (value: string) =>
  localizeEnumLabel({
    category: "outcome",
    labels: {
      ACCEPTED: "已接受",
...
const currentSelectionOperationalEnumMessagesByLocale = {
  en: {
    localizeOutcome: localizeEnglishOutcome,
    localizeWorkstationRunOutcome: (value, workstationKind) =>
      localizeWorkstationRunOutcome(
        value,
        workstationKind,
        localizeEnglishOutcome,
        "Repeated work",
      ),
  ...
  ja: {
    localizeOutcome: localizeJapaneseOutcome,
    localizeWorkstationRunOutcome: (value, workstationKind) =>
      localizeWorkstationRunOutcome(
        value,
        workstationKind,
        localizeJapaneseOutcome,
        "繰り返し作業",
      ),
    localizeRelationState: (value: string) =>
      localizeEnumLabel({
  ...
  ko: {
    localizeOutcome: localizeKoreanOutcome,
    localizeWorkstationRunOutcome: (value, workstationKind) =>
      localizeWorkstationRunOutcome(
        value,
        workstationKind,
        localizeKoreanOutcome,
        "반복 작업",
      ),
    localizeRelationState: (value: string) =>
      localizeEnumLabel({
  ...
  "zh-CN": {
    localizeOutcome: localizeSimplifiedChineseOutcome,
    localizeWorkstationRunOutcome: (value, workstationKind) =>
      localizeWorkstationRunOutcome(
        value,
        workstationKind,
        localizeSimplifiedChineseOutcome,
        "重复工作",
      ),
    localizeRelationState: (value: string) =>
      localizeEnumLabel({
