...
import { factoryGraphNodeIdForWorkState } from "../../../factory-graph-editor/lib/factory-validation-graph-projection";
...
import type { WorkStateDetailMessages } from "../messages/work-state-detail-types";

export function WorkStateTopologyDeleteSection({
  messages,
  placeId,
  stateName,
  workTypeName,
}: {
  messages: WorkStateDetailMessages;
  placeId: string;
  stateName: string;
  workTypeName: string;
}) {
  ...
  const nodeId = factoryGraphNodeIdForWorkState(placeId);
  const blockedReason =
    topologyEditor.blockedRemovalReason &&
    topologyEditor.blockedRemovalReason.length > 0
      ? topologyEditor.blockedRemovalReason
      : null;
  const sectionId = `work-state-topology-delete-${placeId}`;
      ...
      <DashboardActionRow
        actions={
          <DashboardActionButton
            aria-label={messages.topologyDeleteAction(workTypeName, stateName)}
            onClick={() => {
