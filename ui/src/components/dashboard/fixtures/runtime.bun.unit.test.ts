...
describe("dashboard runtime fixtures", () => {
  it("applies active work overlays without mutating the base topology", () => {
    const originalTopologyJSON = JSON.stringify(
      mediumBranchingDashboardTopology,
    );
    ...
    expect(JSON.stringify(mediumBranchingDashboardTopology)).toBe(
      originalTopologyJSON,
    );
    ...
    expect(
      snapshot.runtime.current_work_items_by_place_id?.["story:implemented"],
    ).toEqual([expect.objectContaining({ work_id: "work-active-story" })]);
    expect(
      snapshot.runtime.session.provider_sessions?.map(
  ...
  it("builds retry, failure, and rejected snapshots with observable session outcomes", () => {
    const retrySnapshot = buildDashboardSnapshotFixture(
      mediumBranchingDashboardTopology,
      [retryAttemptRuntimeOverlay],
    );
    const failedSnapshot = buildDashboardSnapshotFixture(
      mediumBranchingDashboardTopology,
      [failedOutcomeRuntimeOverlay],
    );
    const rejectedSnapshot = buildDashboardSnapshotFixture(
      mediumBranchingDashboardTopology,
      [rejectedOutcomeRuntimeOverlay],
    );

    expect(
      retrySnapshot.runtime.session.provider_sessions?.map(
        (attempt) => attempt.outcome,
      ),
    ).toContain("RETRY");
    expect(failedSnapshot.runtime.session.failed_count).toBe(1);
    expect(failedSnapshot.runtime.session.failed_by_work_type).toEqual({
      story: 1,
    });
    expect(failedSnapshot.runtime.session.failed_work_labels).toContain(
      "Failed Story",
    );
    expect(
      failedSnapshot.runtime.session.provider_sessions?.map(
        (attempt) => attempt.outcome,
      ),
    ).toContain("FAILED");
    expect(
      rejectedSnapshot.runtime.session.provider_sessions?.map(
        (attempt) => attempt.outcome,
      ),
    ).toContain("REJECTED");
  });
  ...
  it("composes semantic overlays against one shared topology", () => {
    ...
    expect(snapshot.runtime.active_dispatch_ids).toContain(
      "dispatch-review-active",
    );
    expect(
      snapshot.runtime.session.provider_sessions?.map(
