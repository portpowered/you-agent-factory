...
function workItemWithMultimodalPayload(
...
function appendProviderSession(
  ...
  return {
    ...
    session: {
      ...runtime.session,
      provider_sessions: [
        ...(runtime.session.provider_sessions ?? []),
        attempt,
      ],
    },
  };
...
export function factoryFromDashboardTopology(
  ...
  function appendWorkState(
    ...
    if (place.kind !== "work_state" || !place.type_id || !place.state_value) {
      return;
    }

    const workType: NonNullable<
      CanonicalFactoryDefinition["workTypes"]
    >[number] = workTypes.get(place.type_id) ?? {
      name: place.type_id,
      states: [],
    };
...
export const activeWorkRuntimeOverlay: DashboardRuntimeOverlay = (
  runtime,
  topology,
) => {
  ...
  return appendProviderSession(
    {
      ...
      active_workstation_node_ids: [
        ...(runtime.active_workstation_node_ids ?? []),
        nodeID,
      ],
      ...
      place_token_counts: {
        ...
        "story:implemented":
          (runtime.place_token_counts?.["story:implemented"] ?? 0) + 1,
      },
  ...
  (runtime, topology) => {
    ...
    return {
      ...
      active_executions_by_dispatch_id: Object.fromEntries(
        Object.entries(
          enrichedRuntime.active_executions_by_dispatch_id ?? {},
      ...
      current_work_items_by_place_id: Object.fromEntries(
        Object.entries(
          enrichedRuntime.current_work_items_by_place_id ?? {},
        ).map(([placeID, workItems]) => [
          placeID,
          enrichedWorkItems(workItems),
        ]),
      ),
      session: {
        ...enrichedRuntime.session,
        provider_sessions: enrichedRuntime.session.provider_sessions?.map(
          (attempt) => ({
            ...attempt,
      ...
      workstation_activity_by_node_id: Object.fromEntries(
        Object.entries(
          enrichedRuntime.workstation_activity_by_node_id ?? {},
        ).map(([nodeID, activity]) => [
          nodeID,
          {
            ...activity,
...
export const retryAttemptRuntimeOverlay: DashboardRuntimeOverlay = (
  runtime,
  topology,
) => {
...
export const failedOutcomeRuntimeOverlay: DashboardRuntimeOverlay = (
  runtime,
  topology,
) => {
  ...
  return appendProviderSession(
    {
      ...
      session: {
        ...
        failed_work_labels: [
          ...(runtime.session.failed_work_labels ?? []),
          "Failed Story",
        ],
      },
    },
...
export const rejectedOutcomeRuntimeOverlay: DashboardRuntimeOverlay = (
  runtime,
  topology,
) => {
  const nodeID = firstExistingNodeID(topology, "review");
...
export const dashboardSemanticSnapshotFixtures = {
  ...
  retryAttempt: buildDashboardSnapshotFixture(
    mediumBranchingDashboardTopology,
    [retryAttemptRuntimeOverlay],
  ),
  failedOutcome: buildDashboardSnapshotFixture(
    mediumBranchingDashboardTopology,
    [failedOutcomeRuntimeOverlay],
  ),
  rejectedOutcome: buildDashboardSnapshotFixture(
    mediumBranchingDashboardTopology,
