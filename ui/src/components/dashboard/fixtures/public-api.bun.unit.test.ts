...
import { scriptDashboardIntegrationBackendWorkstationRequestsByDispatchID } from "./script-dashboard-integration-backend-world-view";
import {
...
describe("dashboard fixture catalog", () => {
  it("exports documented fixture catalog entries for direct Storybook and Vitest imports", () => {
    const activeSnapshot = buildDashboardSnapshotFixture(
      mediumBranchingDashboardTopology,
      [activeWorkRuntimeOverlay],
    );

    expect(dashboardTopologyFixtures.oneNode).toBe(oneNodeDashboardTopology);
    expect(dashboardTopologyFixtures.mediumBranching).toBe(
      mediumBranchingDashboardTopology,
    );
    ...
    expect(
      dashboardSemanticSnapshotFixtures.activeWork.runtime
    ...
    expect(
      scriptDashboardIntegrationTimelineEvents.map((event) => event.type),
    ).toContain("SCRIPT_REQUEST");
    expect(
      scriptDashboardIntegrationTimelineEvents.map((event) => event.type),
    ).toContain("SCRIPT_RESPONSE");
    expect(
    ...
    expect(dashboardWorkstationRequestFixtures.rejected.outcome).toBe(
      "REJECTED",
    );
  });
});
