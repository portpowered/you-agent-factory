export const currentFactoryDefinitionAPIErrorMessages = {
  invalidResponse:
    "The current factory editing API returned an invalid response.",
  network: "The dashboard could not reach the current factory editing API.",
  rejectedRequest: "The current factory editing API rejected the request.",
  rejectedSaveRequest:
    "The current factory editing API rejected the save request.",
  unavailableInEnvironment:
    "Current factory editing is unavailable in this environment.",
} as const;
