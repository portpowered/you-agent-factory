...
describe("factory API", () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        body: JSON.stringify({
          ...
          workers: [],
          workstations: [],
          version: incrementSessionFactoryVersion(defaultSessionFactoryVersion),
        }),
        headers: {
          "content-type": "application/json",
        },
  ...
  it("maps FACTORY_NOT_IDLE session save failures into named factory activation errors", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        mockPutSessionFactory({ mode: "factory_not_idle" }),
      );
  ...
  it("maps STALE_FACTORY_VERSION session save failures into named factory activation errors", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(mockPutSessionFactory({ mode: "stale_version" }));
  ...
  it("rejects retired named-factory wrapper responses from the current factory endpoint", async () => {
    await expect(
      ...
      new NamedFactoryAPIError(
        "The current factory API returned an invalid response.",
        {
          code: "INTERNAL_ERROR",
          responseBody: {
            factory: {
