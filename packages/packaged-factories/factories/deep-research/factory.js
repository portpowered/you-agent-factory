...
return (async function () {
  ...
  if (needsSpecialists && maxSubagents > 0) {
    ...
    for (let index = 0; index < specialistFindings.length; index += 1) {
      if (specialistFindings[index].status !== "COMPLETED") {
        ...
        specialistFindings[index] = await agent.run({
          ...
          prompt: retryRequest.prompt + "\n\nThis is a bounded retry after the first provider attempt failed. Return the JSON object with the standalone evidence summary in its evidence field directly.",
    ...
    for (let index = 0; index < specialistFindings.length; index += 1) {
      if (specialistFindings[index].status === "COMPLETED" && !specialistFindings[index].schemaValidated) {
        throw "research specialist " + (index + 1) + " returned an unvalidated result";
      }
    }
  }
  ...
  const specialistEvidence = specialistFindings.length === 0
    ...
    : specialistFindings.map(function (finding, index) {
        ...
        if (finding.status === "COMPLETED") {
          const evidence = finding.output.evidence;
          if (typeof evidence !== "string" || evidence.trim() === "") {
            return "[" + role + " specialist unavailable]\nempty validated specialist evidence";
          }
  ...
  return {
    ...
    synthesis: {
      leadResult: { text: leadSynthesis.output.answer.trim() },
