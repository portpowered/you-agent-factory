...
| `@you/full-flow` | TBD | TBD | deterministic full-flow invocation tests and cycle-route script coverage | NEEDS_ITERATION | Caller-selected bounds are not independently enforced below fixed topology ceilings. Live validation stopped by user direction, so the current artifact has no accepted end-to-end recording. |
...
| `@you/goal` | `BOOT-GOAL-003-R01` | `BOOT-GOAL-004-R01` | deterministic completion, continue, rejection, exact visit-exhaustion, context-preservation, and final-marker coverage; full short Go suite | MEETS_EXPECTATIONS | The canary ran exactly two passes, preserved the request and prior output separately, and stopped after verified completion. The representative completed a scoped Provider Sessions cleanup with independently passing behavior and structure conditions. Repository-wide structure/boundary gates retain unrelated baseline failures. |
| `@you/loop` | `BOOT-LOOP-002-R01` | `BOOT-LOOP-002-R01` | deterministic start-trigger, duration validation, overlap suppression, cancellation, and consecutive-failure-ceiling coverage | MEETS_EXPECTATIONS | Four live occurrences retained the request, ran on exact 20-second nominal intervals with sub-millisecond scheduling drift, and stopped producing events after cancellation. Response-stream output is unsupported for continuous invocation, so the accepted run used quiet output plus replay evidence. |
| `@you/plan-execute` | `BOOT-PLAN-EXECUTE-001-R01` | `BOOT-PLAN-EXECUTE-002-R02` | deterministic planner/executor ordering, operator-default selection, durable PRD handoff, exact completion-token regression, and independently corrected full functional verification | MEETS_EXPECTATIONS | The fixed live Factory wrote agreeing seven-story Markdown and JSON PRDs, handed them to a fresh Cursor Composer executor, accepted exact completion, and completed after three green functional runs. Independent review retained its performance work but rejected its JSON-string public-contract decision and restored intentional plain-text projection. |
| `@you/plan-parallel` | `BOOT-PLAN-PARALLEL-001` (passed twice) | `BOOT-PLAN-PARALLEL-006-R01` | deterministic DAG, assigned-payload, observed-result fan-in, failure routing, prior frozen repeat, and independently corrected full functional verification | MEETS_EXPECTATIONS | The complex live workload generated five independent mutation tasks plus one gated verifier, dispatched all five Cursor Composer executors concurrently, and merged every result. It honestly reported a red verification outcome; independent review retained the performance work, rejected a cross-cutting result-contract change and coverage rewrite, and produced three green 75-76 second runs. Semantic arbitration across sibling patches remains a limitation. |
...
| `@you/spawn` | `BOOT-SPAWN-002-R01` | `BOOT-SPAWN-002-R02` | deterministic exact-count, maximum-budget, invalid-plan, child-failure, merge-failure, ordered-fan-in, and plain-text result coverage | MEETS_EXPECTATIONS | Both current-artifact trials executed exactly the requested task count and gated merge afterward. The representative merged three distinct repository investigations into an accurate implementation assessment. Child processes lacked development tools on `PATH`, so focused verification ran from the parent. |
...
| `@you/tournament` | `BOOT-TOURNAMENT-001-R01` | `BOOT-TOURNAMENT-001-R02` | deterministic one-, two-, and three-round brackets, exact budgets, advancement, mandatory-rationale, invalid-judge, and child-failure coverage; 20-repeat tournament run | MEETS_EXPECTATIONS | The live two-round bracket ran four competitors, two semifinal judges, and one final judge in gated phases. The unchanged champion answer carried both winning rationales and produced an accurate standards-aligned implementation plan. Child development tools required parent-side verification. |
| `@you/tts` | Excluded | Excluded | Not run by explicit user direction | UNVALIDATED | Explicitly excluded from this campaign; it was not tested or fixed. |
