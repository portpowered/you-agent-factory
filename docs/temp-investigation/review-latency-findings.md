...
Session duration is `endedAt - startedAt`.
The recorded `durationMillis` values differ from timestamp subtraction by 0-13 milliseconds.
The table uses timestamp subtraction so every displayed duration is independently reproducible.
...
| ---: | --- | --- | --- | ---: | ---: | --- |
| 1 | `881f4c19-cf82-4d81-8567-b886c55afdb4` | 02:18:30.443 | 02:19:09.593 | 39.150 s | 123.699 s after excluded fix session | CI-status visit; required checks pending |
| 2 | `28ca5651-5ece-499f-8cf7-4b231af808a0` | 02:19:11.959 | 02:19:57.791 | 45.832 s | 2.366 s | CI-status visit; workflow still running |
| 3 | `bd6c6495-24d6-49de-a1e1-c0a668aba55d` | 02:20:00.434 | 02:20:53.471 | 53.037 s | 2.643 s | CI-status visit; coverage jobs still running |
| 4 | `b7295fef-686c-4b1f-a6cb-0a3a8905ee19` | 02:20:55.458 | 02:21:43.898 | 48.440 s | 1.987 s | CI-status visit; coverage jobs still running |
| 5 | `9f22da37-21cf-4e10-a61c-3bc6623cb37a` | 02:21:46.242 | 02:22:27.633 | 41.391 s | 2.344 s | CI-status visit; coverage jobs still running |
| 6 | `63160e47-2aa5-498a-9033-ac967a23a6ac` | 02:22:31.125 | 02:23:13.466 | 42.341 s | 3.492 s | CI-status visit; coverage jobs still running |
| 7 | `ab1a8900-add9-475f-a0bb-597df45d638e` | 02:23:15.678 | 02:24:10.072 | 54.394 s | 2.212 s | CI-status visit; functional coverage still running |
| 8 | `bec76c23-1e37-4420-9442-275b55ab20b0` | 02:24:12.970 | 02:24:31.265 | 18.295 s | 2.898 s | CI-status visit; functional coverage still running |
| 9 | `f85c4900-0c07-425d-adb5-065021927710` | 02:24:33.362 | 02:25:09.242 | 35.880 s | 2.097 s | Terminal-CI detection visit |
| 10 | `f3641625-42c0-4f47-8c46-ef61660cf5cc` | 02:25:11.432 | 02:38:58.025 | 826.593 s | 2.190 s | Independent verification, final review comment, and merge |

The ten sessions total 1,205.353 seconds of Worker Session wall time.
The nine gaps within the sample total 22.229 seconds.
The median session duration is 44.087 seconds.

### Calculation cross-check

A second pass subtracted the recorded timestamps for all ten sessions and all nine internal gaps.
It independently reproduced session 1 as 39.150 seconds, session 6 as 42.341 seconds,
session 10 as 826.593 seconds, the 1,205.353-second total, and the 44.087-second median.
It also reproduced the 22.229-second gap total and the 378.760-second first-nine-session subtotal.
The corrected millisecond-scale differences do not change any rank or rounded savings estimate.
...
Their combined Worker Session wall time is 378.760 seconds.
...
the 1,205.353 seconds of sampled Worker Session time, not the full PR lifetime. This is one PR and
...
| 3 | Fresh-head CI critical path and pending-CI redispatch | Run 33032728213 lasted 563 s (9 m 23 s); Backend Functional Coverage was the longest required job at 515 s. From the first sampled reviewer start to CI completion was 394.557 s. During that overlap, nine reviewer visits consumed 378.760 s (31.4% of sampled Worker time), with a 42.341 s median; these are compute costs, not an additional 378.760 s of wall time. | First enforce and instrument the existing `ci-wait` invariant so pending checks cannot dispatch review. Separately profile and shard/cache Backend Functional Coverage without dropping required checks. | Gate enforcement can save up to 378.760 s of reviewer Worker time and nine raw visits on a lane with this failure mode, but little wall time because CI remains mandatory. A 25% reduction in the 515 s critical job would save at most about 129 s of CI wall time if it remains critical. Validate via zero pending-CI reviewer dispatches and hosted job p50/p90 over 20 runs. **Human tradeoff:** removing required checks could save up to the 563 s run, but reduces verification rigor and requires operator approval. |
...
sample total only 22.229 seconds (2.470 seconds per gap on average), and the final review began
