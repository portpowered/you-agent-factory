...
- When a field is removed from the generated `SubmitWorkRequest` contract, keep the unsupported-field rejection in `decodeSubmitWorkRequestBody`; `json.Unmarshal` into the generated struct will otherwise ignore the stale client field instead of returning the documented `400`.
