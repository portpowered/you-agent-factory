...
| `factory/logs/meta/asks.md` | Canonical customer-ask backlog for the meta cleanup loop | Meta and cleaner prompts should read customer asks here and should not treat `factory/meta/asks.md` as a peer source of truth. |
...
- When maintainer prompts need the customer backlog, point them to `factory/logs/meta/asks.md` explicitly so duplicate ask files cannot silently become peer control planes.
