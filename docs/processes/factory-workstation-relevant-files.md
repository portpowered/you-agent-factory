...
- When the default starter inbox contract changes, keep `pkg/cli/run/run_test.go` aligned too; the OOTB bootstrap smoke should prove the seeded file lands in `inputs/<default-work-type>/default` and that the retired `inputs/tasks` path stays absent during runtime startup.
