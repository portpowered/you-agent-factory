...
- Historical transport roots `pkg/api`, `pkg/apisurface`, `pkg/cli`, `pkg/mcp`, and `pkg/generatedclient` are retired. The package-boundary guard **MUST** reject repository imports of those roots and name the canonical successor under `pkg/transports`.
