...
- The durable product-owned root package families are: `pkg/config`, `pkg/factory`, `pkg/initializer`, `pkg/internal`, `pkg/models`, `pkg/orchestrators`, `pkg/root`, `pkg/transports`, `pkg/platform`, `pkg/wire`, `pkg/work`, and `pkg/workers`.
- Repository-only cross-package fixtures, mocks, assertions, and harnesses **MUST** live under `internal/testutil`; helpers coupled to one package's behavior **SHOULD** remain package-local. Production packages **MUST NOT** import repository-only test support.
...
- Migration-only root-family exceptions are not durable owners. The policy **MUST** record a target owner and a specific active work item for every exception: Batch 006 Transport family move covers `pkg/api`, `pkg/apisurface`, `pkg/cli`, and `pkg/mcp` targeting `pkg/transports`; Batch 007 Service and Factory Session ownership convergence plus Batch 008 Legacy composition-root deletion cover `pkg/service` targeting `pkg/wire` after its domain behavior reaches narrow owners; and Batch 008 Legacy composition-root deletion covers `pkg/runtimehost` and `pkg/composebridge` targeting `pkg/wire`. The completed platform family move is canonical at `pkg/platform/logging`, `pkg/platform/replay`, `pkg/platform/metrics`, `pkg/platform/cursors`, and `pkg/platform/clock`; Factory-event replay policy remains canonical in `pkg/factory/replay`. Each remaining exception **MUST** be removed when its named deletion gate is satisfied, and new behavior **MUST NOT** use an exception as a permanent owner.
