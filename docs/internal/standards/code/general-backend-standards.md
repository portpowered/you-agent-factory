...
- The durable product-owned root package families are: `pkg/config`, `pkg/factory`, `pkg/factorydefinition`, `pkg/factorysessionexecution`, `pkg/factorysessions`, `pkg/initializer`, `pkg/interfaces`, `pkg/internal`, `pkg/models`, `pkg/orchestrators`, `pkg/packagedfactories`, `pkg/petri`, `pkg/root`, `pkg/testutil`, `pkg/transports`, `pkg/platform`, `pkg/wire`, `pkg/work`, and `pkg/workers`.
- Startup ownership is non-overlapping. `pkg/root` normalizes process arguments and environment input and selects process mode, but **MUST NOT** construct domain dependencies or execute component lifecycle. `pkg/wire` constructs the concrete typed application graph and returns construction failures before startup, but **MUST NOT** choose process mode or start long-lived components. `pkg/initializer` starts, stops, cancels, joins, and unwinds already-built transports and sidecars, but **MUST NOT** lazily construct core domain services or rebuild Factory Session runtimes.
...
- `pkg/work` owns Work and Work Request content, query/selection, graph/lineage, pure invocation input and return policy, materialization, and cron/time-work concepts. Factory Session orchestration, worker/provider execution, and generic platform clocks are excluded.
- `pkg/platform` owns cross-cutting logging, replay and artifact infrastructure, metrics, cursor storage, and non-domain clock infrastructure. It may implement domain-owned interfaces, but **MUST NOT** choose Factory, Factory Session, worker, model, or Work policy.
- Migration-only root-family exceptions are not durable owners. The policy **MUST** record a target owner and a specific active work item for every exception: Batch 006 Work family move covers `pkg/invocations`, `pkg/materialize`, `pkg/timework`, `pkg/workcontent`, `pkg/workgraph`, and `pkg/workquery` targeting `pkg/work`; Batch 006 Platform family move covers `pkg/logging`, `pkg/replay`, and `pkg/sessionpersistence` targeting `pkg/platform`; Batch 007 Service and Factory Session ownership convergence plus Batch 008 Legacy composition-root deletion cover `pkg/service` targeting `pkg/wire` after its domain behavior reaches narrow owners; and Batch 008 Legacy composition-root deletion covers `pkg/runtimehost` and `pkg/composebridge` targeting `pkg/wire`. Each exception **MUST** be removed when its named deletion gate is satisfied, and new behavior **MUST NOT** use an exception as a permanent owner.
- Generated-code exceptions are documented separately from product-owned package families: `pkg/transports/http/client`, `pkg/transports/http/generated`, and generated Go files marked with the standard `Code generated ... DO NOT EDIT.` header when a scanner evaluates files. The package-boundary guard keeps these exceptions visible in its diagnostics instead of treating them as product-owned target package families. Generated-code exceptions **MUST NOT** become homes for handwritten product behavior.
- Batch 001 migration-only compatibility shim roots such as `pkg/workflowpolicy`, `pkg/workflowpreview`, `pkg/workflowresult`, `pkg/workflowsource`, and `pkg/workflowvalidation` have been removed and **MUST NOT** be recreated. Core runtime, API, CLI, and UI code **MUST** import the canonical owner under `pkg/orchestrators/javascript/*`.
...
- The historical `pkg/modelhost`, `pkg/localmodels`, and `pkg/hostedworkers` roots are retired. The package-boundary guard **MUST** reject recreating those roots or importing them or their subpackages, and **MUST** direct maintainers to `pkg/models/host`, `pkg/models/local` or `pkg/models/assets`, and `pkg/workers/hosted`, respectively. Canonical nested packages and package-local identifiers or test doubles remain allowed.
- Historical transport roots `pkg/api`, `pkg/apisurface`, `pkg/cli`, `pkg/mcp`, and `pkg/generatedclient` are retired. The package-boundary guard **MUST** reject recreating or importing them and name the canonical successor under `pkg/transports`.
- The package-boundary guard blocks imports of `pkg/wire` from domain and transport packages. `pkg/wire` may import inward owners, and only `pkg/root`, `pkg/initializer`, and `pkg/wire` itself may import the typed application graph. It also rejects handwritten Go files in generated-only transport packages.
