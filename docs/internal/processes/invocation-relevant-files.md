...
- The website's bundler resolver for data-only Packaged Factory exports is
  ...
  lane. Keep every generated import on a documented public package specifier and
  run `bun run check:public-package-boundaries`; its freshness check must reject
  manifest drift instead of letting the generated map become a handwritten
  catalog.
