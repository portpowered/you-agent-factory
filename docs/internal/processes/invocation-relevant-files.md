...
- Packaged factories that intentionally omit model-worker `modelProvider` and
  `model` fields inherit those resolved CLI defaults through
  `pkg/config/operatordefaultsruntime.ApplyToLoadedConfig`; keep cadence,
  invocation payload, worktree, and review configuration authored separately
  so a model/provider override cannot alter workflow behavior. Cover both the
  CLI `RunConfig` handoff and the materialized runtime worker configuration.
