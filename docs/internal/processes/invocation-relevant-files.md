...
- After folding invocation-time policy into `internal/services/invocation_policy`, add nested owner packages to both `go-unit-coverage-package-minimums.json` and `go-functional-coverage-package-minimums.json`, and reset transitional top-level policy shim floors to `0.00` because implementation coverage attributes to the nested owner while shims retain only re-export wiring.
