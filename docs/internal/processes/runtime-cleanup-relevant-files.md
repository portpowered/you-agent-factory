...
| Platform infrastructure | `pkg/platform` | Put logging in `pkg/platform/logging` and file-backed metric recording in `pkg/platform/metrics`, implementing the factory-owned emitter contract in `pkg/factory/metrics`; keep replay/artifact infrastructure, cursor storage, and non-domain clocks in narrow packages under the collapsed platform owner as their Batch 006 slices move. Shared log/metric path reservation is internal to `pkg/platform`. Platform implementations receive explicit inputs and never choose domain policy. |
