...
| Platform infrastructure | `pkg/platform` | Put logging in `pkg/platform/logging`; keep replay/artifact infrastructure, metrics, cursor storage, and non-domain clocks in narrow packages under the collapsed platform owner as their Batch 006 slices move. Platform implementations receive explicit inputs and never choose domain policy. |
