...
| Platform infrastructure | `pkg/platform` | Put logging in `pkg/platform/logging`, file-backed metric recording in `pkg/platform/metrics`, cursor mechanics and safe persistence diagnostics in `pkg/platform/cursors`, and real or deterministic clock implementations in `pkg/platform/clock`. Keep metric and clock meaning in factory-owned contracts, cursor identity/recovery decisions at their domain boundary, and replay/artifact infrastructure in its remaining Batch 006 lane. Shared log/metric path reservation is internal to `pkg/platform`. Platform implementations receive explicit inputs and never choose domain policy. |
