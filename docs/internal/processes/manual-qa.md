...
- Work outcome chart: loading, empty, error, and ready states render explicitly, sparse series do not appear as fabricated zero-value lines, and a populated localized chart exposes exactly one visible y-axis label while its compact legend remains keyboard-operable and contained at narrow widths.
