...
| Manual, opt-in three-sample unit experiment → `make test-unit-latency-budget` | S-02 (historical; not required CI) |
...
| `.github/workflows/regenerate-shared-ci-baselines.yml` → `make regenerate-shared-ci-baselines` | S-01 and S-03 through S-11, subject to the shared allowlist; S-02 remains frozen |
...
| --- | --- | --- | --- | --- |
| S-01 | `docs/internal/baselines/deadcode-baseline.txt` | Backend dead-code gate — normalized unreachable-symbol identity | `make regenerate-shared-ci-baselines BASELINE_REGEN_ROOT=. BASELINE_REGEN_DEADCODE_REPORT="$DEADCODE_REPORT_PATH"` | The protected workflow validates the delivered normalized dead-code report and leaves `make deadcode` as verification only. |
| S-02 | `docs/internal/baselines/go-unit-lane-latency-budget.v1.json` | Historical unit-lane performance comparison — three wall samples plus package and test inventories | `make test-unit-latency-budget` with three explicitly captured sample paths | Frozen opt-in evidence. Required CI and shared baseline regeneration do not rerun the unit corpus or update this file. |
