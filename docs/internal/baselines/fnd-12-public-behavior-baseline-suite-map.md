...
| --- | --- | --- | --- | --- |
| CLI | Captured Make: `make fnd-12-cli-behavior-baselines` (focused filter below). Equivalent: `go test ./pkg/transports/cli/baseline -run '^Test(RootHelpBaseline_MatchesFixture\|FailureBaseline_QuietInvalidTopologyWritesStructuredInvocationFailure)$$' -count=1`. The complete CLI baseline package is owned by `make test-contract`. | `TestRootHelpBaseline_MatchesFixture` — customer-visible `you --help` stdout matches the checked-in fixture via `root.BuildProcess`. | `TestFailureBaseline_QuietInvalidTopologyWritesStructuredInvocationFailure` — invalid factory topology fails before invocation with structured `ErrorResponse` on stderr (code/family/message). | Covered (captured) |
| HTTP | Captured Make: `make fnd-12-http-behavior-baselines` (focused filter below). Equivalent: `go test ./tests/functional/transport/http/server -run TestGeneratedClientAndServerSchemaStayAligned -count=1` plus `go test ./tests/functional/runtime_api -run TestGeneratedAPIIntegrationSmoke_SubmitWorkItemsRejectEmptyStructuredSubmission -count=1`. Broader Make wrap: `make api-smoke` (also regenerates/validates OpenAPI; heavier than the focused pair). | `TestGeneratedClientAndServerSchemaStayAligned` — published generated HTTP client decodes typed status and session observations against the live functional server after representative work submission. | `TestGeneratedAPIIntegrationSmoke_SubmitWorkItemsRejectEmptyStructuredSubmission` — empty structured submit returns HTTP 400. | Covered (captured) |
