...
| Factory Runtime → Workers execution service | Runtime starts one immutable route snapshot and injects the direct execution service into Worker Sessions | One Workers execution edge | Runtime owns route composition while Workers owns admission, capacity, cancellation, and terminal results |
