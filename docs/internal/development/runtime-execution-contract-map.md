...
| --- | --- | --- |
| `pkg/transports` | HTTP, CLI, MCP, generated transport contracts and clients, and boundary mapping. Transport code translates requests and responses and invokes injected application/domain services; it does not own domain policy or canonical runtime state. | The remaining migration-only roots are `pkg/api`, `pkg/apisurface`, and `pkg/cli` under **Batch 006 — Transport family move**. MCP behavior and callers now live under `pkg/transports/mcp` and `pkg/transports/cli/mcp`; remove each remaining exception when its behavior and callers have moved to `pkg/transports`. |
| `pkg/work` | Work and Work Request content, query/selection, graph/lineage, pure invocation input and return policy, materialization, and cron/time-work concepts. Factory Session orchestration, worker/provider execution, and generic platform clocks are excluded. | The migration-only roots are `pkg/invocations`, `pkg/materialize`, `pkg/timework`, `pkg/workcontent`, `pkg/workgraph`, and `pkg/workquery` under **Batch 006 — Work family move**. Remove each exception when its behavior and callers have moved to `pkg/work`; migrations must preserve public vocabulary and must not use the target as a compatibility grab bag. |
| `pkg/platform` | Cross-cutting logging, replay and artifact infrastructure, metrics, cursor storage, and non-domain clocks. It may implement domain-owned interfaces, but it does not choose Factory, Factory Session, worker, model, or Work policy. | The migration-only roots are `pkg/logging`, `pkg/replay`, and `pkg/sessionpersistence` under **Batch 006 — Platform family move**. Remove each exception when its behavior and callers have moved to `pkg/platform`; `pkg/config` remains a durable family, and domain time-work belongs in `pkg/work`. |
