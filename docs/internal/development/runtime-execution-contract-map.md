...
| --- | --- | --- |
| `pkg/transports` | HTTP, CLI, MCP, generated transport contracts and clients, and boundary mapping. Transport code translates requests and responses and invokes injected application/domain services; it does not own domain policy or canonical runtime state. | Transport behavior and callers live under `pkg/transports`; the historical `pkg/transports/http`, `pkg/transports/mapping`, `pkg/transports/cli`, `pkg/transports/mcp`, and `pkg/transports/http/client` roots are retired and blocked by `make pkg-boundary`. |
