# Factory API Contract Data Model

This artifact records the public-contract decisions for the Agent Factory
sharing seam. It explains the canonical authored payload, how
...
| --- | --- | --- | --- | --- |
| factory | public API resource | One persisted factory definition and the canonical public read or write payload for current-factory export, import activation, and editing. | `libraries/agent-factory/api/openapi-main.yaml` `Factory` schema | bundled as `#/components/schemas/Factory`, guarded in route and runtime tests |
| factory name | public identifier | Customer-facing identifier stored on the canonical `Factory.name` field and used to address one persisted factory definition, with `UNDEFINED` reserved for current-factory readback while the default root runtime is active. | `libraries/agent-factory/api/openapi-main.yaml` `FactoryName` schema | bundled as `#/components/schemas/FactoryName` |
...
| sharing payload | public authored contract | The exact authored `Factory` payload that must survive export and import without dashboard-side reshaping. | `Factory` schema plus the UI current-factory and persisted-factory API wrappers | export/import roundtrip coverage in `ui/src/features/import/hooks/use-factory-import-activation.test.tsx` |
| PNG envelope | browser transport wrapper | The PNG metadata wrapper that adds `schemaVersion` to the canonical `Factory` payload and nothing else. | `ui/src/features/export/factory-png-export.ts` and `ui/src/features/import/factory-png-import.ts` `PortOSFactoryPngEnvelope` | focused PNG export/import tests under `ui/src/features/export/` and `ui/src/features/import/` |

## Identifiers

| Identifier | Format | Producer | Consumer | Validation evidence |
| --- | --- | --- | --- | --- |
| `/factories` | HTTP `POST` route | Agent Factory REST API | UI import flows, backend clients | `TestOpenAPIContract_ContainsCoveredJSONOperations` and named-factory server tests |
...
| --- | --- | --- | --- | --- | --- |
| `Factory` | `libraries/agent-factory/api/openapi-main.yaml` | canonical public factory-definition fields, including top-level `name` and server-managed `version` for current-factory editing flows | schema-owned optional-field defaults | bundled contract reviewers, generated Go/UI artifacts, handler boundary | `pkg/api/openapi_contract_test.go` factory schema guard |
...
| `PortOSFactoryPngEnvelope` | `libraries/agent-factory/ui` sharing boundary | `schemaVersion` plus the direct canonical `Factory` fields | `schemaVersion` fixed to the current PNG format version | browser export, browser import, dashboard activation preview | `factory-png-export.test.ts`, `factory-png-import.test.ts` |
...
| --- | --- | --- | --- | --- | --- |
| authored factory contract | `api/openapi-main.yaml` | bundled `api/openapi.yaml` | authored source -> bundled artifact | missing route refs, schema refs, or example wiring drops the published contract | `make generate-api`, bundled contract guards |
...
| import activation seam | `ui/src/features/import/factory-png-import.ts` | `ui/src/features/import/hooks/use-factory-import-activation.ts` then `POST /factories` | PNG reader -> dashboard hook -> API | dashboard-only field renames or reshaping diverge from API contract | `use-factory-import-activation.test.tsx` |

## Canonical Sharing Boundary

The canonical sharing payload is the generated OpenAPI `Factory` schema:
...
1. `GET /factory-sessions/{session_id}/factory` returns the authored
   `Factory` payload for the current factory in the addressed live
...
3. Browser PNG import reads `PortOSFactoryPngEnvelope`, validates
   `schemaVersion`, rejects retired wrapper keys such as nested `factory`
   and legacy `factoryName`, and passes the exact embedded `Factory`
   payload to `POST /factories`.
...
`UNDEFINED` is reserved for current-factory readback only. `POST /factories`
...
- Shared interface, generated schema, contract package, or equivalent selected:
  `Factory` is the one public request and response shape for
  `POST /factories`, `GET /factory-sessions/{session_id}/factory`, and the PNG sharing payload inside
