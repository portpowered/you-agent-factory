...
| Contract drift enforcement: generated and runtime API surfaces stay in sync. | `complete` | `pkg/api/openapi_contract_test.go` keeps `/factory` and the canonical current-factory route on the public `Factory` schema, and `tests/functional/runtime_api/api_named_factory_test.go` now proves `POST /factory` and `GET /factory-sessions/{session_id}/factory` round-trip bundled portable files through that same public payload while the on-disk named-factory layout stays persistence-safe. | No remaining backend contract-drift gap in this lane after `US-002`; `US-004` can reuse these proofs in the final verification inventory. |
