...
focused stdio smoke in `pkg/cli/mcp/serve_runtime_smoke_test.go`. The default
fixture-backed install path remains unchanged in `pkg/cli/mcp/serve_smoke_test.go`.
...
| Artifact | Purpose |
|----------|---------|
| `pkg/cli/mcp/serve.go` | `--runtime` and `--project-root` flags; runtime service wiring |
