...
The phases below are intentionally split so multiple agents can work in parallel once Phase 0 defines the contract.

### Phase 0: Contract And Architecture Spike

Owner lanes can run sequentially first, then unblock all later work.

Deliverables:

- Architecture note: `docs/architecture/dynamic-workflows.md`.
- Data model update in `docs/architecture/data-model.md`.
- OpenAPI draft schemas and paths.
- Runtime choice decision record.
- Workflow SDK draft.
- Capability policy draft.
- Research-derived requirements checklist covering AgentLoom, `codex-dynamic-workflows-lab`, `pi-dynamic-workflows`, Bun `edit-round.workflow.ts`, Claude workflow docs, and `plugin-ultracode`.
- Test fixture plan with example workflow names.

Acceptance:

- Maintainers can review the public contract before runtime work starts.
- CLI/API/UI/MCP all point at the same service operations.
- The plan names all event, log, and metric additions.
- MVP defaults explicitly choose read-only policy, direct workflow filesystem/shell denial, explicit artifact roots, max local concurrency, max total agents, and structured `args`.

### Phase 1A: Domain, Validation, Store

Can run in parallel with Phase 1B and 1C after Phase 0.

Deliverables:

- `pkg/interfaces` domain types.
- Factory config extensions.
- Validation rules.
- Durable file store for workflows/runs/tasks/artifacts/checkpoints.
- Artifact hygiene module.
- NDJSON/event-log run replay support.
- Store fixtures and tests.

Acceptance:

- Saved workflows and run records round-trip.
- Resume checkpoint metadata can be written/read.
- Invalid configs fail with path-aware errors.
- Artifact capture rejects path escapes, symlinks, oversized output, and secret-like content according to policy.

### Phase 1B: OpenAPI And API Surface

Can run in parallel with Phase 1A once schema names settle.

Deliverables:

- OpenAPI paths and schemas.
- Generated Go and TypeScript types.
- `pkg/apisurface` mappers.
- API handler skeletons behind fake service.
- Contract fixtures.

Acceptance:

- Contract generation passes.
- API handlers prove mapping and error behavior without real runtime.

### Phase 1C: CLI And MCP Skeleton

Can run in parallel with Phase 1A/1B using fake service interfaces.

Deliverables:

- `you session` command updates for workflow-backed factories.
- `you mcp serve` entrypoint.
- MCP tool/resource/prompt schemas.
- Host configuration examples for Cursor, Codex, Kiro, Gemini, OpenCode, Claude Code.

Acceptance:

- CLI and MCP can list/preview/start fake workflow runs.
- MCP discovery works in a local smoke test.

### Phase 2A: JavaScript Runtime And Host API

Can run after Phase 1A store interfaces exist.

Deliverables:

- Runtime interface.
- Selected JS runtime implementation.
- Host API registration.
- Primitive globals: `args`, `phase`, `log`, `agent`, `parallel`, and `pipeline`.
- Source validation.
- `meta` extraction and phase validation.
- Structured-output schema validation.
- Cancellation and script error classification.
- Source-map or wrapper line remapping.
- Runtime unit/integration tests.

Acceptance:

- Simple JS workflow calls fake `agent.run`, writes an artifact, checkpoints, and finalizes.
- Script exceptions and cancellation produce classified failures.
- Customer-style workflow fixtures run without source interpolation.

### Phase 2B: Shared Orchestrator Loop And Scheduler

Can run in parallel with Phase 2A against fake runtime, then integrate.

Deliverables:

- Shared factory session orchestration service behavior.
- Dispatch scheduler.
- Budget/concurrency enforcement.
- Pause/resume/cancel.
- Event/log/metric emission.
- Orchestrator tests with fake dispatch executor.

Acceptance:

- `1`, `2`, and `N` child workflows complete.
- Pause/resume/cancel and timeout are deterministic.
- Events reconstruct run/task state.

### Phase 2C: JavaScript Dispatch Bridge

Can run in parallel with 2A/2B.

Deliverables:

- JavaScript `agent()` call to shared dispatch/provider/runner execution mapping.
- Runner normalization and availability checks.
- Worktree manager interface and fake implementation.
- Provider-session correlation.
- Dispatch bridge tests for all target runner ids.

Acceptance:

- A JavaScript agent dispatch can execute through the same provider boundary as a Petri workstation request.
- Unsupported runner and unavailable runner errors are consistent across API/CLI/MCP.

### Phase 3: End-To-End Backend Slice

Depends on 2A/2B/2C.

Deliverables:

- Real workflow run path from API/CLI/MCP to JS runtime to child execution to final artifact.
- Event stream integration.
- Factory session world projection.
- Functional workflow fixtures.
- Stress/race tests.

Acceptance:

- `single-agent`, `two-agent-verify`, and `fanout-synthesize` fixtures pass through API, CLI, and MCP.
- Run status and final artifact are identical across surfaces.
- Cancellation, timeout, and resume have functional coverage.

### Phase 4A: Website Experience

Can start after Phase 1B with mocks; complete after Phase 3.

Deliverables:

- Dynamic workflow feature API module/hooks.
- Run list/detail.
- Preview/approval dialog.
- Task table/detail.
- Artifact viewer.
- Budget/progress/timeline projections.
- Component and browser tests.

Acceptance:

- UI can preview, approve, monitor, pause/resume/cancel, and inspect artifacts.
- Loading, empty, error, running, paused, failed, timed out, canceled, and completed states are visible.
- Browser verification covers mobile and desktop.

### Phase 4B: Example Workflow Library

Can run after Phase 2A host SDK draft.

Deliverables:

- Curated `examples/workflows/` workflow files.
- Bun-inspired fixture workflows and fixture projects.
- Golden expected outputs for fake runners.
- Documentation for authoring workflows.

Acceptance:

- Example workflows are runnable in functional tests.
- Bun-inspired tests validate fan-out mapping, verification, resume, and synthesis behavior.

### Phase 5: Host Compatibility And Hardening

Depends on Phase 3 and Phase 1C.

Deliverables:

- MCP host smoke tests and docs for Cursor, Codex, Kiro, Gemini, OpenCode, Claude Code.
- Security hardening for sandbox/capabilities.
- Operational docs for budgets, logs, metrics, cleanup, worktrees, and provider costs.
- Soak/stress CI lane or opt-in verification command.

Acceptance:

- Each target host can connect to the MCP server and run a fake/dry workflow.
- Policy enforcement prevents unauthorized host operations.
- Stress suite covers high fan-out without provider calls.

## Parallel Work Breakdown

After Phase 0, split into these high-throughput lanes:

1. Contracts lane: OpenAPI, generated types, apisurface mappers, contract fixtures.
2. Store/domain lane: interfaces, validation, file store, checkpoint/artifact persistence.
3. Runtime lane: JavaScript runtime, host API, sandbox/cancellation/error classification.
4. Orchestrator lane: service, scheduler, budget enforcement, event/log/metric emission.
5. Dispatch bridge lane: provider bridge, runner compatibility, worktree isolation.
6. CLI/MCP lane: command group, MCP tools/resources/prompts, host config docs.
7. UI lane: React Query hooks, projections, run/task/artifact components, browser checks.
8. Test-fixture lane: sample workflows, Bun-inspired fixtures, golden outputs, stress harness.

Merge order should be contract/domain first, runtime/orchestrator/dispatch-bridge second, interface parity third, UI and hardening fourth. Feature branches can still be reviewed independently if fake service/runtime seams are used early.
