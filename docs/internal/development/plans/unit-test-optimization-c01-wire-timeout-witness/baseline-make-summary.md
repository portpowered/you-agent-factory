...
Each row has a matching status file and stdout/stderr capture. The accepted
reference median is `239.612s`, recomputed from exactly these three walls.
...
`baseline-make-run-1.v2.json`, `baseline-make-run-1.stdout.log`,
`baseline-make-run-1.stderr.log`, and `baseline-make-run-1.status.txt`. It
used the earlier measurement commit, exited 1, and stopped at 320/446
packages because the discovery code included the Linux-inactive
`filesystemreplace` package. Its status records the failure and the single
explicit replacement; no invalid artifact was silently replaced or deleted.
