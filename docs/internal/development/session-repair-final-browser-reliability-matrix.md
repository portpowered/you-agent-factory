...
| Bounded materialized work-outcome history survives persistence and restore with its exact continuation accumulator. | `App.session-recovery.test.tsx` — **renders six restored outcomes before one uniquely applied live tail** restores and renders ticks 0–6 from IndexedDB before the stream has delivered a message. `dashboard-shared-indexeddb-browser-contexts.integration.test.mjs` — **restores isolated concrete sessions and resumes one tail across lifecycle** renders six deterministic alpha samples before the paused live tail is released. The materialized round-trip and chart hook suites remain the lower-level persistence and projection proof. | Covered at App and real-browser composition boundaries. | `cd ui && bunx vitest run src/App.session-recovery.test.tsx integration/dashboard-shared-indexeddb-browser-contexts.integration.test.mjs --no-file-parallelism --maxWorkers 1` |
| One uniquely identified live tail extends restored history exactly once; duplicate delivery neither replaces history nor changes cumulative outcomes. | `App.session-recovery.test.tsx` — **renders six restored outcomes before one uniquely applied live tail** renders ticks 0–6, appends tick 7 once, redelivers the same event, and asserts unchanged samples plus exactly one stored tail event. The canonical two-page browser scenario sends the alpha tail twice with the same event ID and renders exactly the original six samples plus one new sample before and after reopen. | Covered at App and real-browser composition boundaries. | `cd ui && bunx vitest run src/App.session-recovery.test.tsx integration/dashboard-shared-indexeddb-browser-contexts.integration.test.mjs --no-file-parallelism --maxWorkers 1` |
| A supported page lifecycle boundary durably writes the latest exact-stream checkpoint without allowing an older or other-session write to win. | `App.multi-session-checkpoint-debounce.test.tsx` — **flushes on pagehide without blocking navigation or repeating the handoff**, **flushes only when visibility changes to hidden**, and **keeps newer same-stream state authoritative while A and B lifecycle writes overlap** prove the production lifecycle handoff. The canonical browser scenario dispatches the production `pagehide` boundary, polls IndexedDB transaction completion, reloads alpha, and proves its first exact-session stream resumes from the persisted live cursor while beta remains unchanged. | Covered at App, persistence, and real-browser composition boundaries. | `cd ui && bunx vitest run src/App.multi-session-checkpoint-debounce.test.tsx integration/dashboard-shared-indexeddb-browser-contexts.integration.test.mjs --no-file-parallelism --maxWorkers 1` |
| Two same-origin pages keep concrete UUID, cursor, stream generation, targeted cleanup, and rendered chart state isolated. | `dashboard-shared-indexeddb-browser-contexts.integration.test.mjs` — **restores isolated concrete sessions and resumes one tail across lifecycle** is the canonical proof. In one browser context it opens two same-origin pages, restores separate alpha and beta cursors, rejects cross-session stream URLs, deletes only a stale alpha generation, renders alpha's six-sample baseline plus one deduplicated tail, reopens alpha from the exact durable cursor, and proves beta's UUID, cursor, generation, and chart remain unchanged. Error capture, artifacts, checkpoint cleanup, pages, context, server, and test data remain bounded and failure-safe. | Covered by the canonical merged two-page scenario; no parallel cross-context scenario is needed. | `cd ui && bunx vitest run integration/dashboard-shared-indexeddb-browser-contexts.integration.test.mjs --no-file-parallelism --maxWorkers 1` |

Existing hook, persistence, and timeline tests remain the primary lower-level
