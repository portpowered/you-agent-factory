...
| Export dialog owns its own backdrop, panel, close button, form controls, validation copy, and action buttons. | `ui/src/features/export/components/export-factory-dialog.tsx` | Reuse dashboard typography tokens plus `DashboardButton`; if a reusable dialog shell is needed, extract one instead of keeping another local panel class bundle. | US-002 |
