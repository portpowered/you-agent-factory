...
| `Factory.workers[].modelProvider` | Loose string; runtime currently uses lowercase provider selectors | Named enum `WorkerModelProvider` with canonical public values such as `CLAUDE`, `CODEX` | `interfaces.ModelProviderClaude`, `interfaces.ModelProviderCodex` |
