...
| `Backend Functional Coverage` | `functional-boundary-check` then `cmd/gocoveragecheck` executes maintained short packages under `tests/functional/...` while measuring the same backend-owned code. | `make test-functional-coverage` | Shows the internal-system coverage contributed by functional flows without unit, stress, or release tests affecting the profile. Boundary regressions cannot pass this lane on coverage alone. |
