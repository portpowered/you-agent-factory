...
`go run ./cmd/pkgmaintcheck` is the direct reviewer command for the handwritten `pkg/` maintainability lane. It scans only owned `pkg/` Go source, excludes generated artifacts and `testdata` through the same repo-owned path rules as the backend size gate, and reports `file-lines`, `function-lines`, and `cyclomatic-complexity` violations with actual values and configured limits. Use rule-scoped inline directives only when a later maintainability story needs a narrow exception: `pkgmaintcheck:ignore-file-lines`, `pkgmaintcheck:ignore-function-lines`, or `pkgmaintcheck:ignore-cyclomatic-complexity`, each paired with a concrete justification comment.
