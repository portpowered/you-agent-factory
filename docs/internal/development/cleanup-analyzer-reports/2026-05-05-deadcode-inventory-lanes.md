...
| `ui/src/features/bento/hooks/useDashboardLayout.ts:LEGACY_SELECTION_WIDGET_IDS` and `LEGACY_WORK_OUTCOME_WIDGET_IDS` | Referenced by layout migration code that upgrades persisted dashboard widget IDs. | `retain` | These constants preserve supported local-storage migration behavior and are not dead. |
