...
- Validation source head: `4aa70d5319cbdc34c02aa0a4f48a4249f08285dd` (the exact pushed PR head at the start of this clean-room pass; the only subsequent change is this report-only evidence correction, with no implementation or functional-test source change).
...
3. `go test -json -count=1 -timeout=10m ./tests/functional/factory/packaged/... ./tests/functional/factory/current/... ./tests/functional/factory/definitions/... ./tests/functional/acceptance/...` produced 173 unique `Action=run` records whose `Test` contained `/`, but exited 1 because `packaged/full_flow/TestPackagedFullFlowRunsParallelWorktreesMergesAndReplansToCompletion` observed the known Windows `PROCESS_GONE` host-contended failure. The prescribed isolated package retry passed; the exact package aggregate was rerun and exposed the same transient failure. The per-package record counts are retained in the table; acceptance produced no named records.

This proves the current executable denominator and named-scenario accounting at clean local-real fidelity, but the aggregate command's non-zero exit is retained as an evidence limitation rather than treated as a pass. It does not resolve the stale planning denominator or recreate the absent source plan `docs/temp/functional-test-optimization.md`.
...
| Cleanup matrix H01/H02, U01-U07, B01-B05, Q01-Q02 | PASS for exercised probes | Seven exact-head intentional forced-unwind children reported zero owned residue for the applicable process, listener, session, definition, route, worktree, replay/runtime, selector, root, installed-copy, log/home/work, config, and reserved-port fields; Current/Definitions and acceptance repeat gates also passed. | The exact packaged aggregate is host-contended at `packaged/full_flow`; host-wide resources outside test ownership and unsupported platform signal behavior remain unproven. |
...
| Public behavior preservation | PASS for scoped diff | Current/Definitions and acceptance count-one/count-three gates passed; the isolated `packaged/full_flow` retry and exact-head race run passed; existing Work, Factory Event, dispatch, replay, CLI/API, persistence, and provider-call assertions remained active. | The multi-package packaged aggregate was contaminated by `PROCESS_GONE`; remote provider behavior and parent full-suite AC3 remain unproven. |
| GATE-PKG-001 | BLOCKED by host-contended retry | The exact command `go test -count=1 -timeout=15m ./tests/functional/factory/packaged/...` exited 1 twice at the source head when `packaged/full_flow` lost a `python3` child with `PROCESS_GONE`; the isolated `packaged/full_flow` retry exited 0. No C07 code path was changed in response. | A clean aggregate exit 0 on the current pushed head; remote providers and other package families. |
...
| GATE-REPEAT-004 | BLOCKED for current packaged head | Current-head Current/Definitions count-three and acceptance count-three exited 0. The packaged count-three result remains inherited from the validated implementation ancestor `bebd863b8555142b1960be9eab312746547188e3`; the current-head count-one aggregate was host-contended as described above. Local wall time is not used as an acceptance threshold. | A current-head packaged count-three aggregate, future host contention, and parent Project triple full-suite runs. |
...
| VAL-C07-009 | BLOCKED | The report is the read-only loopback artifact for the exact pushed source head and records the complete live census, matrix evidence, gate outputs, host-contended aggregate result, findings, and unproven edges. | The required exact denominator/source-plan authority and a clean current-head packaged aggregate are unavailable; review-stage PR CI, remote providers, Unix-only behavior, parent Project AC3, and project-wide inventory beyond this lane also remain outside local proof. |
...
3. Execute the packaged, Current/Definitions, and acceptance count-one commands through production root composition. Current/Definitions and acceptance exited 0; the packaged aggregate exited 1 twice on the Windows `packaged/full_flow` `PROCESS_GONE` path, while its isolated retry exited 0.
4. Execute each owned package group with `-count=3`. Current/Definitions and acceptance exited 0 at the source head; packaged count-three remains the previously validated implementation-ancestor result because the current aggregate was host-contended. No production or shared-support repair was made.
...
| C07-F003 | ENVIRONMENTAL EVIDENCE BLOCKER | Run the exact-head JSON discovery command in the clean checkout. | Discovery execution exits 0 while yielding the named records. | The source-head command yielded all 173 named records but exited 1 at `packaged/full_flow` with `PROCESS_GONE`; an isolated retry passed and no C07-caused failure was reproduced. | Exact source-head discovery output and package retry logs retained outside the repository. |
| C07-F004 | REVIEW-STAGE | Push the final report head and inspect the required PR check. | Current-head Backend Functional Coverage has started and review can record the package timing/result. | The existing PR has current-head CI in progress; this report correction must be pushed before the next required CI run can represent the final head. Terminal CI and merge remain review-owned. | GATE-PR-CI-008 handoff row above. |
