...
test-only shared fixture. Its fixture core remains in
`tests/functional/providers/helpers_test.go`; shared-process assertions are in
the existing `cli_script_executor_test.go`, runtime-log scanning is in
`runtime_logging_smoke_test.go`, and forced-cleanup proof is in
`cli_timeout_cleanup_smoke_test.go`. The fixture builds one
`root.BuildProcess` application, starts one loopback listener, runs one
continuously executing process, and exposes explicit
...
2. `go test ./tests/functional/providers -run
   ...
   forced assertion-cleanup tests after the test-only fixture move and split
   across the existing aggregate test files.
...
| C06-F-004 | RESOLVED REVIEW BLOCKER | `git diff origin/main...HEAD -- docs/internal/baselines/deadcode-baseline.txt` on the prior reviewed head `98b2dfb6a0`; `make deadcode` after rebased head `7c89c7e808`. | The excluded baseline remains unchanged and the provider fixture does not add deadcode findings. | Prior head added 34 provider-helper entries, which was a compatibility failure; the rebased review fix keeps the fixture in `_test.go`, removes all 34 lines, and current deadcode has no provider fixture finding. The later backend-size correction splits the test-only declarations across existing aggregate files without changing the baseline. | Prior Compatibility status is recorded as FAIL in this report history; current rebased diff has no baseline change. Windows retains only the existing Unix/Windows lock-helper mismatch. |
