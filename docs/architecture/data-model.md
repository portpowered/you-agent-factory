...
- `Factory Session`: one live running factory instance identified by `session_id`. A factory session owns runtime state such as event history, current work, and the currently active factory loaded into that session. Clients that must survive backend restart also track **logical session identity**: `backendScopeID` plus `logicalSessionKeyID`, an opaque id derived from the normalized factory session target rather than a separately persisted logical-session record. The current live UUID `factorySessionID` and `streamGenerationID` can change across restart while the logical target stays the same. See `docs/architecture/logical-session-identity.md` for normalization, remap, and recovery semantics.
...
- `FactoryResponseEvent`: one ephemeral, session-scoped observation record for invocation progress (message snapshots, tool lifecycle updates, and related public progress while work is running). Response events are delivered on `GET /factory-sessions/{session_id}/response-events` and through CLI NDJSON response-stream output. They do not enter canonical `FactoryEvent` replay history and are not a substitute for durable session facts.
