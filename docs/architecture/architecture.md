...
| Factory event ledger, replay, artifacts, and projections | `pkg/services/recordings` | Own canonical event history, replay policy, durable execution artifacts, and read-model projections. Under the event-contract lease, Recordings also owns the sole public Factory Event kind inventory (`PublicEmittableFactoryEventKinds`, `ExcludedNonPublicFactoryEventKinds`, `ContractOnlyFactoryEventKinds` in `pkg/services/recordings/events/kinds/`). Factory Session response-stream / `FactoryResponseEvent` vocabulary remains a separate observation stream, not a second public Factory Event inventory. `PSS-I05` may later converge shared publish/replay plumbing against this frozen vocabulary and must not introduce a competing public Factory Event kind inventory. |
| Workers and providers | `pkg/services/workers` | Put execution requests/results and normalized failures in `pkg/services/workers/execution`, customer-safe projections in `pkg/services/workers/diagnostics`, runner selection in `pkg/services/workers/runner`, and provider adapters, process runners, sidecars, hosted logic, scripts, testing, and inference under the same service family. |
