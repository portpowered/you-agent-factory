...
| `pkg/cli` | Migration-era root command and execution/session/model/server/MCP adapters. Setup, configuration, packaged docs, factory authoring, and workflow validation/preview already live under `pkg/transports/cli`. | Move each remaining command journey under `pkg/transports/cli`, then remove the historical root under Batch 008. |
