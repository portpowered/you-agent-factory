...
| `pkg/invocations`, `pkg/timework`, `pkg/workgraph`, `pkg/workquery` | `pkg/work` | **Batch 006 — Work family move.** Remove each exception when its query, graph, pure invocation policy, or cron/time-work behavior and callers have moved to `pkg/work`. Work content and materialization already live under this owner. |
