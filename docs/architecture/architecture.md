...
| `pkg/invocations` | `pkg/factorysessions` for stateful orchestration; other narrow owners for remaining helpers | **Batch 007 — Service and Factory Session ownership convergence.** Pure input, argument, interpolation, and return-selection policy has moved to `pkg/work/invocation`. Remove the root exception after live invocation orchestration and the remaining inference and permission helpers move to their narrow owners. |
