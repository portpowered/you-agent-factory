...
| `pkg/invocations` | Narrow model/worker owners for the remaining inference and permission helpers | **Batch 007 — Service and Factory Session ownership convergence.** Pure input, argument, interpolation, and return-selection policy has moved to `pkg/work/invocation`, and live invocation orchestration has moved to `pkg/factorysessions/invocation`. Remove the root exception after the remaining inference and permission helpers move to their narrow owners. |
