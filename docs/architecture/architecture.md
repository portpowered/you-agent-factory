...
| `pkg/invocations`, `pkg/timework`, `pkg/workgraph` | `pkg/work` | **Batch 006 — Work family move.** Remove each exception when its graph, pure invocation policy, or cron/time-work behavior and callers have moved to `pkg/work`. Work content, materialization, and query/selection already live under this owner. |
