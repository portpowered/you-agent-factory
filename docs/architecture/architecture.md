...
| `pkg/invocations`, `pkg/timework` | `pkg/work` | **Batch 006 — Work family move.** Remove each exception when its pure invocation policy or cron/time-work behavior and callers have moved to `pkg/work`. Work content, materialization, query/selection, and graph/lineage already live under this owner. |
