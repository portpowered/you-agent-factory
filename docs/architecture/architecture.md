...
| `pkg/replay` | `pkg/platform` | **Batch 006 — Platform family move.** Logging, runtime metrics, cursor mechanics, and non-domain clocks are canonical under `pkg/platform/logging`, `pkg/platform/metrics`, `pkg/platform/cursors`, and `pkg/platform/clock`; remove the remaining exception when replay and artifact infrastructure and callers move to `pkg/platform`. |
