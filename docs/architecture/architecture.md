...
| `pkg/replay`, `pkg/sessionpersistence` | `pkg/platform` | **Batch 006 — Platform family move.** Logging and runtime metrics are canonical under `pkg/platform/logging` and `pkg/platform/metrics`; remove each remaining exception when its replay/artifact, cursor persistence, or non-domain clock infrastructure and callers have moved to `pkg/platform`. |
