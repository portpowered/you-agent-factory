...
| `pkg/invocations` | `pkg/work` | **Batch 006 — Work family move.** Remove the exception when pure invocation policy and callers have moved to `pkg/work`. Work content, materialization, query/selection, graph/lineage, and cron/time-work behavior already live under this owner. |
