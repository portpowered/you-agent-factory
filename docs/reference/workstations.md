...
| `modelProvider` | Execution-family provider override for this workstation. Supported built-in values are `CLAUDE`, `CODEX`, `GEMINI`, `CURSOR`, `KIRO`, and `OPENCODE`, plus symbolic `DEFAULT` to defer through factory, worker, and operator defaults. |
| `openCodeAgent` | Optional OpenCode agent profile override for this workstation. When set, replaces the bound worker's `openCodeAgent` for dispatches that resolve to provider `OPENCODE`. Must be a non-empty string. |
...
Execution-family provider selection resolves in this order: workstation
`modelProvider`, then factory `modelProvider`, then worker `modelProvider`,
then the operator default. Symbolic `DEFAULT` at workstation or factory scope
defers to the next step in that order. Selecting a built-in provider expects
the corresponding local CLI and auth/setup to already be available before
execution starts.

OpenCode agent selection resolves separately from provider choice:
...
- The resolved name is used only when execution-family selection resolves to
  `OPENCODE`. A non-empty agent on a non-OpenCode provider fails during factory
  build.
