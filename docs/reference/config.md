...
This is distinct from factory-level `modelProvider` in `factory.json`. Public `factory.json` and OpenAPI `modelProvider` fields are unchanged by operator defaults; they continue to control execution-family provider selection as documented on this page and in `you docs workers`.
