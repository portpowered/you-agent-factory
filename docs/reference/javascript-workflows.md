...
| `pipeline` | `await pipeline(items, stage1, stage2, ...)` | Runs one or more stages for each item. `stage1(originalItem, index)` starts the item; every later stage receives `(previousResult, originalItem, index)`. Results remain ordered by input item and include per-stage status and results. |
