...
type promptWorkstation struct {
	Worker     string `yaml:"worker"`
	PromptFile string `yaml:"promptFile"`
}

type authoredPromptFactory struct {
	Workers      []promptWorker      `yaml:"workers"`
	Workstations []promptWorkstation `yaml:"workstations"`
}
...
func TestGenerateArtifactsProducesEquivalentSelfContainedPairsForCompleteInventory(t *testing.T) {
	...
	if len(artifacts) != 19 {
		t.Fatalf("artifacts = %d, want 19", len(artifacts))
	}
...
func TestPackagedFactoryPromptFilesRemainWorkstationOwned(t *testing.T) {
	...
	for slug, wantPromptFiles := range want {
		...
		t.Run(slug, func(t *testing.T) { assertFactoryPromptOwnership(t, slug, wantPromptFiles, bySlug) })
	}
}

func assertFactoryPromptOwnership(t *testing.T, slug string, wantPromptFiles map[string]string, bySlug map[string]packagedfactorycatalog.ArtifactPair) {
	t.Helper()
	...
	var authored authoredPromptFactory
