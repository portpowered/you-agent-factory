...
// VerifyWorkDualLedgerAlignment proves package-target-manifest and
// ownership-inventory agree on Work package dispositions and that every move
// row's manifest destination aligns with the ownership successor path.
func VerifyWorkDualLedgerAlignment(root string) error {
	if err := VerifyWorkUnexpectedPublicSiblingRemaps(root); err != nil {
		return err
	}
	...
	for _, manifestRow := range manifest.Packages {
		if !isWorkOwnerPackagePath(manifestRow.PackagePath) {
			continue
		}
		...
		if err := validateWorkDualLedgerRow(manifestRow, ownershipRow); err != nil {
			return err
		}
	}

	for _, ownershipRow := range inventory.Packages {
		if !isWorkOwnerPackagePath(ownershipRow.PackagePath) {
			continue
		}
...
func isWorkOwnerPackagePath(packagePath string) bool {
	return packagePath == workPackagePrefix ||
		strings.HasPrefix(packagePath, workPackagePrefix+"/")
}

func validateWorkDualLedgerRow(manifestRow PackageTargetLedgerRow, ownershipRow PackageRow) error {
