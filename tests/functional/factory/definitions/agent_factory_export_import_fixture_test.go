...
func newAgentFactoryExportImportFixture(
	...
	runner := support.NewRecordingCommandRunner("layout-only export/import proof must not execute providers")
	edges := serviceedges.Edges{ProviderCommandRunner: runner}

	factoryConfigPath := filepath.Join(authoredDir, interfaces.FactoryConfigFile)
	canonicalPayload, err := flattenFactoryConfigWithEdges(t, edges, factoryConfigPath)
	if err != nil {
		t.Fatalf("flattenFactoryConfigWithEdges(%s): %v", name, err)
	}
	if _, err := decodeFactoryDefinitionForTest(canonicalPayload); err != nil {
		t.Fatalf("decode authored fixture %s: %v", name, err)
	}
	if runner.CallCount() != 0 {
		t.Fatalf("provider command runner calls = %d, want 0 during flatten/readback", runner.CallCount())
	}
...
// TestAgentFactoryExportImportFixtureAuthoredLayoutInterpolatesProjectSpecificPromptContent
// proves authored worker and workstation prompts interpolate project-specific
// content during export/import fixture layout materialization without invoking
// external provider command execution.
func TestAgentFactoryExportImportFixtureAuthoredLayoutInterpolatesProjectSpecificPromptContent(t *testing.T) {
...
// TestAgentFactoryExportImportFixtureFlattenedPayloadKeepsCanonicalArrayRoutes proves
// the public flatten path preserves canonical JSON array routes for export/import
// fixtures without dispatching external provider command execution.
func TestAgentFactoryExportImportFixtureFlattenedPayloadKeepsCanonicalArrayRoutes(t *testing.T) {
