...
type packagedQuorumSharedFixture struct {
	...
	baseURL    string
	factoryDir string
	lifecycle  *packagedQuorumLifecycleLedger
}

type packagedQuorumLifecycleResource struct {
	sessionID     string
	rootDir       string
	factoryDir    string
	closed        bool
...
type packagedQuorumLifecycleLedger struct {
	mu            sync.Mutex
	expected      int
	processStarts int
	resources     []packagedQuorumLifecycleResource
}

func newPackagedQuorumLifecycleLedger(expected int) *packagedQuorumLifecycleLedger {
	return &packagedQuorumLifecycleLedger{expected: expected}
}

func (ledger *packagedQuorumLifecycleLedger) recordProcessStart() {
	ledger.mu.Lock()
	defer ledger.mu.Unlock()
	ledger.processStarts++
}

func (ledger *packagedQuorumLifecycleLedger) register(
	...
	ledger.resources = append(ledger.resources, packagedQuorumLifecycleResource{
		sessionID:  sessionID,
		rootDir:    rootDir,
...
func (ledger *packagedQuorumLifecycleLedger) close(
	sessionID string,
...
func (ledger *packagedQuorumLifecycleLedger) assertClean(t testing.TB) {
	t.Helper()
	ledger.mu.Lock()
	processStarts := ledger.processStarts
	resources := append([]packagedQuorumLifecycleResource(nil), ledger.resources...)
	expected := ledger.expected
	ledger.mu.Unlock()

	if processStarts != 1 {
		t.Errorf("QUORUM-SPINE-001 process starts = %d, want 1 observed API host start", processStarts)
	}
	if len(resources) != expected {
		t.Errorf("QUORUM-SPINE-001 explicit sessions opened = %d, want %d", len(resources), expected)
	}
	...
	t.Logf(
		"quorum lifecycle: process_starts=%d explicit_sessions_opened=%d explicit_sessions_closed=%d unique_session_ids=%d scenario_roots_removed=%d runtime_artifacts=0 isolated_rows=0",
...
func newPackagedQuorumSharedFixture(t *testing.T) *packagedQuorumSharedFixture {
	...
	lifecycle := newPackagedQuorumLifecycleLedger(packagedQuorumExpectedSessions)
...
func (fixture *packagedQuorumSharedFixture) cleanup(t testing.TB) {
	...
	if fixture.baseURL != "" {
		...
		if err == nil {
			...
			response.Body.Close()
			t.Errorf("QUORUM-CLEANUP-001 listener still served /status after process close: %s", strings.TrimSpace(string(body)))
		}
	}
	if err := os.RemoveAll(fixture.rootDir); err != nil {
		t.Errorf("QUORUM-CLEANUP-001 remove shared root %q: %v", fixture.rootDir, err)
	} else if _, err := os.Stat(fixture.rootDir); !os.IsNotExist(err) {
		t.Errorf("QUORUM-CLEANUP-001 shared root %q remains after process close: %v", fixture.rootDir, err)
	}
}
...
func (fixture *packagedQuorumSharedFixture) newScenario(
	...
	workingDirectory := filepath.Join(rootDir, "work")
	if err := os.MkdirAll(homeDir, 0o755); err != nil {
		t.Fatalf("create quorum scenario home: %v", err)
	}
	if err := os.MkdirAll(workingDirectory, 0o755); err != nil {
		t.Fatalf("create quorum scenario working directory: %v", err)
	}
...
func (scenario *packagedQuorumScenario) open(t *testing.T) {
	...
	if scenario.sessionID == factorysessions.DefaultSessionID {
		t.Fatalf("opened quorum Factory Session returned default session %q", scenario.sessionID)
	}
	t.Cleanup(func() {
		scenario.close(t)
	})
	if err := scenario.fixture.lifecycle.register(
		...
		t.Fatalf("register quorum scenario lifecycle: %v", err)
	}
}

func (scenario *packagedQuorumScenario) close(t testing.TB) {
	t.Helper()
	if scenario == nil || scenario.sessionID == "" {
		return
	}
	support.CloseFactorySessionAt(t, scenario.fixture.baseURL, scenario.sessionID)
	assertPackagedQuorumSessionAbsent(t, scenario.fixture.baseURL, scenario.sessionID)
	sessionAbsent := true
	rootRemoved := false
	if err := os.RemoveAll(scenario.rootDir); err != nil {
		t.Errorf("QUORUM-CLEANUP-001 remove scenario root %q: %v", scenario.rootDir, err)
	} else if _, err := os.Stat(scenario.rootDir); !os.IsNotExist(err) {
		t.Errorf("QUORUM-CLEANUP-001 scenario root %q remains: %v", scenario.rootDir, err)
	} else {
		rootRemoved = true
	}
	if err := scenario.fixture.lifecycle.close(scenario.sessionID, sessionAbsent, rootRemoved); err != nil {
		t.Errorf("record quorum scenario cleanup: %v", err)
	}
}

func assertPackagedQuorumSessionAbsent(t testing.TB, baseURL, sessionID string) {
	t.Helper()
	endpoint := strings.TrimSuffix(baseURL, "/") + "/factory-sessions/" + url.PathEscape(sessionID)
	response, err := http.Get(endpoint)
	if err != nil {
		t.Fatalf("GET deleted quorum Factory Session %q: %v", sessionID, err)
	}
	defer response.Body.Close()
	if response.StatusCode != http.StatusNotFound {
		payload, _ := io.ReadAll(response.Body)
		t.Fatalf(
			"GET deleted quorum Factory Session %q status = %d, want 404: %s",
