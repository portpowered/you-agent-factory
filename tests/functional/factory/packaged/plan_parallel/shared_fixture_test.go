...
type planParallelSharedFixture struct {
	...
	baseURL    string
	factoryDir string
	lifecycle  *planParallelLifecycleLedger
}

type planParallelLifecycleResource struct {
	sessionID     string
	rootDir       string
	factoryDir    string
	closed        bool
...
type planParallelLifecycleLedger struct {
	mu            sync.Mutex
	expected      int
	processStarts int
	resources     []planParallelLifecycleResource
}

func newPlanParallelLifecycleLedger(expected int) *planParallelLifecycleLedger {
	return &planParallelLifecycleLedger{expected: expected}
}

func (ledger *planParallelLifecycleLedger) recordProcessStart() {
	ledger.mu.Lock()
	defer ledger.mu.Unlock()
	ledger.processStarts++
}

func (ledger *planParallelLifecycleLedger) register(
	...
	ledger.resources = append(ledger.resources, planParallelLifecycleResource{
		sessionID:  sessionID,
		rootDir:    rootDir,
...
func (ledger *planParallelLifecycleLedger) close(
	sessionID string,
...
func (ledger *planParallelLifecycleLedger) assertClean(t testing.TB) {
	t.Helper()
	ledger.mu.Lock()
	processStarts := ledger.processStarts
	resources := append([]planParallelLifecycleResource(nil), ledger.resources...)
	expected := ledger.expected
	ledger.mu.Unlock()

	if processStarts != 1 {
		t.Errorf("PLAN-PARALLEL-SPINE-001 process starts = %d, want 1 observed API host start", processStarts)
	}
	if len(resources) != expected {
		t.Errorf("PLAN-PARALLEL-SPINE-001 explicit sessions opened = %d, want %d", len(resources), expected)
	}
	...
	t.Logf(
		"plan-parallel lifecycle: process_starts=%d explicit_sessions_opened=%d explicit_sessions_closed=%d unique_session_ids=%d scenario_roots_removed=%d runtime_artifacts=0 isolated_rows=0",
...
func newPlanParallelSharedFixture(t *testing.T) *planParallelSharedFixture {
	...
	lifecycle := newPlanParallelLifecycleLedger(planParallelExpectedSessions)
...
func (fixture *planParallelSharedFixture) cleanup(t testing.TB) {
	...
	if fixture.baseURL != "" {
		...
		if err == nil {
			...
			response.Body.Close()
			t.Errorf("PLAN-PARALLEL-CLEANUP-001 listener still served /status after process close: %s", strings.TrimSpace(string(body)))
		}
	}
	if err := os.RemoveAll(fixture.rootDir); err != nil {
		t.Errorf("PLAN-PARALLEL-CLEANUP-001 remove shared root %q: %v", fixture.rootDir, err)
	} else if _, err := os.Stat(fixture.rootDir); !os.IsNotExist(err) {
		t.Errorf("PLAN-PARALLEL-CLEANUP-001 shared root %q remains after process close: %v", fixture.rootDir, err)
	}
}
...
func (fixture *planParallelSharedFixture) newScenario(
	...
	workingDirectory := filepath.Join(rootDir, "work")
	if err := os.MkdirAll(homeDir, 0o755); err != nil {
		t.Fatalf("create plan-parallel scenario home: %v", err)
	}
	if err := os.MkdirAll(workingDirectory, 0o755); err != nil {
		t.Fatalf("create plan-parallel scenario working directory: %v", err)
	}
...
func (scenario *planParallelScenario) open(t *testing.T) {
	...
	if scenario.sessionID == factorysessions.DefaultSessionID {
		t.Fatalf("opened plan-parallel Factory Session returned default session %q", scenario.sessionID)
	}
	t.Cleanup(func() {
		scenario.close(t)
	})
	if err := scenario.fixture.lifecycle.register(
		...
		t.Fatalf("register plan-parallel scenario lifecycle: %v", err)
	}
}

func (scenario *planParallelScenario) close(t testing.TB) {
	t.Helper()
	if scenario == nil || scenario.sessionID == "" {
		return
	}
	support.CloseFactorySessionAt(t, scenario.fixture.baseURL, scenario.sessionID)
	assertPlanParallelSessionAbsent(t, scenario.fixture.baseURL, scenario.sessionID)
	sessionAbsent := true
	rootRemoved := false
	if err := os.RemoveAll(scenario.rootDir); err != nil {
		t.Errorf("PLAN-PARALLEL-CLEANUP-001 remove scenario root %q: %v", scenario.rootDir, err)
	} else if _, err := os.Stat(scenario.rootDir); !os.IsNotExist(err) {
		t.Errorf("PLAN-PARALLEL-CLEANUP-001 scenario root %q remains: %v", scenario.rootDir, err)
	} else {
		rootRemoved = true
	}
	if err := scenario.fixture.lifecycle.close(scenario.sessionID, sessionAbsent, rootRemoved); err != nil {
		t.Errorf("record plan-parallel scenario cleanup: %v", err)
	}
}

func assertPlanParallelSessionAbsent(t testing.TB, baseURL, sessionID string) {
	t.Helper()
	endpoint := strings.TrimSuffix(baseURL, "/") + "/factory-sessions/" + url.PathEscape(sessionID)
	response, err := http.Get(endpoint)
	if err != nil {
		t.Fatalf("GET deleted plan-parallel Factory Session %q: %v", sessionID, err)
	}
	defer response.Body.Close()
	if response.StatusCode != http.StatusNotFound {
		payload, _ := io.ReadAll(response.Body)
		t.Fatalf(
			"GET deleted plan-parallel Factory Session %q status = %d, want 404: %s",
