...
var docsSmokeTopics = []docsSmokeTopic{
	...
	{name: "relationships", heading: "# Relationships", markers: []string{"DEPENDS_ON", "PARENT_CHILD", "SPAWNED_BY", "requiredState", "sourceWorkName", "targetWorkName", "workTypeName", "requestId", "FACTORY_REQUEST_BATCH", "## Submitted relation name scope", "intra-batch name references", "previously submitted or", "otherwise existing Work", "you submit batch --dry-run", "`you docs guards`", "`you docs batch-inputs`"}, absent: []string{"work_type_name", "source_work_name", "target_work_name", "[Guards](guards.md)", "[Batch Inputs](batch-inputs.md)"}},
	...
	{name: "providers", heading: "# Providers, worker models, and ACP agents", markers: []string{"gpt-5.6-luna", "gpt-5.6-sol", "gpt-5.6-terra", "claude-sonnet-5", "modelProvider: ANTIGRAVITY", "gemini-3.6-flash-high", "gpt-oss-120b-medium", "low", "medium", "high", "GPT-5.6 does not provide video or audio understanding", "referenced_image_paths", "0–5 paths per call", "executorProvider", "modelProvider", "cursor-acp", "kiro-acp", "opencode-acp", "you workers list", "you workers acp add", "you workers acp delete", "~/.you-agent-factory/config.json", "skipPermissions", "you factory config validate", "--provider cursor-acp", "@you/goal", "agent.run", "you docs javascript-workflows", "Factory validation checks configuration shape", "## Configure a Factory worker or one ad-hoc run", "reasoningEffort: high", "timeout: 45m", "--worker-reasoning-effort", "--to-file", "--add-dir <working-directory>", "--print-timeout", "final `result` event", "status: SUCCESS", "public Factory and `you run` contract rejects any"}, absent: []string{"The separate AGY effort value may be omitted or set to", "print-mode command adapter accepts a separate `reasoningEffort`"}},
	...
	{name: "serve-acp", heading: "# Host You As An ACP Agent", markers: []string{"you serve acp", `"args": ["serve", "acp"]`, "root.BuildProcess", "## Shutdown Behavior", "Clean stdin EOF", "cancellation outcome", "## Configure A Minimal ACP Client", "## Exchange One ACP Prompt", "session/new", "session/prompt", "session/update", "stopReason", "## Distinguish This From Related ACP And MCP Surfaces", "you workers acp add", "you workers acp delete", "you mcp serve", "you acp serve", "not a recognized command", "## Know What Is Proven", "root_serve_test.go", "serve_acp_prompt_test.go", "`you docs providers`", "`you docs mcp`", "`you docs sessions`"}, absent: []string{"you docs serve-acp-hosts", "Neither command is an alias for `you workers acp`"}},
}
