...
var docsSmokeTopics = []docsSmokeTopic{
	...
	{name: "packaged-goal", heading: "# Packaged Goal (`@you/goal`)", markers: []string{"you run --named @you/goal", "MODEL_WORKER", "MODEL_WORKSTATION", "goal-executor", "execute-goal", "~/.you-agent-factory/factories", "@you%2Fgoal", "INVOCATION_INPUT_SOURCE_CONFLICT", "INVOCATION_BLOCKED", "INVOCATION_NEEDS_HUMAN", "INVOCATION_PAUSED", "INVOCATION_INTERRUPTED", "INVOCATION_RUNTIME_FAILURE", "INVOCATION_TIMED_OUT", "INVOCATION_PRIMARY_RESULT_UNRESOLVED", "sessionId", "workId", "workState", "you session resume <session-id>", "make docs-reference-smoke", "editable", "`you docs authoring-factories`", "`you docs config`", "`you docs sessions`", "`you docs workstations`", "`you docs workers`"}, absent: []string{"[Authoring Factories](authoring-factories.md)", "[Config](config.md)", "[Sessions](sessions.md)"}},
...
func TestDocsCommandSmoke_PackagedTopicsRemainAvailableOutsideRepositoryDocsTree(t *testing.T) {
	...
	for _, want := range []string{"# Docs", "`agents` - Agent orientation: read order, work submission, command matrix, planner vs executor, and topic router", "`authoring-factories` - Practical factory authoring workflow", "`config` - factory.json topology, operator model defaults, work types, states, workers, workstations, resources, and portability", "`mock-workers` - Mock-worker runs", "`record-replay` - Record and replay run modes", "`guards` - Workstation, input, and factory guards", "`relationships` - Batch DEPENDS_ON", "`work` - Submitted work: session-scoped work routes, tags, batch cross-links, and submission contracts", "`sessions` - Live factory sessions: session list, session show, pause and resume, factory query, status API, dashboard URL, and run modes", "`packaged-goal` - Packaged @you/goal invocation", "`packaged-tts` - Packaged @you/tts invocation", "`batch-inputs` - Batch input files", "`workstations` - Workstation kinds", "`you docs agents`", "`you docs authoring-factories`", "`you docs config`", "`you docs mock-workers`", "`you docs record-replay`", "`you docs guards`", "`you docs relationships`", "`you docs work`", "`you docs sessions`", "`you docs packaged-goal`", "`you docs packaged-tts`", "`you docs batch-inputs`", "`you docs workstations`"} {
