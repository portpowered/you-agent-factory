...
var docsSmokeTopics = []docsSmokeTopic{
	...
	{name: "packaged-factories", heading: "# Packaged Factories", markers: []string{"## Discovery and first use", "you factory list", "you run --named <factory> --help", "operator defaults", "nineteen", "## Common entry contract", "model-generated prose is not byte-stable", "## Catalog", "@you/subagent", "@you/agy-clip-qa", "@you/agy-cold-watch", "@you/fix", "Plans and iterates a requested fix in an isolated named worktree", "@you/ralph", "Plans a request, iterates through every incomplete plan story", "--shot-specification", "--cut-path", "AGY production review composition example", "TestAgyProductionReviewRolesThroughRootBuildProcess", "Do not enable that live smoke in ordinary CI"}, absent: []string{"you docs packaged-goal", "you docs packaged-tts"}},
	...
	{name: "models", heading: "# Models", markers: []string{"you models list", "you models inspect llm", "you models inspect asr", "you models inspect tts", "you models inspect embed", "you models pull llm", "you --json models pull embed", "5.0 GB", "148 MB", "1.714 GB", "639 MB", "local Models composition", "INFINITE_YOU_OMNIVOICE_CACHE_DIR", "readinessState", "lifecycleState", "Invoke a built-in model directly from any directory.", "A Current Factory is not\nrequired.", "its Models scope remains available", "Current Factory", "An explicit `--server` must identify a reachable service", "A missing implicit Current Factory is not an error for a built-in invocation.", "An explicitly supplied or malformed Factory still returns its typed Factory\nerror", "never creates or initializes a Factory as a workaround.", "you models invoke tts --operation TTS", "you models invoke embed --operation EMBED --input text=\"Find similar work\"", "you models invoke llm --offline --operation OMNI --input prompt=\"Explain how a SHA-256 checksum differs from encryption.\"", "you models invoke llm --operation OMNI --input prompt=\"Write a haiku\"", "### Generate an embedding", "parameters=json:", "you --json models invoke embed", "`--offline`", "MODEL_BACKEND_FAILURE", "MODEL_OFFLINE_CACHE_UNAVAILABLE", "POST /models/invocations", "--output speech.wav", "you --json models invoke", "## Operation contracts", "--input audio=@meeting.wav", "--output transcript=meeting.txt", "application/json", "raw audio bytes", "--text", "unqualified `--output <path>`", "artifact references", "before download or backend activation", "INFERENCE_RUN", "INFERENCE_WORKER", "WorkContent", "`you docs providers`", "`you docs workers`", "`you docs workstations`"}, absent: []string{"docs/reference/workstations-and-workers.md", "## Maintainer Long-Test Expectations", "issue #2201", "The placement is therefore not proven intentional", "OMNIVOICE_Q4_K_M", "Run this zero-configuration command", "shared in-process bootstrap"}},
...
func TestDocsCommandSmoke_AuthoringFactoriesDescribesFactoryBuilder(t *testing.T) {
	output := executeDocsSmokeCommand(t, t.TempDir(), "docs", "authoring-factories")
	for _, want := range []string{
		"The nineteen first-party packaged Factories",
		"`@you/factory-builder`",
		"`@you/fix`",
		"Plans and iterates a requested fix in an isolated named worktree",
		"`@you/ralph`",
		"Plans a request, iterates through every incomplete plan story",
