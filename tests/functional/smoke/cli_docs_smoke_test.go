...
var docsSmokeTopics = []docsSmokeTopic{
	...
	{name: "authoring-factories", heading: "# Authoring Factories", markers: []string{"factory.json", "workers/<name>/AGENTS.md", "workstations/<name>/AGENTS.md", "you run --factory ./factory.json \"Fix the lint issues\"", "handlingBehavior: [\"DEFAULT\"]", "you run --dir ./factory --with-mock-workers", "you docs mock-workers", "you docs record-replay", "you docs packaged-goal", "you docs packaged-tts", "you run --named @you/goal", "`you docs agents`", "--no-record", "requestId", "workTypeName"}, absent: []string{"work_type_name", "source_work_name", `"request_id"`, "[Agents](agents.md)"}},
	{name: "packaged-goal", heading: "# Packaged Goal (`@you/goal`)", markers: []string{"you run --named @you/goal", "browser or dashboard interaction", "primaryResult", "invocationReturn", "avoids binding a localhost API listener", "goal-executor", "execute-goal", "~/.you-agent-factory/you-agent-factories", "@you/goal", "INVOCATION_INPUT_SOURCE_CONFLICT", "INVOCATION_BLOCKED", "INVOCATION_NEEDS_HUMAN", "INVOCATION_PAUSED", "INVOCATION_INTERRUPTED", "INVOCATION_RUNTIME_FAILURE", "INVOCATION_TIMED_OUT", "INVOCATION_PRIMARY_RESULT_UNRESOLVED", "sessionId", "workId", "workState", "you session resume <session-id>", "make docs-reference-smoke", "editable", "invocation-return contract", "`you docs authoring-factories`", "`you docs config`", "`you docs sessions`", "`you docs workstations`", "`you docs workers`", "## Goal flow topology", "multi-stage flow", "plan-goal", "check-goal", "review-goal", "structured-review-goal", "goal:init", "goal:plan", "goal:execute", "goal:complete", "goal:blocked", "goal:needs-human", "goal:interrupted", "goal:failed", "## Operator controls during active execution", "you session pause", "SESSION_LIFECYCLE_CONTROL", "`--output response-stream`", "live progress fragments", "## Inspect-first recovery flow", "FactorySession", "goal-planner/AGENTS.md", "structured-review-goal/AGENTS.md", "does not add `/goal/inspect`"}, absent: []string{"[Authoring Factories](authoring-factories.md)", "[Config](config.md)", "[Sessions](sessions.md)", "MODEL_WORKER", "MODEL_WORKSTATION", "Workstation.workPropagation is absent", "does not carry invocationReturn", "automatic JSON detection", "SessionResponseStream"}},
	{name: "packaged-tts", heading: "# Packaged TTS (`@you/tts`)", markers: []string{"you run --named @you/tts", "INFERENCE_WORKER", "INFERENCE_RUN", "does not use agent-loop fields", "~/.you-agent-factory/you-agent-factories", "@you/tts", "artifactPath", "mediaType", "backend", "editable", "raw audio", "shared invocation contract", "INVOCATION_TTS_MODEL_NOT_READY", "INVOCATION_TTS_GENERATION_FAILED", "`you docs authoring-factories`", "`you docs config`", "`you docs sessions`"}, absent: []string{"[Authoring Factories](authoring-factories.md)", "[Config](config.md)", "[Sessions](sessions.md)"}},
	{name: "config", heading: "# Config", markers: []string{"factory.json", "work types, states, workers, workstations, resources, and routing", "## Operator Model Defaults", "~/.you-agent-factory/config.json", "YOU_DEFAULT_WORKER_MODEL_PROVIDER", "YOU_DEFAULT_WORKER_MODEL", "--default-worker-model-provider", "--default-worker-model", "file < env < flag", "you run --runner", "`factory.json` and OpenAPI `runner` fields are unchanged by operator defaults", "## Work Types", "handlingBehavior: [\"DEFAULT\"]", "## Top-Level Fields", "supportingFiles", "## Portability Resource Manifest", "## Resources", "## How The Pieces Fit", "you docs agents", "you docs work", "you docs mock-workers", "you docs record-replay", "you docs guards", "you docs relationships", "you docs authoring-factories", "--factory", "you run --factory ./factory.json \"Fix the lint issues\"", "--with-mock-workers", "--record", "--replay", "--no-record", "you-agent-factory run"}, absent: []string{"Agent Factory run", "## Single-Work API Submission"}},
	...
	{name: "sessions", heading: "# Sessions and Runtime", markers: []string{"you session list", "you session pause", "you session resume", "POST /factory-sessions/{session_id}/pause", "SESSION_LIFECYCLE_CONTROL", "make docs-reference-smoke", "you factory query", "GET /factory-sessions/{session_id}/status", "factoryState", "runtimeStatus", "categories", "INVOCATION_BLOCKED", "INVOCATION_NEEDS_HUMAN", "INVOCATION_PAUSED", "INVOCATION_INTERRUPTED", "INVOCATION_RUNTIME_FAILURE", "INVOCATION_TIMED_OUT", "INVOCATION_CANCELED", "http://localhost:7437/dashboard/ui", "`you docs agents`", "`you docs work`", "`you docs config`"}, absent: []string{"[Agents](agents.md)", "[Work](work.md)", "[Config](config.md)"}},
	{name: "mcp-hosts", heading: "# Dynamic Workflow MCP Host Setup", markers: []string{"you mcp serve", "stdio only for this batch", "## Serve Mode Selection", "Fixture-backed (default)", "Runtime-backed", `"args": ["mcp", "serve", "--runtime"]`, "## Complete Runtime-Backed Factory Session Transcript", "you.factory_session.validate_source", "you.factory_session.start_async", "you.factory_session.get", "you.factory_session.get_result", "you.factory_session.list_dispatches", "you.factory_session.list_artifacts", "you.factory_session.read_events", "you.factory_session.control", `"sessionId":"fs-host-demo-01"`, "compatibility-only aliases", "## Reconnect With Stable Identifiers", `"afterEventId":"event-host-demo-07"`, "FactoryArtifact `id`", "## Typed Outcomes And Host Action", "factory_session.result.not_ready", "factory_session.session.not_found", "factory_session.events.reconnect_cursor_not_found", "factory_session.start.request_id_conflict", "factory_session.service.unavailable", ".cursor/mcp.json", "## Smoke Proof Boundaries", "### Coverage Matrix", "Automated in-repo", "serve_runtime_smoke_test.go", "serve_runtime_resume_smoke_test.go", "serve_runtime_resume_non_regression_test.go", "Documented manual", "pkg/cli/mcp/serve_smoke_test.go", "### Manual Host Smoke Sequence", "## Shared MCP Install Blockers", "no bounded follow-up blocker", "`you docs orchestrators`", "`you docs sessions`"}, absent: []string{"[Orchestrators](orchestrators.md)", "[Sessions](sessions.md)"}},
	...
	{name: "workers", heading: "# Workers", markers: []string{"INFERENCE_WORKER", "AGENT_WORKER", "POLLER_WORKER", "MODEL_WORKER", "SCRIPT_WORKER", "HOSTED_WORKER", "auth.secretRef", "secrets/linear-api-key", "INFINITE_YOU_SECRET_SECRETS_LINEAR_API_KEY", "linear.teamIds", "linear.mapping.workType", "modelProvider", "YOU_DEFAULT_WORKER_MODEL_PROVIDER", "--default-worker-model-provider", "Authored worker `modelProvider` and `model` values always win", "docs/reference/workers.md"}, absent: []string{"docs/reference/workstations-and-workers.md"}},
	...
	{name: "models", heading: "# Models And Model Operations", markers: []string{"INFERENCE_RUN", "INFERENCE_WORKER", "MODEL_INVOKE", "MODEL_WORKER", "Harnessless", "model host", "lease", "WorkContent", "OMNIVOICE_Q4_K_M", "you models invoke", "`you docs workers`", "`you docs workstations`"}, absent: []string{"docs/reference/workstations-and-workers.md"}},
	{name: "mcp", heading: "# MCP Install Path For Factory Preview Tools", markers: []string{"you mcp serve", "mcpServers", `"args": ["mcp", "serve"]`, "you.factory_session.validate_source", "you.factory_session.start_async", "you docs mcp-hosts", "serve_smoke_test.go", "serve_runtime_smoke_test.go", "serve_runtime_resume_smoke_test.go", "serve_runtime_resume_non_regression_test.go", "Automation-Backed In Repo", "## Serve Mode Scope Boundaries", "you mcp serve --runtime", "Fixture-backed (default)", "Runtime-backed", "`you docs orchestrators`"}, absent: []string{"[Orchestrators](orchestrators.md)", "Follow-Up Cell For Async Install Smoke", "follow-up-cell-mcp-session-serve.md"}},
}
...
func TestDocsCommandSmoke_PackagedTopicsRemainAvailableOutsideRepositoryDocsTree(t *testing.T) {
	...
	for _, want := range []string{"# Docs", "`agents` - Agent orientation: read order, work submission, command matrix, planner vs executor, and topic router", "`authoring-factories` - Practical factory authoring workflow", "`config` - factory.json topology, operator model defaults, work types, states, workers, workstations, resources, and portability", "`mock-workers` - Mock-worker runs", "`record-replay` - Record and replay run modes", "`guards` - Workstation, input, and factory guards", "`relationships` - Batch DEPENDS_ON", "`work` - Submitted work: session-scoped work routes, tags, batch cross-links, and submission contracts", "`sessions` - Live factory sessions: session list, session show, pause and resume, factory query, status API, dashboard URL, and run modes", "`packaged-fusion` - Packaged @you/fusion invocation", "`packaged-goal` - Packaged @you/goal batch invocation", "`packaged-tts` - Packaged @you/tts invocation", "`batch-inputs` - Batch input files", "`workstations` - Workstation kinds", "`you docs agents`", "`you docs authoring-factories`", "`you docs config`", "`you docs mock-workers`", "`you docs record-replay`", "`you docs guards`", "`you docs relationships`", "`you docs work`", "`you docs sessions`", "`you docs packaged-fusion`", "`you docs packaged-goal`", "`you docs packaged-tts`", "`you docs batch-inputs`", "`you docs workstations`"} {
