...
var docsSmokeTopics = []docsSmokeTopic{
	{name: "agents", heading: "# Agents", markers: []string{"## Start Here", "you docs agents", "factory/docs/overview.md", "factory/docs/README.md", "## Read Order (Any Factory)", "## Submitting Work", "Autonomous agents must submit work only through the CLI", "`you submit batch`", "### Batch submit for agents", "### Idempotency and duplicate work", "requestId", "duplicate batches", "you submit batch ./batches/release-story-set.json", "## Is the factory running?", "you session list", "you factory query", "/factory-sessions/", "factoryState", "runtimeStatus", "dashboard/ui", "## Operator loop", "## Command Matrix", "Operator-only", "## Planner vs Executor", "## Topic Router", "`you docs config`", "`you docs templates`", "`you docs resources`", "`you docs models`", "`you docs batch-inputs`", "[Config](config.md)", "[Work](work.md)", "[Batch Inputs](batch-inputs.md)", "[Relationships](relationships.md)", "[Authoring Factories](authoring-factories.md)", "FACTORY_REQUEST_BATCH", "POST /work"}, absent: []string{"thoughts:init", "idea:init", "plan:init", "task:in-review", "Work enters a running factory through one of these ingress paths", "| Ingress | When to use |", "| Watched `factory/inputs/**` JSON files |", "| `POST /work` | Single submitted work item", "Place batch files under the inbox paths"}},
	...
	{name: "sessions", heading: "# Sessions and Runtime", markers: []string{"you session list", "you factory query", "GET /factory-sessions/{session_id}/status", "factoryState", "runtimeStatus", "categories", "http://localhost:7437/dashboard/ui", "[Agents](agents.md)", "[Work](work.md)", "[Config](config.md)", "you docs agents"}},
