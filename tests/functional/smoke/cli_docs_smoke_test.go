...
var docsSmokeTopics = []docsSmokeTopic{
	...
	{name: "packaged-goal", heading: "# Packaged Goal (`@you/goal`)", markers: []string{"you run --named @you/goal", "browser or dashboard interaction", "primaryResult", "invocationReturn", "avoids binding a localhost API listener", "goal-executor", "execute-goal", "~/.you-agent-factory/you-agent-factories", "@you/goal", "INVOCATION_INPUT_SOURCE_CONFLICT", "INVOCATION_BLOCKED", "INVOCATION_NEEDS_HUMAN", "INVOCATION_PAUSED", "INVOCATION_INTERRUPTED", "INVOCATION_RUNTIME_FAILURE", "INVOCATION_TIMED_OUT", "INVOCATION_PRIMARY_RESULT_UNRESOLVED", "sessionId", "workId", "workState", "you session resume <session-id>", "make docs-reference-smoke", "editable", "invocation-return contract", "`you docs authoring-factories`", "`you docs config`", "`you docs sessions`", "`you docs workstations`", "`you docs workers`", "## Goal flow topology", "minimal goal repeater", "REPEATER", "Continue and reject outcomes route back to `goal:init`", "goal:init", "goal:execute", "goal:complete", "goal:failed", "## Operator controls during active execution", "you session pause", "SESSION_LIFECYCLE_CONTROL", "`--output response-stream`", "live progress fragments", "## Inspect-first recovery flow", "FactorySession", "goal-executor/AGENTS.md", "execute-goal/AGENTS.md", "does not add `/goal/inspect`"}, absent: []string{"[Authoring Factories](authoring-factories.md)", "[Config](config.md)", "[Sessions](sessions.md)", "MODEL_WORKER", "MODEL_WORKSTATION", "Workstation.workPropagation is absent", "does not carry invocationReturn", "automatic JSON detection", "SessionResponseStream", "multi-stage flow", "plan-goal", "check-goal", "review-goal", "structured-review-goal", "goal:plan", "goal:review", "goal:structured-review", "goal:blocked", "goal:needs-human", "goal:interrupted", "goal-planner/AGENTS.md", "goal-checker/AGENTS.md", "goal-reviewer/AGENTS.md"}},
	...
	{name: "orchestrators", heading: "# Orchestrators and Factory Sessions", markers: []string{"### Beta JavaScript child-agent contract", "`prompt`", "`label`", "`preset`", "`modelProvider`", "`model`", "`reasoningEffort`", "agent-run-valid", "agent-run-invalid", "`agent.run() does not support field \"writableRoots\"`", "dynamically constructed argument object"}},
	{name: "mcp-hosts", heading: "# Dynamic Workflow MCP Host Setup", markers: []string{"you mcp serve", "stdio only for this batch", "## Serve Mode Selection", "Fixture-backed (default)", "Runtime-backed", `"args": ["mcp", "serve", "--runtime"]`, "you.factory_session.validate_source", "you.factory_session.start_async", "you.factory_session.get", "you.factory_session.get_result", ".cursor/mcp.json", "## Smoke Proof Boundaries", "### Coverage Matrix", "Automated in-repo", "serve_runtime_smoke_test.go", "serve_runtime_resume_smoke_test.go", "serve_runtime_resume_non_regression_test.go", "Documented manual", "pkg/cli/mcp/serve_smoke_test.go", "### Manual Host Smoke Sequence", "## Shared MCP Install Blockers", "no bounded follow-up blocker", "`you docs orchestrators`", "`you docs sessions`"}, absent: []string{"[Orchestrators](orchestrators.md)", "[Sessions](sessions.md)"}},
