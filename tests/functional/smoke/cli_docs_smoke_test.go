...
var docsSmokeTopics = []docsSmokeTopic{
	...
	{name: "orchestrators", heading: "# Orchestrators and Factory Sessions", markers: []string{"### Beta JavaScript child-agent contract", "`prompt`", "`label`", "`preset`", "`modelProvider`", "`model`", "`reasoningEffort`", "`skipPermissions`", "agent-run-valid", "agent-run-invalid", "`agent.run() does not support field \"writableRoots\"`", "dynamically constructed argument object", "`you docs javascript-workflows`"}},
	...
	{name: "metrics", heading: "# Metrics", markers: []string{"you metrics", "--group-by workstation", "--group-by worker", "--group-by provider", "--session <id>", "--json", "Input tokens", "Output tokens", "Completed dispatches", "Failures by reason", "p50", "p95", "milliseconds", "unavailable", "all_factory_sessions", "p50: null", "`you docs sessions`", "`you docs record-replay`"}},
}
