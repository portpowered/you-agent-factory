...
var docsSmokeTopics = []docsSmokeTopic{
	...
	{name: "packaged-tts", heading: "# Packaged TTS (`@you/tts`)", markers: []string{"you run --named @you/tts", "~/.you-agent-factory/factories", "@you%2Ftts", "artifactPath", "mediaType", "backend", "editable", "raw audio", "shared invocation contract", "INVOCATION_TTS_MODEL_NOT_READY", "INVOCATION_TTS_GENERATION_FAILED", "`you docs authoring-factories`", "`you docs config`", "`you docs sessions`"}, absent: []string{"[Authoring Factories](authoring-factories.md)", "[Config](config.md)", "[Sessions](sessions.md)"}},
	{name: "config", heading: "# Config", markers: []string{"factory.json", "work types, states, workers, workstations, resources, and routing", "## Operator Model Defaults", "~/.you-agent-factory/config.json", "YOU_DEFAULT_WORKER_MODEL_PROVIDER", "YOU_DEFAULT_WORKER_MODEL", "--default-worker-model-provider", "--default-worker-model", "file < env < flag", "you run --runner", "`factory.json` and OpenAPI `modelProvider` fields are unchanged by operator defaults", "## Work Types", "handlingBehavior: [\"DEFAULT\"]", "## Top-Level Fields", "supportingFiles", "## Portability Resource Manifest", "## Resources", "## How The Pieces Fit", "Retired `runner` Fields", "factory.runner is retired; use factory.modelProvider", "no automatic config migration command", "you docs agents", "you docs work", "you docs mock-workers", "you docs record-replay", "you docs guards", "you docs relationships", "you docs authoring-factories", "--factory", "you run --factory ./factory.json \"Fix the lint issues\"", "--with-mock-workers", "--record", "--replay", "--no-record", "you-agent-factory run"}, absent: []string{"Agent Factory run", "## Single-Work API Submission"}},
	...
	{name: "mcp-hosts", heading: "# Dynamic Workflow MCP Host Setup", markers: []string{"you mcp serve", "stdio only for this batch", "you.factory_session.validate_source", "you.factory_session.start_async", "you.factory_session.get", "you.factory_session.get_result", ".cursor/mcp.json", "## Smoke Proof Boundaries", "### Coverage Matrix", "Automated in-repo", "Documented manual", "pkg/cli/mcp/serve_smoke_test.go", "### Manual Host Smoke Sequence", "## Shared MCP Install Blockers", "no bounded follow-up blocker", "`you docs orchestrators`", "`you docs sessions`"}, absent: []string{"[Orchestrators](orchestrators.md)", "[Sessions](sessions.md)"}},
	{name: "mock-workers", heading: "# Mock Workers", markers: []string{"--with-mock-workers", "mockWorkers", "unmatchedDispatchPolicy", "passthrough", "runType", "accept", "reject", "script", "scriptConfig", "docs/examples/mock-workers.json", "docs/examples/mock-workers-script.json", "docs/examples/mock-workers-mixed.json", "docs/examples/startup-work.json", "## Reviewer Verification", "Do not rely on a live real-agent passthrough run for signoff", "automated service and provider tests"}},
	{name: "record-replay", heading: "# Record and Replay", markers: []string{"--record", "--replay", "--no-record", "~/.you-agent-factory/recordings/", "docs/examples/sample-run.replay.json", "Recording saved:", "`--record` with `--replay`", "`--no-record` with `--record`", "Legacy Dispatch Provider Metadata", "runnerId", "modelProvider", "replay compatibility adapter"}},
	...
	{name: "workstations", heading: "# Workstations Reference", markers: []string{"workstation authoring contract", "MODEL_WORKSTATION", "CLASSIFIER_WORKSTATION", "LOGICAL_MOVE", "`you docs guards`", "`you docs relationships`"}, absent: []string{"../internal/development/parent-aware-fan-in.md", "../internal/development/workstation-guards-and-guarded-loop-breakers.md", "[Guards](guards.md)", "[Relationships](relationships.md)"}},
	...
	{name: "workers", heading: "# Workers", markers: []string{"MODEL_WORKER", "SCRIPT_WORKER", "HOSTED_WORKER", "auth.secretRef", "secrets/linear-api-key", "INFINITE_YOU_SECRET_SECRETS_LINEAR_API_KEY", "linear.teamIds", "linear.mapping.workType", "modelProvider", "YOU_DEFAULT_WORKER_MODEL_PROVIDER", "--default-worker-model-provider", "Authored worker `modelProvider` and `model` values always win", "docs/reference/workers.md"}, absent: []string{"docs/reference/workstations-and-workers.md"}},
	...
	{name: "mcp", heading: "# MCP Install Path For Factory Preview Tools", markers: []string{"you mcp serve", "mcpServers", `"args": ["mcp", "serve"]`, "you.factory_session.validate_source", "you.factory_session.start_async", "you docs mcp-hosts", "serve_smoke_test.go", "Automation-Backed In Repo", "Follow-Up Cell For Async Install Smoke", "dynamic-workflows-cell-mcp-session-serve", "follow-up-cell-mcp-session-serve.md", "`you docs orchestrators`"}, absent: []string{"[Orchestrators](orchestrators.md)"}},
}
