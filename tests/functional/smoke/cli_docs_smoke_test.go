...
var docsSmokeTopics = []docsSmokeTopic{
	...
	{name: "packaged-factories", heading: "# Packaged Factories", markers: []string{"## Discovery and first use", "you factory list", "you run --named <factory> --help", "operator defaults", "## Common entry contract", "model-generated prose is not byte-stable", "## Catalog", "@you/subagent"}, absent: []string{"you docs packaged-goal", "you docs packaged-tts"}},
...
func TestDocsCommandSmoke_PackagedTopicsRemainAvailableOutsideRepositoryDocsTree(t *testing.T) {
	...
	for _, want := range []string{"# Docs", "`agents` - Agent orientation: read order, work submission, command matrix, planner vs executor, and topic router", "`authoring-factories` - Practical factory authoring workflow", "`packaged-factories` - First-party @you/* Factory catalog, live invocation discovery, and operator guide", "`run` - Supported local, one-shot, batch, continuous, and mock-worker run shapes", "`config` - Operator initialization and Factory validation, flattening, expansion, and minimum authoring contract", "`mock-workers` - Mock-worker runs", "`record-replay` - Record and replay run modes", "`guards` - Workstation, input, and factory guards", "`relationships` - Batch DEPENDS_ON", "`work` - Submitted work: session-scoped work routes, tags, batch cross-links, and submission contracts", "`sessions` - Live factory sessions: session list, session show, pause and resume, factory query, status API, dashboard URL, and run modes", "`providers` - ACP agent setup", "`batch-inputs` - Batch input files", "`workstations` - Workstation kinds", "`you docs agents`", "`you docs authoring-factories`", "`you docs packaged-factories`", "`you docs run`", "`you docs config`", "`you docs mock-workers`", "`you docs record-replay`", "`you docs guards`", "`you docs relationships`", "`you docs work`", "`you docs sessions`", "`you docs providers`", "`you docs batch-inputs`", "`you docs workstations`"} {
