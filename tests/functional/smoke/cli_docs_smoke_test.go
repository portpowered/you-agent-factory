...
var docsSmokeTopics = []docsSmokeTopic{
	...
	{name: "factory-validation", heading: "# Factory Validation", markers: []string{"required static gate between authoring", "start a Factory Session or scheduler", "invoke a worker, model, or provider", "create or update a persisted named Factory", "./examples/basic/factory/factory.json", "./packages/packaged-factories/factories/classify/factory.yaml", "./examples/basic/factory", "exactly one regular Factory", "SAME_NAME", "ALL_CHILDREN_COMPLETE", "at most two", "Each workstation input has one optional `guard` object", "classificationRoutes", "./docs/examples/factory-validation/unsupported-three-input-join.json", "Factory validation failed.", "observed arity is 3 inputs", "at most 2 inputs are supported", "supported-two-input-join.json", "Factory validation passed."}, absent: []string{"you config validate", "you factory validate"}},
	{name: "work", heading: "# Submitted Work", markers: []string{"## Single-Work API Submission", "## Submission contract shapes", "SubmitWorkRequest", "WorkRequest", "`items` cannot be combined with `content` or `payload`", "POST /factory-sessions/{session_id}/work", "~default", "workTypeName", "## Watch Work state transitions", "you work watch", "you.work.watch.v1", "sequence", "--follow", "Ctrl-C", "last accepted event ID and sequence", "does not persist a cursor", "jq -c 'select(.terminal)'", "ConvertFrom-Json", "## Tags And Prompt Templates", "Token.Tags", "`you docs config`", "`you docs batch-inputs`", "FACTORY_REQUEST_BATCH"}, absent: []string{"../internal/development/parent-aware-fan-in.md", "../internal/development/workstation-guards-and-guarded-loop-breakers.md", "## Work Types", "supportingFiles", "## Portability Resource Manifest", "[Config](config.md)", "[Batch Inputs](batch-inputs.md)", "POST /work`", "`POST /work/staged-files`"}},
	...
	{name: "workers", heading: "# Workers", markers: []string{"INFERENCE_WORKER", "AGENT_WORKER", "POLLER_WORKER", "MODEL_WORKER", "SCRIPT_WORKER", "HOSTED_WORKER", "auth.secretRef", "secrets/linear-api-key", "INFINITE_YOU_SECRET_SECRETS_LINEAR_API_KEY", "linear.teamIds", "linear.mapping.workType", "modelProvider", "YOU_DEFAULT_WORKER_MODEL_PROVIDER", "--provider", "Authored worker `modelProvider` and `model` values always win", "## Response-stream provider fidelity", "Provider Session", "Factory Session", "FactoryResponseEvent", "Native streaming", "Final-only", "STREAM_GAP", "primaryResult", "byte-identical provider transcripts", "durable process-restart replay", "`you docs run`", "`you docs sessions`", "docs/reference/workers.md"}, absent: []string{"docs/reference/workstations-and-workers.md"}},
	{name: "providers", heading: "# Providers and ACP Agents", markers: []string{"executorProvider", "modelProvider", "cursor-acp", "kiro-acp", "opencode-acp", "you workers list", "you workers acp add", "you workers acp delete", "~/.you-agent-factory/config.json", "skipPermissions", "you factory config validate", "--provider cursor-acp", "@you/goal", "agent.run", "you docs javascript-workflows", "Factory validation checks configuration shape"}},
	{name: "acp", heading: "# Providers and ACP Agents", markers: []string{"you workers acp", "executorProvider", "cursor-acp"}},
...
func TestDocsCommandSmoke_PackagedTopicsRemainAvailableOutsideRepositoryDocsTree(t *testing.T) {
	...
	if got := err.Error(); got != `unsupported docs topic "unknown" (supported: agents, authoring-factories, run, config, factory-validation, mock-workers, record-replay, guards, relationships, work, sessions, orchestrators, javascript-workflows, mcp, workstations, workers, providers, serve-acp, resources, models, batch-inputs, templates)` {
