...
func TestDocsFailureBoundariesAndReuse(t *testing.T) {
	...
	t.Run("writer failure recovers with a fresh buffer", testDocsWriterFailureRecovery)
}

func testDocsUnsupportedTopicRecovery(t *testing.T) {
	...
	result := executeDocumentationCommandResult(t, fixture.process, environment, fixture.tempDir(t), "docs", "unknown")
	...
	valid := executeDocumentationCommand(t, fixture.process, environment, fixture.tempDir(t), "docs", "models")
	if !strings.Contains(valid, "local Models composition") {
		t.Fatalf("valid topic after unsupported input omitted Models content")
	}
}

func testDocsDuplicateTopicRecovery(t *testing.T) {
	fixture := documentationProcess(t)
	environment := isolatedDocumentationEnvironment(t)
	providerCallsBefore := fixture.providerRunner.CallCount()
	result := executeDocumentationCommandResult(t, fixture.process, environment, fixture.tempDir(t), "docs", "models", "models")
...
func testDocsCanonicalAliasReuse(t *testing.T) {
	fixture := documentationProcess(t)
	environment := isolatedDocumentationEnvironment(t)
	commands := []string{"models", "workstation", "batch-work"}
	first := make(map[string]string, len(commands))
	for _, topic := range commands {
		first[topic] = executeDocumentationCommand(t, fixture.process, environment, fixture.tempDir(t), "docs", topic)
	}
	for index := len(commands) - 1; index >= 0; index-- {
		topic := commands[index]
		got := executeDocumentationCommand(t, fixture.process, environment, fixture.tempDir(t), "docs", topic)
		if got != first[topic] {
...
func testDocsWriterFailureRecovery(t *testing.T) {
	fixture := documentationProcess(t)
	environment := isolatedDocumentationEnvironment(t)
	want := executeDocumentationCommand(t, fixture.process, environment, fixture.tempDir(t), "docs", "models")
	writer := &boundedDocumentationWriter{limit: 32}
	var stderr bytes.Buffer
	err := executeDocumentationCommandInto(t, fixture.process, environment, fixture.tempDir(t), writer, &stderr, "docs", "models")
	...
	retry := executeDocumentationCommand(t, fixture.process, environment, fixture.tempDir(t), "docs", "models")
	if retry != want {
