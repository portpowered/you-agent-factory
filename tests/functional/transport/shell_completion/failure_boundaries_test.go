...
func TestCompletionFailureBoundariesAndReuse(t *testing.T) {
	...
	t.Run("duplicate shell input is rejected without partial output", func(t *testing.T) {
		result := executeCompletionCommand(
			t,
			completionProcess(t).invocation(t, false),
			"completion", "bash", "zsh",
	...
	t.Run("scripts and dynamic candidates repeat in both directions", func(t *testing.T) {
		commands := []struct {
			name        string
			args        []string
			withFactory bool
		...
		for _, command := range commands {
			result := executeCompletionCommand(
				t,
				completionProcess(t).invocation(t, command.withFactory),
				command.args...,
			)
	...
	t.Run("writer failure recovers with a fresh buffer", func(t *testing.T) {
		...
		retry := executeCompletionCommand(
			t,
			completionProcess(t).invocation(t, false),
			"completion", "bash",
		)
