...
func TestModelsJoinedBuiltinInvokeWithoutFactoryDeclaration(t *testing.T) {
	...
	if backendResolverCalls != 0 || hostLauncher.Calls() != 0 || protocol.Calls() != 0 || compatibility.Calls() != 0 {
		t.Fatalf("joined validation effects = resolver %d, starts %d, protocol %d, compatibility %d; want 0/0/0/0", backendResolverCalls, hostLauncher.Calls(), protocol.Calls(), compatibility.Calls())
	}
