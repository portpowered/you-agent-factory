...
func TestModelsOmniFileInputsPreserveDetectedTypesAndImageOrderThroughRootBuildProcess(t *testing.T) {
	...
	fileInputs := map[string][]byte{
		filepath.Join(dir, "prompt.txt"): []byte("Compare these inputs"),
		filepath.Join(dir, "a.png"):      []byte("PNG-A"),
		filepath.Join(dir, "b.png"):      []byte("PNG-B"),
		filepath.Join(dir, "voice.wav"):  []byte("RIFF-VOICE"),
		filepath.Join(dir, "clip.mp4"):   []byte("MP4-CLIP"),
	}
	var inputReads []string
	process := support.BuildProcess(t, serviceedges.Edges{
		...
		ModelCLIInputReadFile: func(path string) ([]byte, error) {
			inputReads = append(inputReads, path)
