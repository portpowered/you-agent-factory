...
func TestACPAgentHelperProcess(t *testing.T) {
	...
	if mode != "1" && mode != "fail" && mode != "auth" && mode != "model" && mode != "resource" && mode != "content" && mode != "version" && mode != "init-fail" && mode != "stderr" && mode != "malformed" && mode != "eof" && mode != "block" && mode != "cancelable" && mode != "isolate" && mode != "unsupported" && mode != "persistent" && mode != "serialize" && mode != "crash-once" && mode != "spawn" && mode != "tournament" {
		return
	}
