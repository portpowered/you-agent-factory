...
func cancelConcurrencyWorkerSession(t testing.TB, baseURL, workerSessionID string) factoryapi.WorkerSessionControlResponse {
	t.Helper()
	endpoint := strings.TrimSuffix(baseURL, "/") + "/worker-sessions/" + url.PathEscape(workerSessionID) + "/cancel"
	request, err := http.NewRequest(http.MethodPost, endpoint, strings.NewReader(`{}`))
	if err != nil {
		t.Fatalf("build Worker Session cancel request: %v", err)
	}
	request.Header.Set("Content-Type", "application/json")
	response, err := http.DefaultClient.Do(request)
	if err != nil {
...
func waitForConcurrencyWorkerSession(
	t *testing.T,
	...
	t.Helper()
	wantWorkID := strings.TrimSpace(stringPointerValue(workID))
	if wantWorkID == "" {
		t.Fatal("waitForConcurrencyWorkerSession received empty Work ID")
	}
	result, err := support.WaitForObservation(timeout,
		func() (factoryapi.ListWorkerSessionsResponse, error) {
			return readConcurrencyWorkerSessions(baseURL, sessionID, wantWorkID)
		},
		func(response factoryapi.ListWorkerSessionsResponse) bool {
			return len(response.Sessions) == 1 &&
				response.Sessions[0].WorkerSessionId != "" &&
				(response.Sessions[0].State == factoryapi.WorkerSessionObservationStateRunning || response.Sessions[0].State == factoryapi.WorkerSessionObservationStateStarting)
		},
	)
	if err != nil {
		t.Fatalf("wait for Worker Session for Work %q: %v", wantWorkID, err)
	}
	return result.Sessions[0]
}

func readConcurrencyWorkerSessions(baseURL, sessionID, workID string) (factoryapi.ListWorkerSessionsResponse, error) {
	...
	if response.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(response.Body)
		return factoryapi.ListWorkerSessionsResponse{}, fmt.Errorf("GET %s status = %d: %s", endpoint, response.StatusCode, strings.TrimSpace(string(body)))
	}
