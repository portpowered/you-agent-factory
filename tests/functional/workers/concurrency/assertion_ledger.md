...
| CC-05 | `TestConcurrencySharedProcess/Cancel/CC-05` | Concurrent exact-target API cancels produce one `APPLIED` and one `NOOP`; only A is canceled, its deliberately late success output is absorbed, list/detail both report `CANCELED`, the slot is released once while B remains active, a remote CLI repeat is `NOOP`, unknown IDs remain `NOT_FOUND`, and B completes independently. |
...
that contract should change is owned outside this lane.
